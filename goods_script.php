...
$charset = empty($_GET['charset']) ? XAK_CHARSET : $_GET['charset'];
...
if (!$smarty->is_cached($tpl, $cache_id))
{
    ...
    if ($type == '')
    {
        ...
        $_from = (!empty($_GET['charset']) && $_GET['charset'] != 'UTF8')? urlencode(xak_iconv('UTF-8', 'GBK', $sitename)) : urlencode(@$sitename);
        $goods_url = $xak->url() . 'affiche.php?ad_id=-1&amp;from=' . $_from . '&amp;goods_id=';
    ...
    {
        $uid = (int)$_GET['u'];
        $goods_url = $xak->url() . "goods.php?u=$uid&id=";
        $sql = "SELECT g.goods_id, g.goods_name, g.market_price, g.goods_thumb, IF(g.is_promote = 1 AND '$time' >= g.promote_start_date AND ".
           "'$time' <= g.promote_end_date, g.promote_price, g.shop_price) AS goods_price FROM " . $xak->table('goods') . " g LEFT JOIN " . $xak->table('collect_goods') . " c ON g.goods_id = c.goods_id " .
               " WHERE c.user_id = '$uid'";
    }
    ...
    while ($goods = $db->fetchRow($res))
    {
        ...
        if ($charset != XAK_CHARSET)
        {
            ...
            {
                $tmp_goods_name = htmlentities($goods['goods_name'], ENT_QUOTES, XAK_CHARSET);
            }
            $goods['goods_name']  = xak_iconv(XAK_CHARSET, $charset, $tmp_goods_name);
            $goods['goods_price'] = xak_iconv(XAK_CHARSET, $charset, $goods['goods_price']);
        }
