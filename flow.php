...
if ($_REQUEST['step'] == 'add_to_cart')
{
    ...
    if (empty($goods->spec) AND empty($goods->quick))
    {
        ...
        'LEFT JOIN ' . $GLOBALS['xak']->table('attribute') . ' AS a ON a.attr_id = g.attr_id ' .
...
elseif ($_REQUEST['step'] == 'login')
{
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET')
    {
        ...
        $sql = "SELECT COUNT(*) FROM " . $xak->table('cart') .
    ...
    else
    {
        ...
        if (!empty($_POST['act']) && $_POST['act'] == 'signin')
        {
            ...
            if ($user->login($_POST['username'], $_POST['password'],isset($_POST['remember'])))
            {
                ...
                $sql = "SELECT COUNT(*) FROM " . $xak->table('cart') . " WHERE session_id = '" . SESS_ID . "' ";
                if ($db->getOne($sql) > 0)
                {
                    xak_header("Location: flow.php?step=checkout\n");
                }
                else
                {
        ...
        elseif (!empty($_POST['act']) && $_POST['act'] == 'signup')
        {
            ...
            {
                /* 用户注册成功 */
                xak_header("Location: flow.php?step=consignee\n");
                exit;
            }
            else
            {
...
elseif ($_REQUEST['step'] == 'checkout')
{
    ...
    if (empty($_SESSION['direct_shopping']) && $_SESSION['user_id'] == 0)
    {
        ...
        xak_header("Location: flow.php?step=login\n");
        exit;
    }
    ...
    $sql = 'SELECT count(*) FROM ' . $xak->table('cart') . " WHERE `session_id` = '" . SESS_ID. "' AND `extension_code` != 'package_buy' AND `is_shipping` = 0";
    ...
    if(isset($payment_list))
    {
        foreach ($payment_list as $key => $payment)
        {
            if ($payment['is_cod'] == '1')
            {
                $payment_list[$key]['format_pay_fee'] = '<span id="XAK_CODFEE">' . $payment['format_pay_fee'] . '</span>';
            }
...
elseif ($_REQUEST['step'] == 'done')
{
    ...
    elseif (isset($_POST['bonus_sn']))
    {
        ...
        else
        {
            if ($user_id > 0)
            {
                $sql = "UPDATE " . $xak->table('user_bonus') . " SET user_id = '$user_id' WHERE bonus_id = '$bonus[bonus_id]' LIMIT 1";
                $db->query($sql);
            }
   ...
   /* 判断是不是实体商品 */
    ...
    {
        $sql="SELECT shipping_id FROM " . $xak->table('shipping') . " WHERE shipping_id=".$order['shipping_id'] ." AND enabled =1"; 
        if(!$db->getOne($sql))
        {
    ...
    do
    {
        ...
        $GLOBALS['db']->autoExecute($GLOBALS['xak']->table('order_info'), $order, 'INSERT');
    ...
    {
        $sql = "UPDATE ". $xak->table('goods_activity') ." SET is_finished='2' WHERE act_id=".$order['extension_id'];
        $db->query($sql);
    }
...
{
    $sql = "DELETE FROM " . $xak->table('cart') . " WHERE session_id='" . SESS_ID . "'";
    $db->query($sql);
...
elseif ($_REQUEST['step'] == 'drop_to_collect')
{
    ...
    {
        $rec_id = intval($_GET['id']);
        $goods_id = $db->getOne("SELECT  goods_id FROM " .$xak->table('cart'). " WHERE rec_id = '$rec_id' AND session_id = '" . SESS_ID . "' ");
        $count = $db->getOne("SELECT goods_id FROM " . $xak->table('collect_goods') . " WHERE user_id = '$_SESSION[user_id]' AND goods_id = '$goods_id'");
        if (empty($count))
        {
            $time = gmtime();
...
function flow_update_cart($arr)
{
    ...
    foreach ($arr AS $key => $val)
    {
        ...
        //查询：
        $sql = "SELECT `goods_id`, `goods_attr_id`, `product_id`, `extension_code` FROM" .$GLOBALS['xak']->table('cart').
               " WHERE rec_id='$key' AND session_id='" . SESS_ID . "'";
        $goods = $GLOBALS['db']->getRow($sql);

        $sql = "SELECT g.goods_name, g.goods_number ".
                "FROM " .$GLOBALS['xak']->table('goods'). " AS g, ".
                    $GLOBALS['xak']->table('cart'). " AS c ".
        ...
        if (intval($GLOBALS['_CFG']['use_storage']) > 0 && $goods['extension_code'] != 'package_buy')
        {
            ...
            {
                $sql = "SELECT product_number FROM " .$GLOBALS['xak']->table('products'). " WHERE goods_id = '" . $goods['goods_id'] . "' AND product_id = '" . $goods['product_id'] . "'";
        ...
        $sql = "SELECT b.goods_number, b.rec_id
                FROM " .$GLOBALS['xak']->table('cart') . " a, " .$GLOBALS['xak']->table('cart') . " b
                WHERE a.rec_id = '$key'
        ...
        if ($val > 0)
        {
            ...
            {
                //更新购物车中的商品数量
                $sql = "UPDATE " .$GLOBALS['xak']->table('cart').
...
function flow_cart_stock($arr)
{
    foreach ($arr AS $key => $val)
    {
        ...
        $sql = "SELECT `goods_id`, `goods_attr_id`, `extension_code` FROM" .$GLOBALS['xak']->table('cart').
               " WHERE rec_id='$key' AND session_id='" . SESS_ID . "'";
        $goods = $GLOBALS['db']->getRow($sql);
        ...
        if (intval($GLOBALS['_CFG']['use_storage']) > 0 && $goods['extension_code'] != 'package_buy')
        {
            ...
            {
                $sql = "SELECT product_number FROM " .$GLOBALS['xak']->table('products'). " WHERE goods_id = '" . $goods['goods_id'] . "' AND product_id = '" . $row['product_id'] . "'";
                $product_number = $GLOBALS['db']->getOne($sql);
                if ($product_number < $val)
                {
                    show_message(sprintf($GLOBALS['_LANG']['stock_insufficiency'], $row['goods_name'],
...
{
    /* 取得商品id */
    $sql = "SELECT * FROM " .$GLOBALS['xak']->table('cart'). " WHERE rec_id = '$id'";
    $row = $GLOBALS['db']->getRow($sql);
    if ($row)
    {
        //如果是超值礼包
        ...
        elseif ($row['parent_id'] == 0 && $row['is_gift'] == 0)
        {
            ...
            $sql = "SELECT c.rec_id
                    FROM " . $GLOBALS['xak']->table('cart') . " AS c, " . $GLOBALS['xak']->table('group_goods') . " AS gg, " . $GLOBALS['xak']->table('goods'). " AS g
...
function flow_clear_cart_alone()
{
    ...
    $sql = "SELECT c.rec_id, gg.parent_id
            FROM " . $GLOBALS['xak']->table('cart') . " AS c
                LEFT JOIN " . $GLOBALS['xak']->table('group_goods') . " AS gg ON c.goods_id = gg.goods_id
                LEFT JOIN" . $GLOBALS['xak']->table('goods') . " AS g ON c.goods_id = g.goods_id
...
function favourable_list($user_rank)
{
    ...
    while ($favourable = $GLOBALS['db']->fetchRow($res))
    {
        ...
        foreach ($favourable['gift'] as $key => $value)
        {
            ...
            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['xak']->table('goods') . " WHERE is_on_sale = 1 AND goods_id = ".$value['id'];
...
function act_range_desc($favourable)
{
    ...
    {
        $sql = "SELECT brand_name FROM " . $GLOBALS['xak']->table('brand') .
                " WHERE brand_id " . db_create_in($favourable['act_range_ext']);
        return join(',', $GLOBALS['db']->getCol($sql));
    }
    elseif ($favourable['act_range'] == FAR_CATEGORY)
    {
        $sql = "SELECT cat_name FROM " . $GLOBALS['xak']->table('category') .
                " WHERE cat_id " . db_create_in($favourable['act_range_ext']);
        return join(',', $GLOBALS['db']->getCol($sql));
    }
    elseif ($favourable['act_range'] == FAR_GOODS)
    {
        $sql = "SELECT goods_name FROM " . $GLOBALS['xak']->table('goods') .
...
{
    $sql = "INSERT INTO " . $GLOBALS['xak']->table('cart') . " (" .
