...
elseif ($_REQUEST['act'] == 'view')
{
    ...
    //更新商品点击次数
    $sql = 'UPDATE ' . $xak->table('goods') . ' SET click_count = click_count + 1 '.
...
elseif ($_REQUEST['act'] == 'bid')
{
    ...
    $db->autoExecute($xak->table('auction_log'), $auction_log, 'INSERT');

    /* 出价是否等于一口价 */
    if ($bid_price == $auction['end_price'])
    {
        /* 结束拍卖活动 */
        $sql = "UPDATE " . $xak->table('goods_activity') . " SET is_finished = 1 WHERE act_id = '$id' LIMIT 1";
        $db->query($sql);
    }

    /* 跳转到活动详情页 */
    xak_header("Location: auction.php?act=view&id=$id\n");
    exit;
}
...
function auction_list($size, $page)
{
    ...
    $sql = "SELECT a.*, IFNULL(g.goods_thumb, '') AS goods_thumb " .
            "FROM " . $GLOBALS['xak']->table('goods_activity') . " AS a " .
                "LEFT JOIN " . $GLOBALS['xak']->table('goods') . " AS g ON a.goods_id = g.goods_id " .
