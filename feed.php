...
header('Content-Type: application/xml; charset=' . XAK_CHARSET);
...
$rss = new RSSBuilder(XAK_CHARSET, $uri, htmlspecialchars($_CFG['shop_name']), htmlspecialchars($_CFG['shop_desc']), $uri . 'animated_favicon.gif');
$rss->addDCdata('', 'http://www.xak.cn', date('r'));
...
else
{
    ...
    $sql = 'SELECT c.cat_name, g.goods_id, g.goods_name, g.goods_brief, g.last_update ' .
            'FROM ' . $xak->table('category') . ' AS c, ' . $xak->table('goods') . ' AS g ' .
