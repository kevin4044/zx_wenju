...
if (in_array($action, $ui_arr))
{
    ...
    $sql = "SELECT value FROM " . $xak->table('shop_config') . " WHERE id = 419";
    $row = $db->getRow($sql);
    $car_off = $row['value'];
...
if ($action == 'register')
{
    ...
    /* 取出注册扩展字段 */
    $sql = 'SELECT * FROM ' . $xak->table('reg_fields') . ' WHERE type < 2 AND display = 1 ORDER BY dis_order, id';
...
elseif ($action == 'act_register')
{
    ...
    else
    {
        ...
        {
            /*把新注册用户的扩展信息插入数据库*/
            $sql = 'SELECT id FROM ' . $xak->table('reg_fields') . ' WHERE type = 0 AND display = 1 ORDER BY dis_order, id';   //读出所有自定义扩展字段的id
            ...
            {
                $sql = 'INSERT INTO '. $xak->table('reg_extend_info') . ' (`user_id`, `reg_field_id`, `content`) VALUES' . $extend_field_str;
                $db->query($sql);
            }
            ...
            {
                $sql = 'UPDATE ' . $xak->table('users') . " SET `passwd_question`='$sel_question', `passwd_answer`='$passwd_answer'  WHERE `user_id`='" . $_SESSION['user_id'] . "'";
                $db->query($sql);
            }
            /* 判断是否需要自动发送注册邮件 */
...
elseif ($action == 'validate_email')
{
    ...
    if ($hash)
    {
        ...
        if ($id > 0)
        {
            $sql = "UPDATE " . $xak->table('users') . " SET is_validated = 1 WHERE user_id='$id'";
            $db->query($sql);
            $sql = 'SELECT user_name, email FROM ' . $xak->table('users') . " WHERE user_id = '$id'";
            $row = $db->getRow($sql);
...
elseif ($action == 'profile')
{
    ...
    $sql = 'SELECT reg_field_id, content ' .
           'FROM ' . $xak->table('reg_extend_info') .
...
elseif ($action == 'act_edit_profile')
{
    ...
    /* 更新用户扩展字段的数据 */
    $sql = 'SELECT id FROM ' . $xak->table('reg_fields') . ' WHERE type = 0 AND display = 1 ORDER BY dis_order, id';   //读出所有扩展字段的id
    $fields_arr = $db->getAll($sql);
    ...
    foreach ($fields_arr AS $val)       //循环更新扩展用户信息
    {
        ...
        if(isset($_POST[$extend_field_index]))
        {
            ...
            $sql = 'SELECT * FROM ' . $xak->table('reg_extend_info') . "  WHERE reg_field_id = '$val[id]' AND user_id = '$user_id'";
            if ($db->getOne($sql))      //如果之前没有记录，则插入
            {
                $sql = 'UPDATE ' . $xak->table('reg_extend_info') . " SET content = '$temp_field_content' WHERE reg_field_id = '$val[id]' AND user_id = '$user_id'";
            }
            else
            {
                $sql = 'INSERT INTO '. $xak->table('reg_extend_info') . " (`user_id`, `reg_field_id`, `content`) VALUES ('$user_id', '$val[id]', '$temp_field_content')";
            }
            $db->query($sql);
        }
    }
...
elseif ($action == 'get_passwd_question')
{
    ...
    //取出会员密码问题和答案
    $sql = 'SELECT user_id, user_name, passwd_question, passwd_answer FROM ' . $xak->table('users') . " WHERE user_name = '" . $user_name . "'";
...
elseif ($action == 'act_edit_password')
{
    ...
    if (($user_info && (!empty($code) && md5($user_info['user_id'] . $_CFG['hash_code'] . $user_info['reg_time']) == $code)) || ($_SESSION['user_id']>0 && $_SESSION['user_id'] == $user_id && $user->check_user($_SESSION['user_name'], $old_password)))
    {
        ...
        {
			$sql="UPDATE ".$xak->table('users'). "SET `ec_salt`='0' WHERE user_id= '".$user_id."'";
			$db->query($sql);
            $user->logout();
...
elseif ($action == 'order_list')
{
    ...
    $record_count = $db->getOne("SELECT COUNT(*) FROM " .$xak->table('order_info'). " WHERE user_id = '$user_id'");
...
elseif ($action == 'cancel_order')
{
    ...
    {
        xak_header("Location: user.php?act=order_list\n");
        exit;
    }
    else
    {
...
elseif ($action == 'address_list')
{
    ...
    /* 获取默认收货ID */
    $address_id  = $db->getOne("SELECT address_id FROM " .$xak->table('users'). " WHERE user_id='$user_id'");
...
elseif ($action == 'drop_consignee')
{
    ...
    {
        xak_header("Location: user.php?act=address_list\n");
        exit;
    }
    else
    {
...
elseif ($action == 'collection_list')
{
    ...
    $record_count = $db->getOne("SELECT COUNT(*) FROM " .$xak->table('collect_goods').
                                " WHERE user_id='$user_id' ORDER BY add_time DESC");

    $pager = get_pager('user.php', array('act' => $action), $record_count, $page);
    $smarty->assign('pager', $pager);
    $smarty->assign('goods_list', get_collection_goods($user_id, $pager['size'], $pager['start']));
    $smarty->assign('url',        $xak->url());
    $lang_list = array(
...
elseif ($action == 'delete_collection')
{
    ...
    if ($collection_id > 0)
    {
        $db->query('DELETE FROM ' .$xak->table('collect_goods'). " WHERE rec_id='$collection_id' AND user_id ='$user_id'" );
    }

    xak_header("Location: user.php?act=collection_list\n");
    exit;
}
...
elseif ($action == 'add_to_attention')
{
    ...
    if ($rec_id)
    {
        $db->query('UPDATE ' .$xak->table('collect_goods'). "SET is_attention = 1 WHERE rec_id='$rec_id' AND user_id ='$user_id'" );
    }
    xak_header("Location: user.php?act=collection_list\n");
    exit;
}
...
elseif ($action == 'del_attention')
{
    ...
    if ($rec_id)
    {
        $db->query('UPDATE ' .$xak->table('collect_goods'). "SET is_attention = 0 WHERE rec_id='$rec_id' AND user_id ='$user_id'" );
    }
    xak_header("Location: user.php?act=collection_list\n");
    exit;
}
...
elseif ($action == 'message_list')
{
    ...
    if ($order_id)
    {
        $sql = "SELECT COUNT(*) FROM " .$xak->table('feedback').
                " WHERE parent_id = 0 AND order_id = '$order_id' AND user_id = '$user_id'";
        $order_info = $db->getRow("SELECT * FROM " . $xak->table('order_info') . " WHERE order_id = '$order_id' AND user_id = '$user_id'");
...
elseif ($action == 'comment_list')
{
    ...
    /* 获取用户留言的数量 */
    $sql = "SELECT COUNT(*) FROM " .$xak->table('comment').
...
elseif ($action == 'act_del_tag')
{
    ...
    xak_header("Location: user.php?act=tag_list\n");
    exit;
...
elseif ($action == 'booking_list')
{
    ...
    /* 获取缺货登记的数量 */
    $sql = "SELECT COUNT(*) " .
            "FROM " .$xak->table('booking_goods'). " AS bg, " .
                     $xak->table('goods') . " AS g " .
...
elseif ($action == 'act_del_booking')
{
    ...
    if ($id == 0 || $user_id == 0)
    {
        xak_header("Location: user.php?act=booking_list\n");
        exit;
    }
...
elseif ($action == 'account_detail')
{
    ...
    /* 获取记录条数 */
    $sql = "SELECT COUNT(*) FROM " .$xak->table('account_log').
           " WHERE user_id = '$user_id'" .
           " AND $account_type <> 0 ";
    $record_count = $db->getOne($sql);
    ...
    //获取余额记录
    $account_log = array();
    $sql = "SELECT * FROM " . $xak->table('account_log') .
           " WHERE user_id = '$user_id'" .
           " AND $account_type <> 0 " .
...
elseif ($action == 'account_log')
{
    ...
    /* 获取记录条数 */
    $sql = "SELECT COUNT(*) FROM " .$xak->table('user_account').
           " WHERE user_id = '$user_id'" .
...
elseif ($action == 'cancel')
{
    ...
    if ($id == 0 || $user_id == 0)
    {
        xak_header("Location: user.php?act=account_log\n");
        exit;
    }
...
elseif ($action == 'pay')
{
    ...
    if ($payment_id == 0)
    {
        xak_header("Location: user.php?act=account_deposit&id=".$surplus_id."\n");
        exit;
    }
    ...
    if (!empty($payment_info))
    {
        ...
        $order_amount = $db->getOne("SELECT order_amount FROM " .$xak->table('pay_log')." WHERE log_id = '$order[log_id]'");
        if ($order_amount <> $order['order_amount'])
        {
            $db->query("UPDATE " .$xak->table('pay_log').
...
elseif ($action == 'collect')
{
    ...
    {
        /* 检查是否已经存在于用户的收藏夹 */
        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['xak']->table('collect_goods') .
        ...
        {
            $time = gmtime();
            $sql = "INSERT INTO " .$GLOBALS['xak']->table('collect_goods'). " (user_id, goods_id, add_time)" .
...
elseif ($action == 'del_msg')
{
    ...
    if ($id > 0)
    {
        $sql = 'SELECT user_id, message_img FROM ' .$xak->table('feedback'). " WHERE msg_id = '$id' LIMIT 1";
        $row = $db->getRow($sql);
        if ($row && $row['user_id'] == $user_id)
        {
            ...
            $sql = "DELETE FROM " .$xak->table('feedback'). " WHERE msg_id = '$id' OR parent_id = '$id'";
            $db->query($sql);
        }
    }
    xak_header("Location: user.php?act=message_list&order_id=$order_id\n");
    exit;
}
...
elseif ($action == 'del_cmt')
{
    ...
    if ($id > 0)
    {
        $sql = "DELETE FROM " .$xak->table('comment'). " WHERE comment_id = '$id' AND user_id = '$user_id'";
        $db->query($sql);
    }
    xak_header("Location: user.php?act=comment_list\n");
    exit;
}
...
elseif ($action == 'return_to_cart')
{
    ...
    /* 检查订单是否属于该用户 */
    $order_user = $db->getOne("SELECT user_id FROM " .$xak->table('order_info'). " WHERE order_id = '$order_id'");
    if (empty($order_user))
    {
        $result['error'] = 1;
...
elseif ($action == 'act_edit_surplus')
{
    ...
    /* 跳转 */
    xak_header('Location: user.php?act=order_detail&order_id=' . $order_id . "\n");
    exit;
}
...
elseif ($action == 'act_edit_payment')
{
    ...
    $sql = "UPDATE " . $xak->table('order_info') .
...
elseif ($action == 'save_order_address')
{
    ...
    {
        xak_header('Location: user.php?act=order_detail&order_id=' .$address['order_id']. "\n");
        exit;
    }
    else
    {
...
elseif ($action == 'bonus')
{
    ...
    $page = isset($_REQUEST['page']) ? intval($_REQUEST['page']) : 1;
    $record_count = $db->getOne("SELECT COUNT(*) FROM " .$xak->table('user_bonus'). " WHERE user_id = '$user_id'");
    ...
    if(empty($goodsid))
    {
        ...
        if(empty($affiliate['config']['separate_by']))
        {
            ...
            for ($i = 1 ; $i <=$num ;$i++)
            {
                ...
                if ($up_uid)
                {
                    $sql = "SELECT user_id FROM " . $xak->table('users') . " WHERE parent_id IN($up_uid)";
                    $query = $db->query($sql);
                    $up_uid = '';
            ...
            $sqlcount = "SELECT count(*) FROM " . $xak->table('order_info') . " o".
        " LEFT JOIN".$xak->table('users')." u ON o.user_id = u.user_id".
        " LEFT JOIN " . $xak->table('affiliate_log') . " a ON o.order_id = a.order_id" .
        " WHERE o.user_id > 0 AND (u.parent_id IN ($all_uid) AND o.is_separate = 0 OR a.user_id = '$user_id' AND o.is_separate > 0)";

            $sql = "SELECT o.*, a.log_id, a.user_id as suid,  a.user_name as auser, a.money, a.point, a.separate_type FROM " . $xak->table('order_info') . " o".
                    " LEFT JOIN".$xak->table('users')." u ON o.user_id = u.user_id".
                    " LEFT JOIN " . $xak->table('affiliate_log') . " a ON o.order_id = a.order_id" .
        " WHERE o.user_id > 0 AND (u.parent_id IN ($all_uid) AND o.is_separate = 0 OR a.user_id = '$user_id' AND o.is_separate > 0)".
        ...
        else
        {
            ...
            $sql = "SELECT o.*, a.log_id,a.user_id as suid, a.user_name as auser, a.money, a.point, a.separate_type,u.parent_id as up FROM " . $xak->table('order_info') . " o".
                    " LEFT JOIN".$xak->table('users')." u ON o.user_id = u.user_id".
                    " LEFT JOIN " . $xak->table('affiliate_log') . " a ON o.order_id = a.order_id" .
                    " WHERE o.user_id > 0 AND (o.parent_id = '$user_id' AND o.is_separate = 0 OR a.user_id = '$user_id' AND o.is_separate > 0)" .
                    " ORDER BY order_id DESC" ;
...
elseif ($action =='email_list')
{
    ...
    $ck = $db->getRow("SELECT * FROM " . $xak->table('email_list') . " WHERE email = '$email'");
    if ($job == 'add')
    {
        if (empty($ck))
        {
            $hash = substr(md5(time()), 1, 10);
            $sql = "INSERT INTO " . $xak->table('email_list') . " (email, stat, hash) VALUES ('$email', 0, '$hash')";
            $db->query($sql);
            $info = $_LANG['email_check'];
            $url = $xak->url() . "user.php?act=email_list&job=add_check&hash=$hash&email=$email";
        ...
        {
            $hash = substr(md5(time()),1 , 10);
            $sql = "UPDATE " . $xak->table('email_list') . "SET hash = '$hash' WHERE email = '$email'";
            $db->query($sql);
    ...
    elseif ($job == 'del')
    {
        ...
        elseif ($ck['stat'] == 1)
        {
            ...
            $db->query($sql);
            $info = $_LANG['email_check'];
            $url = $xak->url() . "user.php?act=email_list&job=del_check&hash=$hash&email=$email";
            send_mail('', $email, $_LANG['check_mail'], sprintf($_LANG['check_mail_content'], $email, $_CFG['shop_name'], $url, $url, $_CFG['shop_name'], local_date('Y-m-d')), 1);
        }
        else
        {
    ...
    elseif ($job == 'add_check')
    {
        ...
        else
        {
            ...
            {
                $sql = "UPDATE " . $xak->table('email_list') . "SET stat = 1 WHERE email = '$email'";
                $db->query($sql);
                $info = $_LANG['email_checked'];
            }
            else
    ...
    elseif ($job == 'del_check')
    {
        ...
        elseif ($ck['stat'] == 1)
        {
            if ($_GET['hash'] == $ck['hash'])
            {
                $sql = "DELETE FROM " . $xak->table('email_list') . "WHERE email = '$email'";
                $db->query($sql);
...
else if ($action == 'track_packages')
{
    ...
    $sql = "SELECT order_id,order_sn,invoice_no,shipping_id FROM " .$xak->table('order_info').
...
else if ($action == 'order_query')
{
    ...
    {
        $sql = "SELECT shipping_code FROM " . $xak->table('shipping') . " WHERE shipping_id = '$row[shipping_id]'";
...
elseif ($action == 'transform_points')
{
    ...
    $sql = "SELECT user_id, user_name, pay_points, rank_points FROM " . $xak->table('users')  . " WHERE user_id='$user_id'";
    $row = $db->getRow($sql);
...
elseif ($action == 'act_transform_points')
{
    ...
    /* 取出用户数据 */
    $sql = "SELECT user_name, user_id, pay_points, rank_points FROM " . $xak->table('users') . " WHERE user_id='$user_id'";
    $row = $db->getRow($sql);
...
elseif ($action == 'act_transform_ucenter_points')
{
    ...
    if ($result === true)
    {
        $sql = "UPDATE " . $xak->table('users') . " SET {$shop_points[$fromcredits]}={$shop_points[$fromcredits]}-'$exchange_amount' WHERE user_id='{$row['user_id']}'";
        $db->query($sql);
        $sql = "INSERT INTO " . $xak->table('account_log') . "(user_id, {$shop_points[$fromcredits]}, change_time, change_desc, change_type)" . " VALUES ('{$row['user_id']}', '-$exchange_amount', '". gmtime() ."', '" . $cfg['uc_lang']['exchange'] . "', '98')";
        $db->query($sql);
