...
$hash_code = $db->getOne("SELECT `value` FROM " . $xak->table('shop_config') . " WHERE `code`='hash_code'", true);
...
switch ($action)
{
    case 'get_goods_info':
    {
        ...
        $sql = "SELECT `goods_id`, `goods_name`, `goods_number`, `shop_price`, `keywords`, `goods_brief`, `goods_thumb`, `goods_img`, `last_update` FROM " . $xak->table('goods') . " WHERE `is_delete`='0' ORDER BY `goods_id` ASC $limit ";
    ...
    case 'get_shop_info':
    {
        ...
        $sql = "SELECT `value` FROM " . $xak->table('shop_config') . " WHERE code='currency_format'";
        $currency_format = $db->getOne($sql);
        $sql = "SELECT r.region_name, sc.value FROM " . $xak->table('region') . " AS r INNER JOIN " . $xak->table('shop_config') . " AS sc ON r.`region_id`=sc.`value` WHERE sc.`code`='shop_country' OR sc.`code`='shop_province' OR sc.`code`='shop_city' ORDER BY sc.`id` ASC";
    ...
    case 'get_shipping':
    {
        ...
        $sql = "SELECT `shipping_id`, `shipping_name`, `insure` FROM " . $xak->table('shipping');
    ...
    case 'get_goods_attribute':
    {
        ...
        if (!empty($goods_id))
        {
            $sql = "SELECT t2.attr_name, t1.attr_value FROM " . $xak->table('goods_attr') . " AS t1 LEFT JOIN " . $xak->table('attribute') . " AS t2 ON t1.attr_id=t2.attr_id WHERE t1.goods_id='$goods_id'";
            $result = $db->getAll($sql);
            if (!empty($result))
            {
                $results['result'] = 'true';
