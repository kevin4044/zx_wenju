<?php
    ...
    {
        if (defined('XAK_CHARSET') && XAK_CHARSET == 'gbk')
        {
            $str = addslashes(stripslashes(xak_iconv('utf-8', 'gbk', $str)));
        }
    ...
    function show_json(&$json, $array, $convert = false)
    {
        ...
        if (!$convert && defined('XAK_CHARSET') && XAK_CHARSET == 'gbk')
        {
            $json_str = xak_iconv('UTF-8', 'GBK', $json_str);
        }
    ...
    {
        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['xak']->table('category').
    ...
    {
        return str_replace('/api.php', '', preg_replace("/\/api\/client/", '', $GLOBALS['xak']->url())) . $img_url;
    }
    ...
    function API_UserLogin($post)
    {
        ...
        " FROM " . $GLOBALS['xak']->table('admin_user') .
        ...
        if ($row)
        {
            ...
            // 更新最后登录时间和IP
            $GLOBALS['db']->query("UPDATE " .$GLOBALS['xak']->table('admin_user').
            " SET last_login='" . gmtime() . "', last_ip='" . real_ip() . "'".
            " WHERE user_id='$_SESSION[admin_id]'");
            client_show_message(100, true, VERSION, 0, true, XAK_CHARSET);
        }
        else
        {
            client_show_message(103);
    ...
    function API_AddCategory($post)
    {
        ...
        if ($GLOBALS['db']->autoExecute($GLOBALS['xak']->table('category'), $cat) !== false)
        {
            $insert_id = $GLOBALS['db']->insert_id();
            if($cat['show_in_nav'] == 1)
            {
                $vieworder = $GLOBALS['db']->getOne("SELECT max(vieworder) FROM ". $GLOBALS['xak']->table('nav') . " WHERE type = 'middle'");
                $vieworder += 2;
                //显示在自定义导航栏中
                $sql = "INSERT INTO " . $GLOBALS['xak']->table('nav') .
    ...
    function API_GetCategory($post)
    {
        $sql = "SELECT c.cat_id, c.cat_name, c.keywords, c.cat_desc, c.parent_id, c.sort_order, c.measure_unit, c.show_in_nav, c.style, c.is_show, c.grade, c.filter_attr, COUNT(s.cat_id) AS has_children ".
                'FROM ' . $GLOBALS['xak']->table('category') . " AS c ".
                "LEFT JOIN " . $GLOBALS['xak']->table('category') . " AS s ON s.parent_id=c.cat_id ".
                " GROUP BY c.cat_id ".
    ...
    function API_AddBrand($post)
    {
        ...
        /*检查品牌名是否重复*/
        $exc = new exchange($GLOBALS['xak']->table("brand"), $GLOBALS['db'], 'brand_id', 'brand_name');
        ...
        $sql = "INSERT INTO ".$GLOBALS['xak']->table('brand')."(brand_name, site_url, brand_desc, brand_logo, is_show, sort_order) ".
    ...
    {
        $sql = "SELECT brand_id, brand_name, brand_logo, brand_desc, site_url, is_show FROM ".$GLOBALS['xak']->table('brand')." ORDER BY sort_order ASC";
        $result = $GLOBALS['db']->getAllCached($sql);
    ...
    function API_AddGoods($post)
    {
        ...
        if (!$is_insert)
        {
            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['xak']->table('goods') .
        ...
        {
            $max_id     = $is_insert ? $GLOBALS['db']->getOne("SELECT MAX(goods_id) + 1 FROM ".$GLOBALS['xak']->table('goods')) : $_POST['goods_id'];
       ...
       if($catgory_id == '' && $new_cat_name != '')
        ...
        if($catgory_id == '' && $new_cat_name != '')
        {
            $sql = "INSERT INTO " . $GLOBALS['xak']->table('category') . "(cat_name, parent_id, is_show)" .
        ...
        //处理快速添加品牌
         if($brand_id == '' && $new_brand_name != '')
        {
            $sql = "INSERT INTO ".$GLOBALS['xak']->table('brand')."(brand_name) " . "VALUES ('$new_brand_name')";
            $GLOBALS['db']->query($sql);
        ...
        if ($is_insert)
        {
            if ($code == '')
            {
                $sql = "INSERT INTO " . $GLOBALS['xak']->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
        ...
        else
        {
            ...
            $sql = "UPDATE " . $GLOBALS['xak']->table('goods') . " SET " .
        ...
        if (isset($_POST['attr_id_list']) && isset($_POST['attr_value_list']))
        {
            ...
            $sql = "SELECT attr_id, attr_index FROM " . $GLOBALS['xak']->table('attribute') . " WHERE cat_id = '$goods_type' ";
            ...
            $sql = "SELECT * FROM " . $GLOBALS['xak']->table('goods_attr') . " WHERE goods_id = '$goods_id' ";
            ...
            $sql = "UPDATE " .$GLOBALS['xak']->table('goods'). " SET keywords = '$keywords' WHERE goods_id = '$goods_id' LIMIT 1";
            ...
            foreach ($goods_attr_list as $attr_id => $attr_value_list)
            {
                foreach ($attr_value_list as $attr_value => $info)
                {
                    if ($info['sign'] == 'insert')
                    {
                        $sql = "INSERT INTO " .$GLOBALS['xak']->table('goods_attr'). " (attr_id, goods_id, attr_value, attr_price)".
                                "VALUES ('$attr_id', '$goods_id', '$attr_value', '$info[attr_price]')";
                    }
                    elseif ($info['sign'] == 'update')
                    {
                        $sql = "UPDATE " .$GLOBALS['xak']->table('goods_attr'). " SET attr_price = '$info[attr_price]' WHERE goods_attr_id = '$info[goods_attr_id]' LIMIT 1";
                    }
                    else
                    {
                        $sql = "DELETE FROM " .$GLOBALS['xak']->table('goods_attr'). " WHERE goods_attr_id = '$info[goods_attr_id]' LIMIT 1";
                    }
                    $GLOBALS['db']->query($sql);
                }
            }
        ...
        */
        ...
        if (!$is_insert && isset($_POST['old_img_desc']))
        {
            ...
            {
                $sql = "UPDATE " . $GLOBALS['xak']->table('goods_gallery') . " SET img_desc = '$img_desc' WHERE img_id = '$img_id' LIMIT 1";
                $GLOBALS['db']->query($sql);
            }
        }
    ...
    function API_DeleteBrand($post)
    {
        ...
        $exc = new exchange($GLOBALS['xak']->table("brand"), $GLOBALS['db'], 'brand_id', 'brand_name');
        $brand = $GLOBALS['db']->getRow("SELECT brand_logo FROM " . $GLOBALS['xak']->table('brand') . " WHERE brand_id='$brand_id'");
        ...
        /* 更新商品的品牌编号 */
        $sql = "UPDATE " .$GLOBALS['xak']->table('goods'). " SET brand_id=0 WHERE brand_id='$brand_id'";
        $GLOBALS['db']->query($sql);
        client_show_message(0, true);
    }
    ...
    function API_DeleteCategory($post)
    {
        ...
        $cat_name = $GLOBALS['db']->getOne('SELECT cat_name FROM ' .$GLOBALS['xak']->table('category'). " WHERE cat_id='$cat_id'");

        /* 当前分类下是否有子分类 */
        $cat_count = $GLOBALS['db']->getOne('SELECT COUNT(*) FROM ' .$GLOBALS['xak']->table('category'). " WHERE parent_id='$cat_id'");

        /* 当前分类下是否存在商品 */
        $goods_count = $GLOBALS['db']->getOne('SELECT COUNT(*) FROM ' .$GLOBALS['xak']->table('goods'). " WHERE cat_id='$cat_id'");
        /* 如果不存在下级子分类或商品，则删除之 */
        if ($cat_count == 0 && $goods_count == 0)
        {
            /* 删除分类 */
            $sql = 'DELETE FROM ' .$GLOBALS['xak']->table('category'). " WHERE cat_id = '$cat_id'";
            if ($GLOBALS['db']->query($sql))
            {
                $GLOBALS['db']->query("DELETE FROM " . $GLOBALS['xak']->table('nav') . "WHERE ctype = 'c' AND cid = '" . $cat_id . "' AND type = 'middle'");
                clear_cache_files();
    ...
    {
        require_once(ROOT_PATH . ADMIN_PATH . '/includes/cls_exchange.php');
        $exc = new exchange($GLOBALS['xak']->table("goods"), $GLOBALS['db'], 'goods_id', 'goods_name');
    ...
    function API_EditCategory($post)
    {
        ...
        $dat = $GLOBALS['db']->getRow("SELECT cat_name, show_in_nav FROM ". $GLOBALS['xak']->table('category') . " WHERE cat_id = '$cat_id'");

        if ($GLOBALS['db']->autoExecute($GLOBALS['xak']->table('category'), $cat, 'UPDATE', "cat_id='$cat_id'"))
        {
            if($cat['cat_name'] != $dat['cat_name'])
            {
                //如果分类名称发生了改变
                $sql = "UPDATE " . $GLOBALS['xak']->table('nav') . " SET name = '" . $cat['cat_name'] . "' WHERE ctype = 'c' AND cid = '" . $cat_id . "' AND type = 'middle'";
                $GLOBALS['db']->query($sql);
            }
            if($cat['show_in_nav'] != $dat['show_in_nav'])
            {
                ...
                {
                    //显示
                    $nid = $GLOBALS['db']->getOne("SELECT id FROM ". $GLOBALS['xak']->table('nav') . " WHERE ctype = 'c' AND cid = '" . $cat_id . "' AND type = 'middle'");
                    if(empty($nid))
                    {
                        //不存在
                        $vieworder = $GLOBALS['db']->getOne("SELECT max(vieworder) FROM ". $GLOBALS['xak']->table('nav') . " WHERE type = 'middle'");
                        ...
                        $sql = "INSERT INTO " . $GLOBALS['xak']->table('nav') . " (name,ctype,cid,ifshow,vieworder,opennew,url,type) VALUES('" . $cat['cat_name'] . "', 'c', '$cat_id','1','$vieworder','0', '" . $uri . "','middle')";
                    }
                    else
                    {
                        $sql = "UPDATE " . $GLOBALS['xak']->table('nav') . " SET ifshow = 1 WHERE ctype = 'c' AND cid = '" . $cat_id . "' AND type = 'middle'";
                    }
                    $GLOBALS['db']->query($sql);
                }
                else
                {
                    //去除
                    $GLOBALS['db']->query("UPDATE " . $GLOBALS['xak']->table('nav') . " SET ifshow = 0 WHERE ctype = 'c' AND cid = '" . $cat_id . "' AND type = 'middle'");
                }
            }
        }
        /* 更新分類信息成功 */
