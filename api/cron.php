...
function get_cron_info()
{
    ...
    $sql   = "SELECT * FROM " . $GLOBALS['xak']->table('crons') . " WHERE enable = 1 AND nextime < $GLOBALS[timestamp]";
...
function write_error_arr($err_arr)
{
    if (!empty($err_arr))
    {
        ...
        if ($query)
        {
            $sql = "INSERT INTO " . $GLOBALS['xak']->table('error_log') . "(info, file, time) VALUES " . $query;
