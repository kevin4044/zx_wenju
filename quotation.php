...
if ($action == 'print_quotation')
{
    ...
    " FROM " . $xak->table('goods') . " AS g LEFT JOIN " . $xak->table('category') . " AS c ON g.cat_id = c.cat_id LEFT JOIN ".$xak->table('products')."as p  On g.goods_id=p.goods_id" . $where . " AND is_on_sale = 1 AND is_alone_sale = 1 ";
    ...
    $user_rank = $db->getAll("SELECT * FROM " .$xak->table('user_rank') . "WHERE show_price = 1 OR rank_id = '$_SESSION[user_rank]'");
    $rank_point = 0;
    if (!empty($_SESSION['user_id']))
    {
        $rank_point = $db->getOne("SELECT rank_points FROM " . $xak->table('users') . " WHERE user_id = '$_SESSION[user_id]'");
    }
...
function product_info($goods_attr,$goods_id)
{
    ...
    $sql="SELECT attr_value,attr_price FROM ". $GLOBALS['xak']->table('goods_attr') ." WHERE goods_id='$goods_id' AND (goods_attr_id = $goods_attr)";
