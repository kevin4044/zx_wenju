...
$exc = new exchange($xak->table("attribute"), $db, 'attr_id', 'attr_name');
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    {
        $sql = "SELECT * FROM " . $xak->table('attribute') . " WHERE attr_id = '$_REQUEST[attr_id]'";
        $attr = $db->getRow($sql);
    }
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if ($is_insert)
    {
        $db->autoExecute($xak->table('attribute'), $attr, 'INSERT');
    ...
    {
        $db->autoExecute($xak->table('attribute'), $attr, 'UPDATE', "attr_id = '$_POST[attr_id]'");
...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    if (isset($_POST['checkboxes']))
    {
        ...
        $sql = "DELETE FROM " . $xak->table('attribute') . " WHERE attr_id " . db_create_in($ids);
        $db->query($sql);

        $sql = "DELETE FROM " . $xak->table('goods_attr') . " WHERE attr_id " . db_create_in($ids);
        $db->query($sql);
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $db->query("DELETE FROM " .$xak->table('attribute'). " WHERE attr_id='$id'");
    $db->query("DELETE FROM " .$xak->table('goods_attr'). " WHERE attr_id='$id'");
...
elseif ($_REQUEST['act'] == 'get_attr_num')
{
    ...
    $sql = "SELECT COUNT(*) ".
           " FROM " . $xak->table('goods_attr') . " AS a, ".
...
function get_attrlist()
{
    ...
    $sql = "SELECT COUNT(*) FROM " . $GLOBALS['xak']->table('attribute') . " AS a $where";
    ...
    $sql = "SELECT a.*, t.cat_name " .
            ...
            " LEFT JOIN " . $GLOBALS['xak']->table('goods_type') . " AS t ON a.cat_id = t.cat_id " . $where .
