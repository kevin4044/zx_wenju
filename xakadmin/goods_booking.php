...
if ($_REQUEST['act'] == 'remove')
{
    ...
    $db->query("DELETE FROM " .$xak->table('booking_goods'). " WHERE rec_id='$id'");
...
if ($_REQUEST['act'] =='update')
{
    ...
    $sql = "UPDATE  ".$xak->table('booking_goods').
    ...
    if (!empty($_POST['send_email_notice']) or isset($_POST['remail']))
    {
        ...
        $sql = 'SELECT bg.email, bg.link_man, bg.goods_id, g.goods_name ' .
               'FROM ' .$xak->table('booking_goods'). ' AS bg, ' .$xak->table('goods'). ' AS g ' .
        ...
        $goods_link = $xak->url() . 'goods.php?id=' . $booking_info['goods_id'];
    ...
    xak_header("Location: ?act=detail&id=".$_REQUEST['rec_id']."&send_ok=$send_ok\n");
    exit;
}
...
function get_bookinglist()
{
    ...
    $sql = 'SELECT COUNT(*) FROM ' .$GLOBALS['xak']->table('booking_goods'). ' AS bg, '.
    ...
    $sql = 'SELECT bg.rec_id, bg.link_man, g.goods_id, g.goods_name, bg.goods_number, bg.booking_time, bg.is_dispose '.
            'FROM ' .$GLOBALS['xak']->table('booking_goods'). ' AS bg, ' .$GLOBALS['xak']->table('goods'). ' AS g '.
            "WHERE bg.goods_id = g.goods_id $where " .
...
function get_booking_info($id)
{
    ...
    $sql ="SELECT bg.rec_id, bg.user_id, IFNULL(u.user_name, '$_LANG[guest_user]') AS user_name, ".
            ...
            "LEFT JOIN " . $xak->table('goods') . " AS g ON g.goods_id=bg.goods_id ".
            "LEFT JOIN " . $xak->table('users') . " AS u ON u.user_id=bg.user_id ".
            "WHERE bg.rec_id ='$id'";
