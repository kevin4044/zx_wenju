...
elseif ($_REQUEST['act'] == 'download')
{
    ...
    /* 文件标题 */
    echo xak_iconv(XAK_CHARSET, 'GB2312', $filename . $_LANG['sales_statistics']) . "\t\n";

    /* 订单数量, 销售出商品数量, 销售金额 */
    echo xak_iconv(XAK_CHARSET, 'GB2312', $_LANG['period']) ."\t";
    echo xak_iconv(XAK_CHARSET, 'GB2312', $_LANG['order_count_trend']) ."\t";
    echo xak_iconv(XAK_CHARSET, 'GB2312', $_LANG['order_amount_trend']) . "\t\n";

    foreach ($data_list AS $data)
    {
        echo xak_iconv(XAK_CHARSET, 'GB2312', $data['period']) . "\t";
        echo xak_iconv(XAK_CHARSET, 'GB2312', $data['order_count']) . "\t";
        echo xak_iconv(XAK_CHARSET, 'GB2312', $data['order_amount']) . "\t";
        echo "\n";
    }
}
