...
$exc = new exchange($xak->table("goods_activity"), $db, 'act_id', 'act_name');
...
elseif ($_REQUEST['act'] =='insert')
{
    ...
    /* 检查商品是否存在 */
    $sql = "SELECT goods_name FROM ".$xak->table('goods')." WHERE goods_id = '$_POST[goods_id]'";
    ...
    $db->AutoExecute($xak->table('goods_activity'),$record,'INSERT');
...
elseif ($_REQUEST['act'] =='update')
{
    ...
    {
        $_POST['goods_name'] = $db->getOne("SELECT goods_name FROM " . $xak->table('goods') . "WHERE goods_id= '$_POST[goods_id]'");
    }
    if (empty($_POST['start_price']))
    {
        $_POST['start_price'] = 0;
    ...
    $db->autoExecute($xak->table('goods_activity'), $record, 'UPDATE', "act_id = '" . $_POST['id'] . "' AND act_type = " . GAT_SNATCH );
...
function get_snatch_detail()
{
    ...
    /* 获得记录总数以及总页数 */
    $sql = "SELECT count(*) FROM ".$GLOBALS['xak']->table('snatch_log'). $where;
    $filter['record_count'] = $GLOBALS['db']->getOne($sql);
    ...
    $sql = "SELECT s.log_id, u.user_name, s.bid_price, s.bid_time ".
            " FROM ".$GLOBALS['xak']->table('snatch_log')." AS s ".
            " LEFT JOIN ".$GLOBALS['xak']->table('users')." AS u ON s.user_id = u.user_id  ". $where.
