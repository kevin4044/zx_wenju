...
if ($_REQUEST['act'] == 'list')
{
    ...
    /* 取得商品总点击数量 */
    $sql = 'SELECT SUM(click_count) FROM ' .$xak->table('goods') .' WHERE is_delete = 0';
    ...
    if ($is_multi)
    {
        ...
        foreach($start_date_arr AS $k => $val)
        {
             $sql = 'SELECT i.pay_id, p.pay_name, i.pay_time, COUNT(i.order_id) AS order_num ' .
                'FROM ' .$xak->table('payment'). ' AS p, ' .$xak->table('order_info'). ' AS i '.
        ...
        foreach($start_date_arr AS $k => $val)
        {
             $sql = 'SELECT sp.shipping_id, sp.shipping_name AS ship_name, i.shipping_time, COUNT(i.order_id) AS order_num ' .
               'FROM ' .$xak->table('shipping'). ' AS sp, ' .$xak->table('order_info'). ' AS i ' .
...
elseif ($act = 'download')
{
    ...
    echo xak_iconv(XAK_CHARSET, 'GB2312', $data) . "\t";
    exit;
...
/*------------------------------------------------------ */
 ...
 function get_orderinfo($start_date, $end_date)
 {
    ...
    /* 未确认订单数 */
    $sql = 'SELECT COUNT(*) AS unconfirmed_num FROM ' .$GLOBALS['xak']->table('order_info').
    ...
    /* 已确认订单数 */
    $sql = 'SELECT COUNT(*) AS confirmed_num FROM ' .$GLOBALS['xak']->table('order_info').
    ...
    /* 已成交订单数 */
    $sql = 'SELECT COUNT(*) AS succeed_num FROM ' .$GLOBALS['xak']->table('order_info').
           " WHERE 1 " . order_query_sql('finished') .
           " AND add_time >= '$start_date' AND add_time < '" . ($end_date + 86400) . "'";
    $order_info['succeed_num'] = $GLOBALS['db']->getOne($sql);

    /* 无效或已取消订单数 */
    $sql = "SELECT COUNT(*) AS invalid_num FROM " .$GLOBALS['xak']->table('order_info').
