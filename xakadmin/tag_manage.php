...
elseif($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if($is_insert)
    {
        $sql = 'INSERT INTO ' . $xak->table('tag') . '(tag_id, goods_id, tag_words)' .
...
elseif ($_REQUEST['act'] == 'batch_drop')
{
    ...
    if (isset($_POST['checkboxes']))
    {
        ...
        {
            $sql = "DELETE FROM " .$xak->table('tag'). " WHERE tag_id='$id'";
            $db->query($sql);
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $tag_name = $db->getOne("SELECT tag_words FROM " .$xak->table('tag'). " WHERE tag_id = '$id'");
...
function tag_is_only($name, $tag_id, $goods_id = '')
{
    ...
    {
        $db = $GLOBALS['db'];
        $sql = 'SELECT goods_id FROM ' . $GLOBALS['xak']->table('tag') . " WHERE tag_id = '$tag_id'";
        $row = $GLOBALS['db']->getRow($sql);
        $goods_id = $row['goods_id'];
    }

    $sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['xak']->table('tag') . " WHERE tag_words = '$name'" .
...
{
    $db = $GLOBALS['db'];
    $sql = 'UPDATE ' . $GLOBALS['xak']->table('tag') . " SET tag_words = '$name'";
    if(!empty($goods_id))
    {
...
function get_tag_list()
{
    ...
    $sql = "SELECT t.tag_id, u.user_name, t.goods_id, g.goods_name, t.tag_words ".
            "FROM " .$GLOBALS['xak']->table('tag'). " AS t ".
            "LEFT JOIN " .$GLOBALS['xak']->table('users'). " AS u ON u.user_id=t.user_id ".
            "LEFT JOIN " .$GLOBALS['xak']->table('goods'). " AS g ON g.goods_id=t.goods_id ".
...
{
    $sql = 'SELECT t.tag_id, t.tag_words, t.goods_id, g.goods_name FROM ' . $GLOBALS['xak']->table('tag') . ' AS t' .
           ' LEFT JOIN ' . $GLOBALS['xak']->table('goods') . ' AS g ON t.goods_id=g.goods_id' .
           " WHERE tag_id = '$tag_id'";
    $row = $GLOBALS['db']->getRow($sql);
