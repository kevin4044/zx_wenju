...
if (isset($_REQUEST['act']) && ($_REQUEST['act'] == 'query' ||  $_REQUEST['act'] == 'download'))
{
    ...
    if ($_REQUEST['act'] == 'download')
    {
        ...
        /* 文件标题 */
        echo xak_iconv(XAK_CHARSET, 'GB2312', $_REQUEST['start_date']. $_LANG['to'] .$_REQUEST['end_date']. $_LANG['sales_list']) . "\t\n";

        /* 商品名称,订单号,商品数量,销售价格,销售日期 */
        echo xak_iconv(XAK_CHARSET, 'GB2312', $_LANG['goods_name']) . "\t";
        echo xak_iconv(XAK_CHARSET, 'GB2312', $_LANG['order_sn']) . "\t";
        echo xak_iconv(XAK_CHARSET, 'GB2312', $_LANG['amount']) . "\t";
        echo xak_iconv(XAK_CHARSET, 'GB2312', $_LANG['sell_price']) . "\t";
        echo xak_iconv(XAK_CHARSET, 'GB2312', $_LANG['sell_date']) . "\t\n";

        foreach ($goods_sales_list['sale_list_data'] AS $key => $value)
        {
            echo xak_iconv(XAK_CHARSET, 'GB2312', $value['goods_name']) . "\t";
            echo xak_iconv(XAK_CHARSET, 'GB2312', '[ ' . $value['order_sn'] . ' ]') . "\t";
            echo xak_iconv(XAK_CHARSET, 'GB2312', $value['goods_num']) . "\t";
            echo xak_iconv(XAK_CHARSET, 'GB2312', $value['sales_price']) . "\t";
            echo xak_iconv(XAK_CHARSET, 'GB2312', $value['sales_time']) . "\t";
            echo "\n";
        }
        exit;
    }
...
function get_sale_list($is_pagination = true){
    ...
    $sql = "SELECT COUNT(og.goods_id) FROM " .
           $GLOBALS['xak']->table('order_info') . ' AS oi,'.
    ...
    $sql = 'SELECT og.goods_id, og.goods_sn, og.goods_name, og.goods_number AS goods_num, og.goods_price '.
           ...
           "FROM " . $GLOBALS['xak']->table('order_goods')." AS og, ".$GLOBALS['xak']->table('order_info')." AS oi ".
