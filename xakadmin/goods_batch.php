...
elseif ($_REQUEST['act'] == 'upload')
{
    ...
    if($_POST['data_cat'] == 'xakshop')
    {
        foreach ($data AS $line)
        {
            ...
            // 转换编码
            if (($_POST['charset'] != 'UTF8') && (strpos(strtolower(XAK_CHARSET), 'utf') === 0))
            {
                $line = xak_iconv($_POST['charset'], 'UTF8', $line);
            }
    ...
    elseif($_POST['data_cat'] == 'taobao')
    {
        ...
        foreach ($data AS $line)
        {
            ...
            $max_id     = $db->getOne("SELECT MAX(goods_id) + $id_is FROM ".$xak->table('goods'));
            $id_is++;
    ...
    elseif($_POST['data_cat'] == 'taobao46')
    {
        ...
        foreach ($data AS $line)
        {
            ...
            if (($_POST['charset'] == 'UTF8') && (strpos(strtolower(XAK_CHARSET), 'utf') == 0))
            {
                $line = xak_iconv($_POST['charset'], 'GBK', $line);
            }
            // 初始化
            $arr    = array();
            $line_list = explode("\t",$line);
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    if (isset($_POST['checked']))
    {
        ...
        foreach ($_POST['checked'] AS $key => $value)
        {
            ...
            foreach ($field_list AS $field)
            {
                ...
                if (!empty($field_value))
                {
                    ...
                    elseif ($field == 'brand_name')
                    {
                        ...
                        {
                            $sql = "INSERT INTO " . $xak->table('brand') . " (brand_name) VALUES ('" . addslashes($field_value) . "')";
                            $db->query($sql);
            ...
            $db->autoExecute($xak->table('goods'), $field_arr, 'INSERT');
            ...
            if (!empty($field_arr['original_img']) || !empty($field_arr['goods_img']) || !empty($field_arr['goods_thumb']))
            {
                ...
                //修改商品图
                $db->query("UPDATE " . $xak->table('goods') . " SET goods_img = '$goods_img', goods_thumb = '$goods_thumb', original_img = '$original_img' WHERE goods_id='" . $goods_gallery['goods_id'] . "'");

                //添加商品相册图
                if ($_CFG['auto_generate_gallery'])
                {
                    $db->autoExecute($xak->table('goods_gallery'), $goods_gallery, 'INSERT');
                }
            }
        }
    }
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    else
    {
        ...
        $sql = "SELECT DISTINCT goods_id FROM " . $xak->table('goods') .
                " WHERE goods_sn " . db_create_in($goods_sns);
        $goods_ids = join(',', $db->getCol($sql));
        $where = " WHERE goods_id " . db_create_in($goods_ids);
    }
    $sql = "SELECT DISTINCT goods_id, goods_sn, goods_name, market_price, shop_price, goods_number, integral, give_integral, brand_id, is_real FROM " . $xak->table('goods') . $where;
    $smarty->assign('goods_list', $db->getAll($sql));

    /* 取编辑商品的货品列表 */
    $product_exists = false;
    $sql = "SELECT * FROM " . $xak->table('products') . $where;
    ...
    $sql = "SELECT DISTINCT goods_id, user_rank, user_price FROM " . $xak->table('member_price') . $where;
    $res = $db->query($sql);
    while ($row = $db->fetchRow($res))
    {
        ...
        if (!empty($_POST['goods_id']))
        {
            foreach ($_POST['goods_id'] AS $goods_id)
            {
                ...
                if (!empty($_POST['product_number'][$goods_id]))
                {
                    ...
                    {
                        $db->autoExecute($xak->table('products'), array('product_number', $value), 'UPDATE', "goods_id = '$goods_id' AND product_id = " . $key);
                ...
                $db->autoExecute($xak->table('goods'), $goods, 'UPDATE', "goods_id = '$goods_id'");
                ...
                if (!empty($_POST['rank_id']))
                {
                    foreach ($_POST['rank_id'] AS $rank_id)
                    {
                        ...
                        $sql = "SELECT COUNT(*) FROM " . $xak->table('member_price') . " WHERE goods_id = '$goods_id' AND user_rank = '$rank_id'";
                        if ($db->getOne($sql) > 0)
                        {
                            if ($rank['user_price'] < 0)
                            {
                                $db->query("DELETE FROM " . $xak->table('member_price') . " WHERE goods_id = '$goods_id' AND user_rank = '$rank_id'");
                            }
                            else
                            {
                                $db->autoExecute($xak->table('member_price'), $rank, 'UPDATE', "goods_id = '$goods_id' AND user_rank = '$rank_id'");
                            }
                        ...
                        else
                        {
                            if ($rank['user_price'] >= 0)
                            {
                                $db->autoExecute($xak->table('member_price'), $rank, 'INSERT');
                            }
                        }
                    }
                }
...
elseif ($_REQUEST['act'] == 'download')
{
    ...
    if (isset($_LANG['upload_goods']))
    {
        ...
        if ($_GET['charset'] == 'zh_cn' || $_GET['charset'] == 'zh_tw')
        {
            ...
            echo xak_iconv(XAK_CHARSET, $to_charset, join(',', $_LANG['upload_goods']));
        }
        else
        {
