...
$exc = new exchange($xak->table("article_cat"), $db, 'cat_id', 'cat_name');
/* act操作项的初始化 */
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    {
        $sql = "SELECT cat_type FROM " . $xak->table('article_cat') . " WHERE cat_id = '$_POST[parent_id]'";
    ...
    $sql = "INSERT INTO ".$xak->table('article_cat')."(cat_name, cat_type, cat_desc,keywords, parent_id, sort_order, show_in_nav)
    ...
    if($_POST['show_in_nav'] == 1)
    {
        ...
        $vieworder += 2;
        //显示在自定义导航栏中
        $sql = "INSERT INTO " . $xak->table('nav') . " (name,ctype,cid,ifshow,vieworder,opennew,url,type) VALUES('" . $_POST['cat_name'] . "', 'a', '" . $db->insert_id() . "','1','$vieworder','0', '" . build_uri('article_cat', array('acid'=> $db->insert_id()), $_POST['cat_name']) . "','middle')";
        $db->query($sql);
    }
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT cat_id, cat_name, cat_type, cat_desc, show_in_nav, keywords, parent_id,sort_order FROM ".
           $xak->table('article_cat'). " WHERE cat_id='$_REQUEST[id]'";
    $cat = $db->GetRow($sql);
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    $row = $db->getRow("SELECT cat_type, parent_id FROM " . $xak->table('article_cat') . " WHERE cat_id='$_POST[id]'");
    ...
    $dat = $db->getOne("SELECT cat_name, show_in_nav FROM ". $xak->table('article_cat') . " WHERE cat_id = '" . $_POST['id'] . "'");
    if ($exc->edit("cat_name = '$_POST[cat_name]', cat_desc ='$_POST[cat_desc]', keywords='$_POST[keywords]',parent_id = '$_POST[parent_id]', cat_type='$cat_type', sort_order='$_POST[sort_order]', show_in_nav = '$_POST[show_in_nav]'",  $_POST['id']))
    {
        ...
        {
            //如果分类名称发生了改变
            $sql = "UPDATE " . $xak->table('nav') . " SET name = '" . $_POST['cat_name'] . "' WHERE ctype = 'a' AND cid = '" . $_POST['id'] . "' AND type = 'middle'";
            $db->query($sql);
        }
        if($_POST['show_in_nav'] != $dat['show_in_nav'])
        {
            ...
            {
                //显示
                $nid = $db->getOne("SELECT id FROM ". $xak->table('nav') . " WHERE ctype = 'a' AND cid = '" . $_POST['id'] . "' AND type = 'middle'");
                if(empty($nid))
                {
                    $vieworder = $db->getOne("SELECT max(vieworder) FROM ". $xak->table('nav') . " WHERE type = 'middle'");
                    $vieworder += 2;
                ...
                {
                    $sql = "UPDATE " . $xak->table('nav') . " SET ifshow = 1 WHERE ctype = 'a' AND cid = '" . $_POST['id'] . "' AND type = 'middle'";
                }
                $db->query($sql);
            }
            else
            {
                //去除
                $db->query("UPDATE " . $xak->table('nav') . " SET ifshow = 0 WHERE ctype = 'a' AND cid = '" . $_POST['id'] . "' AND type = 'middle'");
            }
        }
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $sql = "SELECT cat_type FROM " . $xak->table('article_cat') . " WHERE cat_id = '$id'";
    ...
    $sql = "SELECT COUNT(*) FROM " . $xak->table('article_cat') . " WHERE parent_id = '$id'";
    if ($db->getOne($sql) > 0)
    {
        /* 还有子分类，不能删除 */
    ...
    /* 非空的分类不允许删除 */
    $sql = "SELECT COUNT(*) FROM ".$xak->table('article')." WHERE cat_id = '$id'";
    if ($db->getOne($sql) > 0)
    {
    ...
    {
        $exc->drop($id);
        $db->query("DELETE FROM " . $xak->table('nav') . "WHERE  ctype = 'a' AND cid = '$id' AND type = 'middle'");
        clear_cache_files();
...
if ($_REQUEST['act'] == 'toggle_show_in_nav')
{
    ...
    if (cat_update($id, array('show_in_nav' => $val)) != false)
    {
        ...
        {
            //显示
            $nid = $db->getOne("SELECT id FROM ". $xak->table('nav') . " WHERE ctype='a' AND cid='$id' AND type = 'middle'");
            if(empty($nid))
            {
                //不存在
                $vieworder = $db->getOne("SELECT max(vieworder) FROM ". $xak->table('nav') . " WHERE type = 'middle'");
                $vieworder += 2;
                $catname = $db->getOne("SELECT cat_name FROM ". $xak->table('article_cat') . " WHERE cat_id = '$id'");
                $uri = build_uri('article_cat', array('acid'=> $id), $_POST['cat_name']);

                $sql = "INSERT INTO " . $xak->table('nav') . " (name,ctype,cid,ifshow,vieworder,opennew,url,type) ".
                    "VALUES('" . $catname . "', 'a', '$id','1','$vieworder','0', '" . $uri . "','middle')";
            }
            else
            {
                $sql = "UPDATE " . $xak->table('nav') . " SET ifshow = 1 WHERE ctype='a' AND cid='$id' AND type = 'middle'";
            }
            $db->query($sql);
        }
        else
        {
            //去除
            $db->query("UPDATE " . $xak->table('nav') . " SET ifshow = 0 WHERE ctype='a' AND cid='$id' AND type = 'middle'");
        }
        clear_cache_files();
...
function cat_update($cat_id, $args)
{
    ...
    return $GLOBALS['db']->autoExecute($GLOBALS['xak']->table('article_cat'), $args, 'update', "cat_id='$cat_id'");
}
?>
