...
$exc   = new exchange($xak->table("ad"), $db, 'ad_id', 'ad_name');
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    /* 查看广告名称是否有重复 */
    $sql = "SELECT COUNT(*) FROM " .$xak->table('ad'). " WHERE ad_name = '$ad_name'";
    if ($db->getOne($sql) > 0)
    {
    ...
    /* 插入数据 */
    $sql = "INSERT INTO ".$xak->table('ad'). " (position_id,media_type,ad_name,ad_link,ad_code,start_time,end_time,link_man,link_email,link_phone,click_count,enabled)
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    /* 获取广告数据 */
    $sql = "SELECT * FROM " .$xak->table('ad'). " WHERE ad_id='".intval($_REQUEST['id'])."'";
...
elseif ($_REQUEST['act'] == 'add_js')
{
    ...
    $js_code .= ' src='.'"'.$xak->url().'affiche.php?act=js&type='.$_REQUEST['type'].'&ad_id='.intval($_REQUEST['id']).'"'.'></script>';

    $site_url = $xak->url().'affiche.php?act=js&type='.$_REQUEST['type'].'&ad_id='.intval($_REQUEST['id']);
...
function get_adslist()
{
    ...
    /* 获得总记录数据 */
    $sql = 'SELECT COUNT(*) FROM ' .$GLOBALS['xak']->table('ad'). ' AS ad ' . $where;
    ...
    $sql = 'SELECT ad.*, COUNT(o.order_id) AS ad_stats, p.position_name '.
            'FROM ' .$GLOBALS['xak']->table('ad'). 'AS ad ' .
            'LEFT JOIN ' . $GLOBALS['xak']->table('ad_position'). ' AS p ON p.position_id = ad.position_id '.
            'LEFT JOIN ' . $GLOBALS['xak']->table('order_info'). " AS o ON o.from_ad = ad.ad_id $where " .
            'GROUP BY ad.ad_id '.
