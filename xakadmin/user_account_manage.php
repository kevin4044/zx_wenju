...
if ($_REQUEST['act'] == 'list')
{
    ...
    $sql = " SELECT IFNULL(SUM(user_money), 0) AS user_money, IFNULL(SUM(frozen_money), 0) AS frozen_money FROM " .
           $xak->table('account_log') . " WHERE `change_time` >= " . $start_date ." AND `change_time` < " .($end_date+86400);
    ...
    $sql = "SELECT IFNULL(SUM(surplus), 0) AS surplus, IFNULL(SUM(integral_money), 0) AS integral_money FROM ".
           $xak->table('order_info') ." WHERE 1 AND `add_time` >= " . $start_date ." AND `add_time` < " .($end_date+86400);
    $money_list = $db->getRow($sql);
...
{
    $sql = " SELECT IFNULL(SUM(amount), 0) AS total_amount FROM " . $GLOBALS['xak']->table('user_account') . " AS a, " . $GLOBALS['xak']->table('users') . " AS u ".
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        $filter['record_count'] = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['xak']->table('order_info') ." AS o, " . $GLOBALS['xak']->table('users') ." AS u " . $ex_where);

        /* 分页大小 */
        $filter = page_and_size($filter);

        $sql = "SELECT o.order_id, o.order_sn, u.user_name, o.surplus, o.integral_money, o.add_time FROM ".
            $GLOBALS['xak']->table('order_info') ." AS o," . $GLOBALS['xak']->table('users')." AS u " . $ex_where .
