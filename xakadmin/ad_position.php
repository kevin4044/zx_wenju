...
$exc = new exchange($xak->table("ad_position"), $db, 'position_id', 'position_name');
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    {
        /* 将广告位置的信息插入数据表 */
        $sql = 'INSERT INTO '.$xak->table('ad_position').' (position_name, ad_width, ad_height, position_desc, position_style) '.
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    /* 获取广告位数据 */
    $sql = 'SELECT * FROM ' .$xak->table('ad_position'). " WHERE position_id='$id'";
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    /* 查看广告位是否与其它有重复 */
    $sql = 'SELECT COUNT(*) FROM ' .$xak->table('ad_position').
           " WHERE position_name = '$position_name' AND position_id <> '$position_id'";
    if ($db->getOne($sql) == 0)
    {
        $sql = "UPDATE " .$xak->table('ad_position'). " SET ".
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    /* 查询广告位下是否有广告存在 */
    $sql = "SELECT COUNT(*) FROM " .$GLOBALS['xak']->table('ad'). " WHERE position_id = '$id'";
...
function ad_position_list()
{
    ...
    /* 记录总数以及页数 */
    $sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['xak']->table('ad_position');
    ...
    /* 查询数据 */
    $arr = array();
    $sql = 'SELECT * FROM ' .$GLOBALS['xak']->table('ad_position'). ' ORDER BY position_id DESC';
