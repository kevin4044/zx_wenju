...
if ($_REQUEST['act'] == 'list')
{
    ...
    $crons_enable = $db->getOne("SELECT enable FROM " . $GLOBALS['xak']->table('crons') . " WHERE cron_code='auto_manage'");
...
elseif ($_REQUEST['act'] == 'del')
{
    ...
    $sql = "DELETE FROM " . $xak->table('auto_manage') . " WHERE item_id = '$goods_id' AND type = 'goods'";
    $db->query($sql);
...
elseif ($_REQUEST['act'] == 'edit_starttime')
{
    ...
    $db->autoReplace($xak->table('auto_manage'), array('item_id' => $id,'type' => 'goods',
...
function get_auto_goods()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['xak']->table('goods') . " g" . $where;
        ...
        /* 查询 */
            $sql = "SELECT g.*,a.starttime,a.endtime FROM " . $GLOBALS['xak']->table('goods') . " g LEFT JOIN " . $GLOBALS['xak']->table('auto_manage') . " a ON g.goods_id = a.item_id AND a.type='goods'" . $where .
