...
$exc = new exchange($xak->table("pack"), $db, 'pack_id', 'pack_name');
...
if ($_REQUEST['act'] == 'insert')
{
    ...
    /*插入数据*/
    $sql = "INSERT INTO ".$xak->table('pack')."(pack_name, pack_fee, free_money, pack_desc, pack_img)
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT pack_id, pack_name, pack_fee, free_money, pack_desc, pack_img FROM " .$xak->table('pack'). " WHERE pack_id='$_REQUEST[id]'";
    $pack = $db->GetRow($sql);
...
if ($_REQUEST['act'] == 'drop_pack_img')
{
    ...
    /* 取得logo名称 */
    $sql = "SELECT pack_img FROM " .$xak->table('pack'). " WHERE pack_id = '$pack_id'";
    $img_name = $db->getOne($sql);

    if (!empty($img_name))
    {
        @unlink(ROOT_PATH . DATA_DIR . '/packimg/' .$img_name);
        $sql = "UPDATE " .$xak->table('pack'). " SET pack_img = '' WHERE pack_id = '$pack_id'";
        $db->query($sql);
    }
...
function packs_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT count(*) FROM " .$GLOBALS['xak']->table('pack');
