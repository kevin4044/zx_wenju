 ...
 * ----------------------------------------------------------------------------
 * $Author: wangleisvn $
 * $File: privilege.php 16529 2009-08-12 05:38:57Z wangleisvn $
*/
...
/* 初始化 $exc 对象 */
$exc = new exchange($xak->table("role"), $db, 'role_id', 'role_name');
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "INSERT INTO ".$xak->table('role')." (role_name, action_list, role_describe) ".
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $_REQUEST['id'] = !empty($_REQUEST['id']) ? intval($_REQUEST['id']) : 0;
        /* 获得该管理员的权限 */
    $priv_str = $db->getOne("SELECT action_list FROM " .$xak->table('role'). " WHERE role_id = '$_GET[id]'");
    ...
    /* 获取角色信息 */
    $sql = "SELECT role_id, role_name, role_describe FROM " .$xak->table('role').
...
elseif ($_REQUEST['act'] == 'update')
{
    /* 更新管理员的权限 */
    $act_list = @join(",", $_POST['action_code']);
    $sql = "UPDATE " .$xak->table('role'). " SET action_list = '$act_list', role_name = '".$_POST['user_name']."', role_describe = '".$_POST['role_describe']." ' ".
           "WHERE role_id = '$_POST[id]'";
    $db->query($sql);
    $user_sql = "UPDATE " .$xak->table('admin_user'). " SET action_list = '$act_list' ".
           "WHERE role_id = '$_POST[id]'";
    $db->query($user_sql);
    /* 提示信息 */
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $id = intval($_GET['id']);
    $num_sql = "SELECT count(*) FROM " .$xak->table('admin_user'). " WHERE role_id = '$_GET[id]'";
...
function get_role_list()
{
    ...
    $sql  = 'SELECT role_id, role_name, action_list, role_describe '.
            'FROM ' .$GLOBALS['xak']->table('role').' ORDER BY role_id DESC';
