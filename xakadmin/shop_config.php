...
elseif ($_REQUEST['act'] == 'post')
{
    ...
    $arr = array();
    $sql = 'SELECT id, value FROM ' . $xak->table('shop_config');
    $res= $db->query($sql);
    ...
    foreach ($_POST['value'] AS $key => $val)
    {
        if($arr[$key] != $val)
        {
            $sql = "UPDATE " . $xak->table('shop_config') . " SET value = '" . trim($val) . "' WHERE id = '" . $key . "'";
            $db->query($sql);
        }
    }
    ...
    /* 处理上传文件 */
    $file_var_list = array();
    $sql = "SELECT * FROM " . $xak->table('shop_config') . " WHERE parent_id > 0 AND type = 'file'";
    $res = $db->query($sql);
    while ($row = $db->fetchRow($res))
    {
    ...
    foreach ($_FILES AS $code => $file)
    {
        ...
        if ((isset($file['error']) && $file['error'] == 0) || (!isset($file['error']) && $file['tmp_name'] != 'none'))
        {
            ...
            else
            {
                ...
                {
                    $sql = "UPDATE " . $xak->table('shop_config') . " SET value = '$file_name' WHERE code = '$code'";
                    $db->query($sql);
                }
                else
                {
    ...
    if (!empty($_POST['invoice_rate']))
    {
        ...
        $sql = "UPDATE " . $xak->table('shop_config') . " SET value = '" . serialize($invoice) . "' WHERE code = 'invoice_type'";
        $db->query($sql);
    }
    ...
    $shop_country   = $db->getOne("SELECT region_name FROM ".$xak->table('region')." WHERE region_id='$_CFG[shop_country]'");
    $shop_province  = $db->getOne("SELECT region_name FROM ".$xak->table('region')." WHERE region_id='$_CFG[shop_province]'");
    $shop_city      = $db->getOne("SELECT region_name FROM ".$xak->table('region')." WHERE region_id='$_CFG[shop_city]'");

    $spt = '<script type="text/javascript" src="http://api.xakshop.com/record.php?';
    $spt .= "url=" .urlencode($xak->url());
...
function update_configure($key, $val='')
{
    if (!empty($key))
    {
        $sql = "UPDATE " . $GLOBALS['xak']->table('shop_config') . " SET value='$val' WHERE code='$key'";
...
function get_settings($groups=null, $excludes=null)
{
    ...
    /* 取出全部数据：分组和变量 */
    $sql = "SELECT * FROM ".$GLOBALS['xak']->table('shop_config')." WHERE type<>'hidden' $config_groups $excludes_groups ORDER BY parent_id, sort_order, id";
