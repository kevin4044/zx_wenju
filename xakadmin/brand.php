...
$exc = new exchange($xak->table("brand"), $db, 'brand_id', 'brand_name');
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "INSERT INTO ".$xak->table('brand')."(brand_name, site_url, brand_desc, brand_logo, is_show, sort_order) ".
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT brand_id, brand_name, site_url, brand_logo, brand_desc, brand_logo, is_show, sort_order ".
            "FROM " .$xak->table('brand'). " WHERE brand_id='$_REQUEST[id]'";
    $brand = $db->GetRow($sql);
...
elseif($_REQUEST['act'] == 'add_brand')
{
    ...
    {
        $sql = "INSERT INTO " . $xak->table('brand') . "(brand_name)" .
               "VALUES ( '$brand')";
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    /* 删除该品牌的图标 */
    $sql = "SELECT brand_logo FROM " .$xak->table('brand'). " WHERE brand_id = '$id'";
    ...
    /* 更新商品的品牌编号 */
    $sql = "UPDATE " .$xak->table('goods'). " SET brand_id=0 WHERE brand_id='$id'";
    $db->query($sql);
...
elseif ($_REQUEST['act'] == 'drop_logo')
{
    ...
    /* 取得logo名称 */
    $sql = "SELECT brand_logo FROM " .$xak->table('brand'). " WHERE brand_id = '$brand_id'";
    $logo_name = $db->getOne($sql);

    if (!empty($logo_name))
    {
        @unlink(ROOT_PATH . DATA_DIR . '/brandlogo/' .$logo_name);
        $sql = "UPDATE " .$xak->table('brand'). " SET brand_logo = '' WHERE brand_id = '$brand_id'";
        $db->query($sql);
    }
...
function get_brandlist()
{
    ...
    if ($result === false)
    {
        ...
        {
            $sql = "SELECT COUNT(*) FROM ".$GLOBALS['xak']->table('brand') .' WHERE brand_name = \''.$_POST['brand_name'].'\'';
        }
        else
        {
        ...
        if (isset($_POST['brand_name']))
        {
            ...
            $sql = "SELECT * FROM ".$GLOBALS['xak']->table('brand')." WHERE brand_name like '%{$keyword}%' ORDER BY sort_order ASC";
        }
        else
        {
            $sql = "SELECT * FROM ".$GLOBALS['xak']->table('brand')." ORDER BY sort_order ASC";
        }
