...
function get_search_log()
{
    ...
    $sql = "SELECT COUNT(*) FROM " . $GLOBALS['xak']->table('keywords') . " WHERE  searchengine='xakshop' $where";
    $filter['record_count'] = $GLOBALS['db']->getOne($sql);
    $logdb = array();
    $filter = page_and_size($filter);
    $sql = "SELECT * FROM " . $GLOBALS['xak']->table('keywords') .
           " WHERE  searchengine='xakshop' $where" .
