...
$exc = new exchange($xak->table("user_rank"), $db, 'rank_id', 'rank_name');
$exc_user = new exchange($xak->table("users"), $db, 'user_rank', 'user_rank');
...
{
    $ranks = array();
    $ranks = $db->getAll("SELECT * FROM " .$xak->table('user_rank'));
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "INSERT INTO " .$xak->table('user_rank') ."( ".
...
elseif ($_REQUEST['act'] == 'edit_min_points')
{
    ...
    $rank = $db->getRow("SELECT max_points, special_rank FROM " . $xak->table('user_rank') . " WHERE rank_id = '$rank_id'");
...
elseif ($_REQUEST['act'] == 'edit_max_points')
{
    ...
    $rank = $db->getRow("SELECT min_points, special_rank FROM " . $xak->table('user_rank') . " WHERE rank_id = '$rank_id'");
