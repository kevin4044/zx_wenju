...
admin_priv('cron');
$exc = new exchange($xak->table('crons'), $db, 'cron_code', 'cron_name');
if ($_REQUEST['act'] == 'list')
{
    $cron_list = array();
    $sql = "SELECT * FROM " . $xak->table('crons');
    $res = $db->query($sql);
...
elseif ($_REQUEST['act'] == 'install')
{
    ...
    elseif ($_POST['step'] == 2)
    {
        ...
        $sql = "SELECT COUNT(*) FROM " . $xak->table('crons') .
        ...
        $sql = "INSERT INTO " . $xak->table('crons') . " (cron_code, cron_name, cron_desc, cron_config, nextime, day, week, hour, minute, run_once, allow_ip, alow_files)" .
...
elseif ($_REQUEST['act'] == 'edit')
{
    if (empty($_POST['step']))
    {
        ...
        $sql = "SELECT * FROM " . $xak->table('crons') . " WHERE cron_code = '$_REQUEST[code]'";
        $cron = $db->getRow($sql);
        if (empty($cron))
        {
            $links[] = array('text' => $_LANG['back_list'], 'href' => 'cron.php?act=list');
...
{
    $sql = "DELETE FROM " . $xak->table('crons') .
...
elseif ($_REQUEST['act'] == 'do')
{
    ...
    {
        $cron = array();
        $sql = "SELECT cron_config FROM " . $xak->table('crons') . " WHERE cron_code = '$_REQUEST[code]'";
        $temp = $db->getRow($sql);
