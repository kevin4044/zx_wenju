...
/** 配送区域管理程序
 * ----------------------------------------------------------------------------
 * $File: shipping_area.php
*/

define('XAK_INC', true);

require(dirname(__FILE__) . '/includes/init.php');
$exc = new exchange($xak->table('shipping_area'), $db, 'shipping_area_id', 'shipping_area_name');
...
elseif ($_REQUEST['act'] == 'add' && !empty($_REQUEST['shipping']))
{
    ...
    $shipping = $db->getRow("SELECT shipping_name, shipping_code FROM " .$xak->table('shipping'). " WHERE shipping_id='$_REQUEST[shipping]'");
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "SELECT COUNT(*) FROM " .$xak->table("shipping_area").
    ...
    {
        $shipping_code = $db->getOne("SELECT shipping_code FROM " .$xak->table('shipping').
        ...
        if (isset($_POST['regions']) && is_array($_POST['regions']))
        {
            ...
            {
                $sql = "INSERT INTO ".$xak->table('area_region')." (shipping_area_id, region_id) VALUES ('$new_id', '$val')";
                $db->query($sql);
            }
        }
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT a.shipping_name, a.shipping_code, a.support_cod, b.* ".
            "FROM " .$xak->table('shipping'). " AS a, " .$xak->table('shipping_area'). " AS b ".
...
//       }
    ...
    $sql = "SELECT a.region_id, r.region_name ".
            "FROM ".$xak->table('area_region')." AS a, ".$xak->table('region'). " AS r ".
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    {
        $shipping_code = $db->getOne("SELECT shipping_code FROM " .$xak->table('shipping'). " WHERE shipping_id='$_POST[shipping]'");
        $plugin        = '../includes/modules/shipping/'. $shipping_code. ".php";
        ...
        $sql = "UPDATE " .$xak->table('shipping_area').
        ...
        $sql = "SELECT region_id, parent_id FROM " . $xak->table('region');
        $res = $db->query($sql);
        ...
        /* 清除原有的城市和地区 */
        $db->query("DELETE FROM ".$xak->table("area_region")." WHERE shipping_area_id='$_POST[id]'");

        /* 添加选定的城市和地区 */
        foreach ($selected_regions AS $key => $val)
        {
            $sql = "INSERT INTO ".$xak->table('area_region')." (shipping_area_id, region_id) VALUES ('$_POST[id]', '$val')";
            $db->query($sql);
        }
...
elseif ($_REQUEST['act'] == 'multi_remove')
{
    ...
    if (isset($_POST['areas']) && count($_POST['areas']) > 0)
    {
        ...
        {
            $db->query("DELETE FROM " .$xak->table('shipping_area'). " WHERE shipping_area_id='$v'");
            $i++;
        }
...
elseif ($_REQUEST['act'] == 'remove_area')
{
    ...
    $exc->drop($id);
    $db->query('DELETE FROM '.$xak->table('area_region').' WHERE shipping_area_id='.$id);
...
{
    $sql = "SELECT * FROM " . $GLOBALS['xak']->table('shipping_area');
    if ($shipping_id > 0)
    {
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        $sql = "SELECT r.region_name " .
                "FROM " . $GLOBALS['xak']->table('area_region'). " AS a, " .
                    $GLOBALS['xak']->table('region') . " AS r ".
