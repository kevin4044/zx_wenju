...
if ($_REQUEST['act'] == '')
{
    $smarty->assign('shop_url', urlencode($xak->url()));
...
elseif ($_REQUEST['act'] == 'top')
{
    // 获得管理员设置的菜单
    $lst = array();
    $nav = $db->GetOne('SELECT nav_list FROM ' . $xak->table('admin_user') . " WHERE user_id = '" . $_SESSION['admin_id'] . "'");
...
elseif ($_REQUEST['act'] == 'menu')
{
	...
	// 菜单图标
    include_once('includes/inc_menu_ico.php');
    ...
    foreach ($modules AS $key => $val)
    {
        ...
        if (is_array($val))
        {
            foreach ($val AS $k => $v)
            {
				...
				$menus[$key]['children'][$k]['ico'] =   $_ICO[$k];
            }
        }
        else
        {
    ...
    $smarty->assign('charset', XAK_CHARSET);
    $smarty->assign('admin_id', $_SESSION['admin_id']);
...
elseif ($_REQUEST['act'] == 'main')
{
    ...
    if(isset($_SESSION['shop_guide']) && $_SESSION['shop_guide'] === true)
    {
        ...
        xak_header("Location: ./index.php?act=first\n");
    ...
    'FROM ' . $xak->table('admin_message') . ' AS a, ' . $xak->table('admin_user') . ' AS b ' .
    ...
    /* 已完成的订单 */
    $order['finished']     = $db->GetOne('SELECT COUNT(*) FROM ' . $xak->table('order_info').
    ...
    /* “未确认”的订单 */
    $order['unconfirmed']  = $db->GetOne('SELECT COUNT(*) FROM ' .$xak->table('order_info').
    " WHERE 1 " . order_query_sql('unconfirmed'));
    $status['unconfirmed'] = OS_UNCONFIRMED;

    /* “部分发货”的订单 */
    $order['shipped_part']  = $db->GetOne('SELECT COUNT(*) FROM ' .$xak->table('order_info').
...
//    $today_start = mktime(0,0,0,date('m'),date('d'),date('Y'));
    ...
    if ($_CFG['use_storage'])
    {
        $sql = 'SELECT COUNT(*) FROM ' .$xak->table('goods'). ' WHERE is_delete = 0 AND goods_number <= warn_number AND is_real = 1';
        $goods['warn'] = $db->GetOne($sql);
        $sql = 'SELECT COUNT(*) FROM ' .$xak->table('goods'). ' WHERE is_delete = 0 AND goods_number <= warn_number AND is_real=0 AND extension_code=\'virtual_card\'';
    ...
    /* 访问统计信息 */
    $today  = local_getdate();
    $sql    = 'SELECT COUNT(*) FROM ' .$xak->table('stats').
    ...
    "FROM " . $xak->table('feedback') . " AS f ".
    "LEFT JOIN " . $xak->table('feedback') . " AS r ON r.parent_id=f.msg_id " .
    'WHERE f.parent_id=0 AND ISNULL(r.msg_id) ' ;
    $smarty->assign('feedback_number', $db->GetOne($sql));

    /* 未审核评论 */
    $smarty->assign('comment_number', $db->getOne('SELECT COUNT(*) FROM ' . $xak->table('comment') .
    ...
    /* IP库版本 */
    $sys_info['ip_version'] = xak_geoip('255.255.255.0');
    ...
    /* 缺货登记 */
    $smarty->assign('booking_goods', $db->getOne('SELECT COUNT(*) FROM ' . $xak->table('booking_goods') . ' WHERE is_dispose = 0'));

    /* 退款申请 */
    $smarty->assign('new_repay', $db->getOne('SELECT COUNT(*) FROM ' . $xak->table('user_account') . ' WHERE process_type = ' . SURPLUS_RETURN . ' AND is_paid = 0 '));



    assign_query_info();
    $smarty->assign('xak_version',  VERSION);
    $smarty->assign('xak_release',  RELEASE);
    $smarty->assign('xak_lang',     $_CFG['lang']);
    $smarty->assign('xak_charset',  strtoupper(XAK_CHARSET));
...
elseif ($_REQUEST['act'] == 'first')
{
    ...
    $sql = 'SELECT value from ' . $xak->table('shop_config') . " WHERE code='shop_name'";
    $shop_name = $db->getOne($sql);

    $smarty->assign('shop_name', $shop_name);

    $sql = 'SELECT value from ' . $xak->table('shop_config') . " WHERE code='shop_title'";
...
elseif ($_REQUEST['act'] == 'second')
{
    ...
    if(!empty($shop_name))
    {
        $sql = 'UPDATE ' . $xak->table('shop_config') . " SET value = '$shop_name' WHERE code = 'shop_name'";
        $db->query($sql);
    }

    if(!empty($shop_title))
    {
        $sql = 'UPDATE ' . $xak->table('shop_config') . " SET value = '$shop_title' WHERE code = 'shop_title'";
        $db->query($sql);
    }

    if(!empty($shop_address))
    {
        $sql = 'UPDATE ' . $xak->table('shop_config') . " SET value = '$shop_address' WHERE code = 'shop_address'";
        $db->query($sql);
    }

    if(!empty($shop_country))
    {
        $sql = 'UPDATE ' . $xak->table('shop_config') . "SET value = '$shop_country' WHERE code='shop_country'";
        $db->query($sql);
    }

    if(!empty($shop_province))
    {
        $sql = 'UPDATE ' . $xak->table('shop_config') . "SET value = '$shop_province' WHERE code='shop_province'";
        $db->query($sql);
    }

    if(!empty($shop_city))
    {
        $sql = 'UPDATE ' . $xak->table('shop_config') . "SET value = '$shop_city' WHERE code='shop_city'";
        $db->query($sql);
    }
    ...
    if(!empty($shipping))
    {
        ...
        $sql = "SELECT shipping_id FROM " .$xak->table('shipping'). " WHERE shipping_code = '$shipping'";
        $shipping_id = $db->GetOne($sql);

        if($shipping_id <= 0)
        {
            $insure = empty($modules[0]['insure']) ? 0 : $modules[0]['insure'];
            $sql = "INSERT INTO " . $xak->table('shipping') . " (" .
        ...
        if(!empty($area_name))
        {
            $sql = "SELECT shipping_area_id FROM " .$xak->table("shipping_area").
            ...
            if($area_id <= 0)
            {
                ...
                $sql = "INSERT INTO " .$xak->table('shipping_area').
            ...
            /* 添加选定的城市和地区 */
            $sql = "REPLACE INTO ".$xak->table('area_region')." (shipping_area_id, region_id) VALUES ('$area_id', '$region_id')";
            $db->query($sql);
        }
    }
    ...
    if(!empty($payment))
    {
        ...
        /* 安装，检查该支付方式是否曾经安装过 */
        $sql = "SELECT COUNT(*) FROM " . $xak->table('payment') . " WHERE pay_code = '$payment'";
        if ($db->GetOne($sql) > 0)
        {
            $sql = "UPDATE " . $xak->table('payment') .
...
//            $modules = read_modules('../includes/modules/payment');
            ...
            $sql = "INSERT INTO " . $xak->table('payment') . " (pay_code, pay_name, pay_desc, pay_config, is_cod, pay_fee, enabled, is_online)" .
...
elseif ($_REQUEST['act'] == 'third')
{
    ...
    if(!empty($good_brand))
    {
        $sql = 'INSERT INTO ' . $xak->table('brand') . " (brand_name, is_show)" .
        " values('" . $good_brand . "', '1')";
        $db->query($sql);
    ...
    if(!empty($good_category))
    {
        $sql = 'INSERT INTO ' . $xak->table('category') . " (cat_name, parent_id, is_show)" .
        ...
        $max_id     = $db->getOne("SELECT MAX(goods_id) + 1 FROM ".$xak->table('goods'));
        ...
        if(!empty($good_name))
        {
            ...
            $sql = 'INSERT INTO ' . $xak->table('goods') . "(goods_name, goods_sn, goods_number, cat_id, brand_id, goods_brief, shop_price, market_price, goods_img, goods_thumb, original_img,add_time, last_update,
...
elseif ($_REQUEST['act'] == 'save_todolist')
{
    ...
    $sql = "UPDATE" .$GLOBALS['xak']->table('admin_user'). " SET todolist='" . $content . "' WHERE user_id = " . $_SESSION['admin_id'];
    $GLOBALS['db']->query($sql);
}

elseif ($_REQUEST['act'] == 'get_todolist')
{
    $sql     = "SELECT todolist FROM " .$GLOBALS['xak']->table('admin_user'). " WHERE user_id = " . $_SESSION['admin_id'];
...
elseif ($_REQUEST['act'] == 'send_mail')
{
    ...
    $sql = "SELECT * FROM " . $xak->table('email_sendlist') . " ORDER BY pri DESC, last_send ASC LIMIT 1";
    $row = $db->getRow($sql);
    ...
    {
        $sql = "DELETE FROM " . $xak->table('email_sendlist') . " WHERE id = '$row[id]'";
        $db->query($sql);
        $count = $db->getOne("SELECT COUNT(*) FROM " . $xak->table('email_sendlist'));
        make_json_result('', $_LANG['mailsend_skip'], array('count' => $count, 'goon' => 1));
    }

    //查询相关模板
    $sql = "SELECT * FROM " . $xak->table('mail_templates') . " WHERE template_id = '$row[template_id]'";
    $rt = $db->getRow($sql);
    ...
    if ($rt['template_id'] && $rt['template_content'])
    {
        ...
        else
        {
            ...
            {
                $time = time();
                $sql = "UPDATE " . $xak->table('email_sendlist') . " SET error = error + 1, pri = 0, last_send = '$time' WHERE id = '$row[id]'";
            }
            else
            {
                //将出错超次的纪录删除
...
elseif ($_REQUEST['act'] == 'license')
{
    ...
    if (isset($is_ajax) && $is_ajax)
    {
        ...
        {
            case 'login_succ':
                if (isset($license['request']['info']['service']['xakshop_b2c']['cert_auth']['auth_str']))
                {
                    make_json_result(process_login_license($license['request']['info']['service']['xakshop_b2c']['cert_auth']));
                }
                else
                {
                    make_json_error(0);
            ...
            case 'reg_succ':
                ...
                {
                    case 'login_succ':
                        if (isset($_license['request']['info']['service']['xakshop_b2c']['cert_auth']['auth_str']) && $_license['request']['info']['service']['xakshop_b2c']['cert_auth']['auth_str'] != '')
                        {
                            make_json_result(process_login_license($license['request']['info']['service']['xakshop_b2c']['cert_auth']));
                        }
                        else
...
{
   $type = $_REQUEST['act'];
   include('templates/other.htm');
   exit();	
}
/*------------------------------------------------------ */
//-- 输出右侧
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'right')
{
   $type = $_REQUEST['act'];	
   include('templates/other.htm');
   exit();	
