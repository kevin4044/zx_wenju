...
/*初始化数据交换对象 */
$exc   = new exchange($xak->table("article"), $db, 'article_id', 'title');
//$image = new cls_image();
...
if ($_REQUEST['act'] == 'add')
{
    ...
    /* 清理关联商品 */
    $sql = "DELETE FROM " . $xak->table('goods_article') . " WHERE article_id = 0";
    $db->query($sql);
...
if ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "INSERT INTO ".$xak->table('article')."(title, cat_id, article_type, is_open, author, ".
    ...
    $sql = "UPDATE " . $xak->table('goods_article') . " SET article_id = '$article_id' WHERE article_id = 0";
    $db->query($sql);
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    /* 取文章数据 */
    $sql = "SELECT * FROM " .$xak->table('article'). " WHERE article_id='$_REQUEST[id]'";
...
if ($_REQUEST['act'] =='update')
{
    ...
    $sql = "SELECT file_url FROM " . $xak->table('article') . " WHERE article_id = '$_POST[id]'";
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    /* 删除原来的文件 */
    $sql = "SELECT file_url FROM " . $xak->table('article') . " WHERE article_id = '$id'";
    $old_url = $db->getOne($sql);
    ...
    if ($exc->drop($id))
    {
        $db->query("DELETE FROM " . $xak->table('comment') . " WHERE " . "comment_type = 1 AND id_value = $id");
...
elseif ($_REQUEST['act'] == 'add_link_goods')
{
    ...
    if ($article_id == 0)
    {
        $article_id = $db->getOne('SELECT MAX(article_id)+1 AS article_id FROM ' .$xak->table('article'));
    }

    foreach ($add_ids AS $key => $val)
    {
        $sql = 'INSERT INTO ' . $xak->table('goods_article') . ' (goods_id, article_id) '.
...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    if (isset($_POST['type']))
    {
        if ($_POST['type'] == 'button_remove')
        {
            ...
            /* 删除原来的文件 */
            $sql = "SELECT file_url FROM " . $xak->table('article') .
...
{
    $sql = "DELETE FROM " . $GLOBALS['xak']->table('goods_article') .
...
function get_article_goods($article_id)
{
    ...
    $sql  = 'SELECT g.goods_id, g.goods_name'.
            ' FROM ' . $GLOBALS['xak']->table('goods_article') . ' AS ga'.
            ' LEFT JOIN ' . $GLOBALS['xak']->table('goods') . ' AS g ON g.goods_id = ga.goods_id'.
...
function get_articleslist()
{
    ...
    if ($result === false)
    {
        ...
        /* 文章总数 */
        $sql = 'SELECT COUNT(*) FROM ' .$GLOBALS['xak']->table('article'). ' AS a '.
               'LEFT JOIN ' .$GLOBALS['xak']->table('article_cat'). ' AS ac ON ac.cat_id = a.cat_id '.
               'WHERE 1 ' .$where;
