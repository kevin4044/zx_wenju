...
/** 地区列表管理文件
 * ----------------------------------------------------------------------------
 * $File: area_manage.php
*/

define('XAK_INC', true);

require(dirname(__FILE__) . '/includes/init.php');
$exc = new exchange($xak->table('region'), $db, 'region_id', 'region_name');
...
elseif ($_REQUEST['act'] == 'add_area')
{
    ...
    $sql = "INSERT INTO " . $xak->table('region') . " (parent_id, region_name, region_type) ".
...
elseif ($_REQUEST['act'] == 'drop_area')
{
    ...
    $sql = "SELECT * FROM " . $xak->table('region') . " WHERE region_id = '$id'";
    $region = $db->getRow($sql);

//    /* 如果底下有下级区域,不能删除 */
//    $sql = "SELECT COUNT(*) FROM " . $xak->table('region') . " WHERE parent_id = '$id'";
//    if ($db->getOne($sql) > 0)
//    {
...
//    }
    ...
    $sql="DELETE FROM ". $xak->table("region")."WHERE region_id".db_create_in($delete_region);
     $db->query($sql);
    if ($exc->drop($id))
    {
...
function new_region_id($region_id)
{
    ...
    $sql="SELECT region_id FROM ". $GLOBALS['xak']->table("region")."WHERE parent_id ".db_create_in($region_id);
