...
if ($_REQUEST['act'] == 'list')
{
    ...
    /* 查询IP地址列表 */
    $ip_list = array();
    $res = $db->query("SELECT DISTINCT ip_address FROM " .$xak->table('admin_log'));
...
if ($_REQUEST['act'] == 'batch_drop')
{
    ...
    if ($drop_type_date)
    {
        if ($_POST['log_date'] == '0')
        {
            xak_header("Location: admin_logs.php?act=list\n");
            exit;
        }
        elseif ($_POST['log_date'] > '0')
        {
            ...
            $sql = "DELETE FROM " .$xak->table('admin_log').$where;
            $res = $db->query($sql);
            if ($res)
            {
    ...
    else
    {
        ...
        {
            $sql = "DELETE FROM " .$xak->table('admin_log'). " WHERE log_id = '$id'";
            $result = $db->query($sql);
...
function get_admin_logs()
{
    ...
    /* 获得总记录数据 */
    $sql = 'SELECT COUNT(*) FROM ' .$GLOBALS['xak']->table('admin_log'). ' AS al ' . $where;
    ...
    /* 获取管理员日志记录 */
    $list = array();
    $sql  = 'SELECT al.*, u.user_name FROM ' .$GLOBALS['xak']->table('admin_log'). ' AS al '.
            'LEFT JOIN ' .$GLOBALS['xak']->table('admin_user'). ' AS u ON u.user_id = al.user_id '.
