...
$exc = new exchange($xak->table('goods'), $db, 'goods_id', 'goods_name');
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    if ($is_add)
    {
        ...
        if (!empty($_COOKIE['XAKCP']['last_choose']))
        {
            $last_choose = explode('|', $_COOKIE['XAKCP']['last_choose']);
        }
        $goods = array(
            'goods_id'      => 0,
            'goods_desc'    => '',
        ...
        $sql = "DELETE FROM " . $xak->table('link_goods') .
        ...
        $sql = "DELETE FROM " . $xak->table('group_goods') .
                " WHERE parent_id = 0 AND admin_id = '$_SESSION[admin_id]'";
        $db->query($sql);

        /* 关联文章 */
        $goods_article_list = array();
        $sql = "DELETE FROM " . $xak->table('goods_article') .
                " WHERE goods_id = 0 AND admin_id = '$_SESSION[admin_id]'";
        $db->query($sql);

        /* 属性 */
        $sql = "DELETE FROM " . $xak->table('goods_attr') . " WHERE goods_id = 0";
        $db->query($sql);
    ...
    {
        /* 商品信息 */
        $sql = "SELECT * FROM " . $xak->table('goods') . " WHERE goods_id = '$_REQUEST[goods_id]'";
        $goods = $db->getRow($sql);
        ...
        if ($_REQUEST['act'] == 'copy')
        {
            ...
            $sql = "SELECT '0' AS goods_id, link_goods_id, is_double, '$_SESSION[admin_id]' AS admin_id" .
                    " FROM " . $xak->table('link_goods') .
                    " WHERE goods_id = '$_REQUEST[goods_id]' ";
            $res = $db->query($sql);
            while ($row = $db->fetchRow($res))
            {
                $db->autoExecute($xak->table('link_goods'), $row, 'INSERT');
            }
            ...
            $sql = "SELECT 0 AS parent_id, goods_id, goods_price, '$_SESSION[admin_id]' AS admin_id " .
                    "FROM " . $xak->table('group_goods') .
                    " WHERE parent_id = '$_REQUEST[goods_id]' ";
            $res = $db->query($sql);
            while ($row = $db->fetchRow($res))
            {
                $db->autoExecute($xak->table('group_goods'), $row, 'INSERT');
            }
            ...
            $sql = "SELECT 0 AS goods_id, article_id, '$_SESSION[admin_id]' AS admin_id " .
                    "FROM " . $xak->table('goods_article') .
                    " WHERE goods_id = '$_REQUEST[goods_id]' ";
            $res = $db->query($sql);
            while ($row = $db->fetchRow($res))
            {
                $db->autoExecute($xak->table('goods_article'), $row, 'INSERT');
            }
            ...
            $sql = "SELECT 0 AS goods_id, attr_id, attr_value, attr_price " .
                    "FROM " . $xak->table('goods_attr') .
                    " WHERE goods_id = '$_REQUEST[goods_id]' ";
            $res = $db->query($sql);
            while ($row = $db->fetchRow($res))
            {
                $db->autoExecute($xak->table('goods_attr'), addslashes_deep($row), 'INSERT');
            }
        }

        // 扩展分类
        $other_cat_list = array();
        $sql = "SELECT cat_id FROM " . $xak->table('goods_cat') . " WHERE goods_id = '$_REQUEST[goods_id]'";
        ...
        /* 图片列表 */
        $sql = "SELECT * FROM " . $xak->table('goods_gallery') . " WHERE goods_id = '$goods[goods_id]'";
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if ($_POST['goods_sn'])
    {
        $sql = "SELECT COUNT(*) FROM " . $xak->table('goods') .
    ...
    {
        $max_id     = $is_insert ? $db->getOne("SELECT MAX(goods_id) + 1 FROM ".$xak->table('goods')) : $_REQUEST['goods_id'];
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $xak->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
    ...
    else
    {
        ...
        $sql = "UPDATE " . $xak->table('goods') . " SET " .
    ...
    if ((isset($_POST['attr_id_list']) && isset($_POST['attr_value_list'])) || (empty($_POST['attr_id_list']) && empty($_POST['attr_value_list'])))
    {
        ...
        $sql = "SELECT attr_id, attr_index FROM " . $xak->table('attribute') . " WHERE cat_id = '$goods_type'";
        ...
        $sql = "SELECT g.*, a.attr_type
                FROM " . $xak->table('goods_attr') . " AS g
                    LEFT JOIN " . $xak->table('attribute') . " AS a
        ...
        $sql = "UPDATE " .$xak->table('goods'). " SET keywords = '$keywords' WHERE goods_id = '$goods_id' LIMIT 1";
        ...
        foreach ($goods_attr_list as $attr_id => $attr_value_list)
        {
            foreach ($attr_value_list as $attr_value => $info)
            {
                if ($info['sign'] == 'insert')
                {
                    $sql = "INSERT INTO " .$xak->table('goods_attr'). " (attr_id, goods_id, attr_value, attr_price)".
                            "VALUES ('$attr_id', '$goods_id', '$attr_value', '$info[attr_price]')";
                }
                elseif ($info['sign'] == 'update')
                {
                    $sql = "UPDATE " .$xak->table('goods_attr'). " SET attr_price = '$info[attr_price]' WHERE goods_attr_id = '$info[goods_attr_id]' LIMIT 1";
                }
                else
                {
                    $sql = "DELETE FROM " .$xak->table('goods_attr'). " WHERE goods_attr_id = '$info[goods_attr_id]' LIMIT 1";
                }
                $db->query($sql);
            }
        }
    ...
    if ($goods_img !== false)
    {
        $db->query("UPDATE " . $xak->table('goods') . " SET goods_img = '$goods_img' WHERE goods_id='$goods_id'");
    }

    if ($original_img !== false)
    {
        $db->query("UPDATE " . $xak->table('goods') . " SET original_img = '$original_img' WHERE goods_id='$goods_id'");
    }

    if ($goods_thumb !== false)
    {
        $db->query("UPDATE " . $xak->table('goods') . " SET goods_thumb = '$goods_thumb' WHERE goods_id='$goods_id'");
    }
    ...
    if (isset($img))
    {
        ...
        $sql = "INSERT INTO " . $xak->table('goods_gallery') . " (goods_id, img_url, img_desc, thumb_url, img_original) " .
    ...
    if (!$is_insert && isset($_POST['old_img_desc']))
    {
        ...
        {
            $sql = "UPDATE " . $xak->table('goods_gallery') . " SET img_desc = '$img_desc' WHERE img_id = '$img_id' LIMIT 1";
            $db->query($sql);
        }
    }
    ...
    {
        $db->query("UPDATE " . $xak->table('goods') . " SET original_img='' WHERE `goods_id`='{$goods_id}'");
        $db->query("UPDATE " . $xak->table('goods_gallery') . " SET img_original='' WHERE `goods_id`='{$goods_id}'");
        @unlink('../' . $original_img);
        @unlink('../' . $img);
    }

    /* 记录上一次选择的分类和品牌 */
    setcookie('XAKCP[last_choose]', $catgory_id . '|' . $brand_id, gmtime() + 86400);
    /* 清空缓存 */
    clear_cache_files();
...
elseif ($_REQUEST['act'] == 'edit_goods_sn')
{
    ...
    $sql="SELECT goods_id FROM ". $xak->table('products')."WHERE product_sn='$goods_sn'";
    if($db->getOne($sql))
    {
        make_json_error($_LANG['goods_sn_exists']);
    }
...
elseif ($_REQUEST['act'] == 'check_products_goods_sn')
{
    ...
    else
    {
        foreach ($products_sn as $val)
        {
            ...
            $sql="SELECT goods_id FROM ". $xak->table('products')."WHERE product_sn='$val'";
            if($db->getOne($sql))
            {
                make_json_error($val.$_LANG['goods_sn_exists']);
            }
...
elseif ($_REQUEST['act'] == 'drop_goods')
{
    ...
    /* 删除商品的货品记录 */
    $sql = "DELETE FROM " . $xak->table('products') .
            " WHERE goods_id = '$goods_id'";
    $db->query($sql);
    ...
    $sql = "DELETE FROM " . $xak->table('goods_gallery') . " WHERE goods_id = '$goods_id'";
    $db->query($sql);

    /* 删除相关表记录 */
    $sql = "DELETE FROM " . $xak->table('collect_goods') . " WHERE goods_id = '$goods_id'";
    $db->query($sql);
    $sql = "DELETE FROM " . $xak->table('goods_article') . " WHERE goods_id = '$goods_id'";
    $db->query($sql);
    $sql = "DELETE FROM " . $xak->table('goods_attr') . " WHERE goods_id = '$goods_id'";
    $db->query($sql);
    $sql = "DELETE FROM " . $xak->table('goods_cat') . " WHERE goods_id = '$goods_id'";
    $db->query($sql);
    $sql = "DELETE FROM " . $xak->table('member_price') . " WHERE goods_id = '$goods_id'";
    $db->query($sql);
    $sql = "DELETE FROM " . $xak->table('group_goods') . " WHERE parent_id = '$goods_id'";
    $db->query($sql);
    $sql = "DELETE FROM " . $xak->table('group_goods') . " WHERE goods_id = '$goods_id'";
    $db->query($sql);
    $sql = "DELETE FROM " . $xak->table('link_goods') . " WHERE goods_id = '$goods_id'";
    $db->query($sql);
    $sql = "DELETE FROM " . $xak->table('link_goods') . " WHERE link_goods_id = '$goods_id'";
    $db->query($sql);
    $sql = "DELETE FROM " . $xak->table('tag') . " WHERE goods_id = '$goods_id'";
    $db->query($sql);
    $sql = "DELETE FROM " . $xak->table('comment') . " WHERE comment_type = 0 AND id_value = '$goods_id'";
    $db->query($sql);
    $sql = "DELETE FROM " . $xak->table('collect_goods') . " WHERE goods_id = '$goods_id'";
    $db->query($sql);
    $sql = "DELETE FROM " . $xak->table('booking_goods') . " WHERE goods_id = '$goods_id'";
    $db->query($sql);
    $sql = "DELETE FROM " . $xak->table('goods_activity') . " WHERE goods_id = '$goods_id'";
    $db->query($sql);

    /* 如果不是实体商品，删除相应虚拟商品记录 */
    if ($goods['is_real'] != 1)
    {
        $sql = "DELETE FROM " . $xak->table('virtual_card') . " WHERE goods_id = '$goods_id'";
...
elseif ($_REQUEST['act'] == 'drop_image')
{
    ...
    /* 删除图片文件 */
    $sql = "SELECT img_url, thumb_url, img_original " .
            " FROM " . $GLOBALS['xak']->table('goods_gallery') .
            " WHERE img_id = '$img_id'";
    ...
    /* 删除数据 */
    $sql = "DELETE FROM " . $GLOBALS['xak']->table('goods_gallery') . " WHERE img_id = '$img_id' LIMIT 1";
...
elseif ($_REQUEST['act'] == 'add_link_goods')
{
    ...
    foreach ($linked_array AS $val)
    {
        ...
        {
            /* 双向关联 */
            $sql = "INSERT INTO " . $xak->table('link_goods') . " (goods_id, link_goods_id, is_double, admin_id) " .
...
elseif ($_REQUEST['act'] == 'drop_link_goods')
{
    ...
    {
        $sql = "UPDATE " .$xak->table('link_goods') . " SET is_double = 0 ".
                " WHERE link_goods_id = '$goods_id' AND goods_id " . $drop_goods_ids;
    }
    if ($goods_id == 0)
    {
...
elseif ($_REQUEST['act'] == 'add_group_goods')
{
    ...
    {
        $sql = "INSERT INTO " . $xak->table('group_goods') . " (parent_id, goods_id, goods_price, admin_id) " .
...
elseif ($_REQUEST['act'] == 'get_article_list')
{
    ...
    $sql        = 'SELECT article_id, title FROM ' .$xak->table('article'). $where.
...
elseif ($_REQUEST['act'] == 'add_goods_article')
{
    ...
    {
        $sql = "INSERT INTO " . $xak->table('goods_article') . " (goods_id, article_id, admin_id) " .
...
elseif ($_REQUEST['act'] == 'drop_goods_article')
{
    ...
    $sql = "DELETE FROM " .$xak->table('goods_article') . " WHERE " . db_create_in($articles, "article_id") . " AND goods_id = '$goods_id'";
    $db->query($sql);
...
elseif ($_REQUEST['act'] == 'product_list')
{
    ...
    /* 取出商品信息 */
    $sql = "SELECT goods_sn, goods_name, goods_type, shop_price FROM " . $xak->table('goods') . " WHERE goods_id = '$goods_id'";
    $goods = $db->getRow($sql);
    if (empty($goods))
    {
...
elseif ($_REQUEST['act'] == 'product_remove')
{
    ...
    /* 删除货品 */
    $sql = "DELETE FROM " . $xak->table('products') . " WHERE product_id = '$product_id'";
    $result = $db->query($sql);
    if ($result)
    {
        /* 修改商品库存 */
...
elseif ($_REQUEST['act'] == 'edit_product_sn')
{
    ...
    /* 修改 */
    $sql = "UPDATE " . $xak->table('products') . " SET product_sn = '$product_sn' WHERE product_id = '$product_id'";
    $result = $db->query($sql);
    if ($result)
    {
        clear_cache_files();
...
elseif ($_REQUEST['act'] == 'edit_product_number')
{
    ...
    /* 修改货品库存 */
    $sql = "UPDATE " . $xak->table('products') . " SET product_number = '$product_number' WHERE product_id = '$product_id'";
    $result = $db->query($sql);
    if ($result)
    {
        /* 修改商品库存 */
...
elseif ($_REQUEST['act'] == 'product_add_execute')
{
    ...
    /* 取出商品信息 */
    $sql = "SELECT goods_sn, goods_name, goods_type, shop_price FROM " . $xak->table('goods') . " WHERE goods_id = '" . $product['goods_id'] . "'";
    $goods = $db->getRow($sql);
    if (empty($goods))
    {
        sys_msg($_LANG['sys']['wrong'] . $_LANG['cannot_found_goods'], 1, array(), false);
    ...
    foreach($product['product_sn'] as $key => $value)
    {
        ...
        /* 插入货品表 */
        $sql = "INSERT INTO " . $GLOBALS['xak']->table('products') . " (goods_id, goods_attr, product_sn, product_number)  VALUES ('" . $product['goods_id'] . "', '$goods_attr', '$value', '" . $product['product_number'][$key] . "')";
        ...
        if (empty($value))
        {
            $sql = "UPDATE " . $GLOBALS['xak']->table('products') . "
...
elseif ($_REQUEST['act'] == 'batch_product')
{
    ...
    if ($_POST['type'] == 'drop')
    {
        ...
        $sum = 0;
        $goods_id = 0;
        $sql = "SELECT product_id, goods_id, product_number FROM  " . $GLOBALS['xak']->table('products') . " WHERE product_id $product_bound";
        ...
        if (!empty($product_array))
        {
            ...
            /* 删除货品 */
            $sql = "DELETE FROM " . $xak->table('products') . " WHERE product_id $product_bound";
            if ($db->query($sql))
            {
                //记录日志
...
{
    $sql = "DELETE FROM " . $GLOBALS['xak']->table('volume_price') .
    ...
    foreach ($price_list AS $key => $price)
    {
        ...
        if (!empty($price))
        {
            $sql = "INSERT INTO " . $GLOBALS['xak']->table('volume_price') .
