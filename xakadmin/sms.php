...
//        break;
    ...
    case 'send_sms' :
        ...
        if ($send_rank != 0)
        {
            ...
            if($rank_array['0'] == 1)
            {
                $sql = 'SELECT mobile_phone FROM ' . $xak->table('users') . "WHERE mobile_phone <>'' ";
                $row = $db->query($sql);
            ...
            {
                $rank_sql = "SELECT * FROM " . $xak->table('user_rank') . " WHERE rank_id = '" . $rank_array['1'] . "'";
                $rank_row = $db->getRow($rank_sql);
                //$sql = 'SELECT mobile_phone FROM ' . $xak->table('users') . "WHERE mobile_phone <>'' AND rank_points > " .$rank_row['min_points']." AND rank_points < ".$rank_row['max_points']." ";

                if($rank_row['special_rank']==1) 
                {
                    $sql = 'SELECT mobile_phone FROM ' . $xak->table('users') . " WHERE mobile_phone <>'' AND user_rank = '" . $rank_array['1'] . "'";
                }
                else
                {
                    $sql = 'SELECT mobile_phone FROM ' . $xak->table('users') . "WHERE mobile_phone <>'' AND rank_points > " .$rank_row['min_points']." AND rank_points < ".$rank_row['max_points']." ";
