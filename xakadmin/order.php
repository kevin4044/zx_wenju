...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    $sql = "SELECT agency_id FROM " . $xak->table('admin_user') . " WHERE user_id = '$_SESSION[admin_id]'";
    ...
    /* 取得上一个、下一个订单号 */
    if (!empty($_COOKIE['XAKCP']['lastfilter']))
    {
        $filter = unserialize(urldecode($_COOKIE['XAKCP']['lastfilter']));
    ...
    $sql = "SELECT MAX(order_id) FROM " . $xak->table('order_info') . " as o WHERE order_id < '$order[order_id]'";
    if ($agency_id > 0)
    {
    ...
    $sql = "SELECT MIN(order_id) FROM " . $xak->table('order_info') . " as o WHERE order_id > '$order[order_id]'";
    if ($agency_id > 0)
    {
        $sql .= " AND agency_id = '$agency_id'";
    }
    ...
    /* 取得所有办事处 */
    $sql = "SELECT agency_id, agency_name FROM " . $xak->table('agency');
    $smarty->assign('agency_list', $db->getAll($sql));

    /* 取得区域名 */
    $sql = "SELECT concat(IFNULL(c.region_name, ''), '  ', IFNULL(p.region_name, ''), " .
                "'  ', IFNULL(t.region_name, ''), '  ', IFNULL(d.region_name, '')) AS region " .
            "FROM " . $xak->table('order_info') . " AS o " .
                "LEFT JOIN " . $xak->table('region') . " AS c ON o.country = c.region_id " .
                "LEFT JOIN " . $xak->table('region') . " AS p ON o.province = p.region_id " .
                "LEFT JOIN " . $xak->table('region') . " AS t ON o.city = t.region_id " .
                "LEFT JOIN " . $xak->table('region') . " AS d ON o.district = d.region_id " .
    ...
    {
        /* 查询广告的名称 */
         $ad_name = $db->getOne("SELECT ad_name FROM " .$xak->table('ad'). " WHERE ad_id='$order[from_ad]'");
         $order['referer'] = $_LANG['from_ad_js'] . $ad_name . ' ('.$_LANG['from'] . $order['referer'].')';
    }

    /* 此订单的发货备注(此订单的最后一条操作记录) */
    $sql = "SELECT action_note FROM " . $xak->table('order_action').
    ...
    if ($order['user_id'] > 0)
    {
        ...
        $sql = "SELECT rank_name FROM " . $xak->table('user_rank') . $where;
        ...
        $sql = "SELECT COUNT(*) " .
                "FROM " . $xak->table('bonus_type') . " AS bt, " . $xak->table('user_bonus') . " AS ub " .
        ...
        // 地址信息
        $sql = "SELECT * FROM " . $xak->table('user_address') . " WHERE user_id = '$order[user_id]'";
    ...
    $sql = "SELECT o.*, IF(o.product_id > 0, p.product_number, g.goods_number) AS storage, o.goods_attr, g.suppliers_id, IFNULL(b.brand_name, '') AS brand_name, p.product_sn
            FROM " . $xak->table('order_goods') . " AS o
                LEFT JOIN " . $xak->table('products') . " AS p
                    ON p.product_id = o.product_id
                LEFT JOIN " . $xak->table('goods') . " AS g
                    ON o.goods_id = g.goods_id
                LEFT JOIN " . $xak->table('brand') . " AS b
    ...
    /* 取得订单操作记录 */
    $act_list = array();
    $sql = "SELECT * FROM " . $xak->table('order_action') . " WHERE order_id = '$order[order_id]' ORDER BY log_time DESC,action_id DESC";
    $res = $db->query($sql);
    while ($row = $db->fetchRow($res))
    {
    ...
    if (isset($_GET['print']))
    {
        ...
        $smarty->assign('shop_url',     $xak->url());
    ...
    elseif (isset($_GET['shipping_print']))
    {
        ...
        $region = $db->getAll("SELECT region_id, region_name FROM " . $xak->table("region") . " WHERE region_id IN ($region_id)");
        if (!empty($region))
        {
        ...
        $smarty->assign('shop_address', $_CFG['shop_address']);
        $smarty->assign('service_phone',$_CFG['service_phone']);
        $shipping = $db->getRow("SELECT * FROM " . $xak->table("shipping") . " WHERE shipping_id = " . $order['shipping_id']);
        ...
        {
            $shipping_code = $db->getOne("SELECT shipping_code FROM " . $xak->table('shipping') . " WHERE shipping_id=" . $order['shipping_id']);
            if ($shipping_code)
            {
...
elseif ($_REQUEST['act'] == 'delivery_info')
{
    ...
    if ($agency_id > 0)
    {
        ...
        /* 取当前办事处信息 */
        $sql = "SELECT agency_name FROM " . $xak->table('agency') . " WHERE agency_id = '$agency_id' LIMIT 0, 1";
    ...
    /* 取得订单操作记录 */
    $act_list = array();
    $sql = "SELECT * FROM " . $xak->table('order_action') . " WHERE order_id = '" . $delivery_order['order_id'] . "' AND action_place = 1 ORDER BY log_time DESC,action_id DESC";
    $res = $db->query($sql);
    while ($row = $db->fetchRow($res))
    {
        $row['order_status']    = $_LANG['os'][$row['order_status']];
...
elseif ($_REQUEST['act'] == 'delivery_ship')
{
    ...
    $delivery_stock_sql = "SELECT DG.goods_id, DG.is_real, DG.product_id, SUM(DG.send_number) AS sums, IF(DG.product_id > 0, P.product_number, G.goods_number) AS storage, G.goods_name, DG.send_number
        FROM " . $GLOBALS['xak']->table('delivery_goods') . " AS DG, " . $GLOBALS['xak']->table('goods') . " AS G, " . $GLOBALS['xak']->table('products') . " AS P
    ...
    else
    {
        ...
        FROM " . $GLOBALS['xak']->table('delivery_goods') . " AS DG, " . $GLOBALS['xak']->table('goods') . " AS G
        WHERE DG.goods_id = G.goods_id
        AND DG.delivery_id = '$delivery_id'
    ...
    if ($_CFG['use_storage'] == '1' && $_CFG['stock_dec_time'] == SDT_SHIP)
    {
        ...
        foreach ($delivery_stock_result as $value)
        {
            ...
            if ($value['is_real'] != 0)
            {
                ...
                {
                    $minus_stock_sql = "UPDATE " . $GLOBALS['xak']->table('products') . "
                                        SET product_number = product_number - " . $value['sums'] . "
                                        WHERE product_id = " . $value['product_id'];
                    $GLOBALS['db']->query($minus_stock_sql, 'SILENT');
                }

                $minus_stock_sql = "UPDATE " . $GLOBALS['xak']->table('goods') . "
    ...
    $query = $db->autoExecute($xak->table('delivery_order'), $_delivery, 'UPDATE', "delivery_id = $delivery_id", 'SILENT');
    if (!$query)
    {
        /* 操作失败 */
    ...
    if ($order_finish)
    {
        ...
        if ($cfg == '1')
        {
            ...
            $smarty->assign('confirm_url', $xak->url() . 'receive.php?id=' . $order['order_id'] . '&con=' . rawurlencode($order['consignee']));
            $smarty->assign('send_msg_url',$xak->url() . 'user.php?act=message_list&order_id=' . $order['order_id']);
...
elseif ($_REQUEST['act'] == 'delivery_cancel_ship')
{
    ...
    if ($_CFG['use_storage'] == '1' && $_CFG['stock_dec_time'] == SDT_SHIP)
    {
        ...
        $delivery_stock_sql = "SELECT DG.goods_id, DG.product_id, DG.is_real, SUM(DG.send_number) AS sums
            FROM " . $GLOBALS['xak']->table('delivery_goods') . " AS DG
...
elseif ($_REQUEST['act'] == 'step_post')
{
    ...
    if ('user' == $step)
    {
        ...
        do
        {
            ...
            if ($db->autoExecute($xak->table('order_info'), $order, 'INSERT', '', 'SILENT'))
            {
                break;
            }
            else
        ...
        /* 插入 pay_log */
        $sql = 'INSERT INTO ' . $xak->table('pay_log') . " (order_id, order_amount, order_type, is_paid)" .
                " VALUES ('$order_id', 0, '" . PAY_ORDER . "', 0)";
        $db->query($sql);

        /* 下一步 */
        xak_header("Location: order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=goods\n");
        exit;
    }
    /* 编辑商品信息 */
    elseif ('edit_goods' == $step)
    {
        if (isset($_POST['rec_id']))
        {
            foreach ($_POST['rec_id'] AS $key => $rec_id)
            {
              ...
              if($product_id)
                {
                   ...
                   $sql = "SELECT product_number ".
                          'FROM ' . $GLOBALS['xak']->table('products') .
              ...
              {
                /* 修改 */
                $sql = "UPDATE " . $xak->table('order_goods') .
    ...
    elseif ('add_goods' == $step)
    {
        ...
        $sql = "SELECT attr_value ".
            'FROM ' . $GLOBALS['xak']->table('goods_attr') .
       ...
       $res = $db->query($sql);
        ...
        $sql = "SELECT * FROM " .$GLOBALS['xak']->table('products'). " WHERE goods_id = '$goods_id' LIMIT 0, 1";
        ...
        {
        /* 插入订单商品 */
            $sql = "INSERT INTO " . $xak->table('order_goods') .
    ...
    elseif ('goods' == $step)
    {
        ...
        if (isset($_POST['next']))
        {
            xak_header("Location: order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=consignee\n");
            exit;
        }
        /* 完成 */
        elseif (isset($_POST['finish']))
        {
            ...
            {
                /* 跳转到订单详情 */
                xak_header("Location: order.php?act=info&order_id=" . $order_id . "\n");
                exit;
            }
        }
    }
    ...
    elseif ('consignee' == $step)
    {
        ...
        if (isset($_POST['next']))
        {
            ...
            {
                /* 存在实体商品，去配送方式 */
                xak_header("Location: order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=shipping\n");
                exit;
            }
            else
            {
                /* 不存在实体商品，去支付方式 */
                xak_header("Location: order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=payment\n");
                exit;
            }
        }
        elseif (isset($_POST['finish']))
        {
            ...
            if ($agency_changed)
            {
                xak_header("Location: order.php?act=list\n");
            }
            else
            {
                xak_header("Location: order.php?act=info&order_id=" . $order_id . "\n");
    ...
    elseif ('payment' == $step)
    {
        ...
        {
            /* 下一步 */
            xak_header("Location: order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=other\n");
            exit;
        }
        elseif (isset($_POST['finish']))
        {
    ...
    elseif ('other' == $step)
    {
        ...
        {
            /* 下一步 */
            xak_header("Location: order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=money\n");
            exit;
        }
        elseif (isset($_POST['finish']))
        {
    ...
    elseif ('money' == $step)
    {
        ...
        if ($old_order['user_id'] > 0)
        {
            ...
            if ($old_order['bonus_id'] != $order['bonus_id'])
            {
                ...
                {
                    $sql = "UPDATE " . $xak->table('user_bonus') .
...
elseif ($_REQUEST['act'] == 'process')
{
    ...
    if ('drop_order_goods' == $func)
    {
        ...
        if ($_CFG['use_storage'] == '1' && $_CFG['stock_dec_time'] == SDT_PLACE)
        {
             $goods = $db->getRow("SELECT goods_id, goods_number FROM " . $xak->table('order_goods') . " WHERE rec_id = " . $rec_id );
             $sql = "UPDATE " . $xak->table('goods') .
        ...
        /* 删除 */
        $sql = "DELETE FROM " . $xak->table('order_goods') .
                " WHERE rec_id = '$rec_id' LIMIT 1";
        $db->query($sql);
    ...
    elseif ('cancel_order' == $func)
    {
        ...
        if ($step_act == 'add')
        {
            ...
            if ($order_id > 0)
            {
                $sql = "DELETE FROM " . $xak->table('order_info') .
...
elseif ($_REQUEST['act'] == 'merge')
{
    ...
    $sql = "SELECT o.order_sn, u.user_name " .
            "FROM " . $xak->table('order_info') . " AS o " .
                "LEFT JOIN " . $xak->table('users') . " AS u ON o.user_id = u.user_id " .
            "WHERE o.user_id > 0 " .
...
elseif ($_REQUEST['act'] == 'operate')
{
    ...
    elseif (isset($_POST['assign']))
    {
        ...
        if ($new_agency_id != $order['agency_id'])
        {
            ...
            {
                $db->query("UPDATE " . $xak->table($value) . " SET agency_id = '$new_agency_id' " .
    ...
    elseif (isset($_POST['remove']))
    {
        ...
        if (!$batch)
        {
            ...
            /* 删除订单 */
            $db->query("DELETE FROM ".$xak->table('order_info'). " WHERE order_id = '$order_id'");
            $db->query("DELETE FROM ".$xak->table('order_goods'). " WHERE order_id = '$order_id'");
            $db->query("DELETE FROM ".$xak->table('order_action'). " WHERE order_id = '$order_id'");
    ...
    elseif (isset($_REQUEST['remove_invoice']))
    {
        ...
        foreach($delivery_id as $value_is)
        {
            ...
            // 更新：删除发货单
            $sql = "DELETE FROM ".$xak->table('delivery_order'). " WHERE delivery_id = '$value_is'";
            $db->query($sql);
        }
    ...
    elseif (isset($_REQUEST['remove_back']))
    {
        ...
        foreach ($back_id as $value_is)
            {
                $sql = "DELETE FROM ".$xak->table('back_order'). " WHERE back_id = '$value_is'";
                $db->query($sql);
            }
        }
        else
        {
            $sql = "DELETE FROM ".$xak->table('back_order'). " WHERE back_id = '$back_id'";
            $db->query($sql);
        }
        /* 返回 */
    ...
    elseif (isset($_POST['print']))
    {
        ...
        foreach ($order_sn_list as $order_sn)
        {
            ...
            $sql = "SELECT o.*, g.goods_number AS storage, o.goods_attr, IFNULL(b.brand_name, '') AS brand_name " .
                    "FROM " . $xak->table('order_goods') . " AS o ".
                    "LEFT JOIN " . $xak->table('goods') . " AS g ON o.goods_id = g.goods_id " .
                    "LEFT JOIN " . $xak->table('brand') . " AS b ON g.brand_id = b.brand_id " .
                    "WHERE o.order_id = '$order[order_id]' ";
            $res = $db->query($sql);
            while ($row = $db->fetchRow($res))
            {
    ...
    else
    {
        ...
        {
            /* 一个订单 */
            xak_header("Location: order.php?act=operate_post&order_id=" . $order_id .
        ...
        {
            /* 多个订单 */
            xak_header("Location: order.php?act=batch_operate_post&order_id=" . $order_id .
                    "&operation=" . $operation . "&action_note=" . urlencode($action_note) . "\n");
            exit;
        }
    }
...
elseif ($_REQUEST['act'] == 'batch_operate_post')
{
    ...
    if ('confirm' == $operation)
    {
        ...
        {
            $sql = "SELECT * FROM " . $xak->table('order_info') .
    ...
    elseif ('remove' == $operation)
    {
        foreach ($order_id_list as $id_order)
        {
            ...
            /* 删除订单 */
            $db->query("DELETE FROM ".$xak->table('order_info'). " WHERE order_id = '$order[order_id]'");
            $db->query("DELETE FROM ".$xak->table('order_goods'). " WHERE order_id = '$order[order_id]'");
            $db->query("DELETE FROM ".$xak->table('order_action'). " WHERE order_id = '$order[order_id]'");
            $action_array = array('delivery', 'back');
...
elseif ($_REQUEST['act'] == 'operate_post')
{
    ...
    elseif ('split' == $operation)
    {
        ...
        foreach ($goods_list as $key => $value)
        {
            ...
            {
                $sql = "SELECT COUNT(*) FROM " . $GLOBALS['xak']->table('virtual_card') . " WHERE goods_id = '" . $value['goods_id'] . "' AND is_saled = 0 ";
            ...
            else
            {
                ...
                if (empty($value['product_id']))
                {
                    $sql = "SELECT goods_number FROM " . $GLOBALS['xak']->table('goods') . " WHERE goods_id = '" . $value['goods_id'] . "' LIMIT 0,1";
                }
                /* （货品） */
                else
                {
        ...
        $sql ="select add_time from ". $GLOBALS['xak']->table('order_info') ." WHERE order_sn = '" . $delivery['order_sn'] . "'";
        ...
        /* 发货单入库 */
        $query = $db->autoExecute($xak->table('delivery_order'), $_delivery, 'INSERT', '', 'SILENT');
            ...
            if (!empty($goods_list))
            {
                foreach ($goods_list as $value)
                {
                    ...
                    if (empty($value['extension_code']) || $value['extension_code'] == 'virtual_card')
                    {
                        ...
                        $query = $db->autoExecute($xak->table('delivery_goods'), $delivery_goods, 'INSERT', '', 'SILENT');
                    }
                    // 商品（超值礼包）
                    elseif ($value['extension_code'] == 'package_buy')
                    {
                        foreach ($value['package_goods_list'] as $pg_key => $pg_value)
                        {
                            ...
                            $query = $db->autoExecute($xak->table('delivery_goods'), $delivery_pg_goods, 'INSERT', '', 'SILENT');
                        }
                    }
                }
            }
    ...
    elseif ('unship' == $operation)
    {
        ...
        /* 将订单的商品发货数量更新为 0 */
        $sql = "UPDATE " . $GLOBALS['xak']->table('order_goods') . "
                SET send_number = 0
                WHERE order_id = '$order_id'";
    ...
    elseif ('return' == $operation)
    {
        ...
        if ($order['user_id'] > 0)
        {
            ...
            $sql = "SELECT  goods_number, send_number FROM". $GLOBALS['xak']->table('order_goods') . "
        ...
        if ($delivery_list)
        {
            foreach ($delivery_list as $list)
            {
                ...
                $sql_back_goods = "INSERT INTO " . $xak->table('back_goods') . " (back_id, goods_id, product_id, product_sn, goods_name,goods_sn, is_real, send_number, goods_attr)
        ...
        /* 修改订单的发货单状态为退货 */
        $sql_delivery = "UPDATE " . $xak->table('delivery_order') . "
                         SET status = 1
                         WHERE status IN (0, 2)
                         AND order_id = " . $order['order_id'];
...
elseif ($_REQUEST['act'] == 'json')
{
    ...
    if ($func == 'get_goods_info')
    {
        ...
        $sql = "SELECT goods_id, c.cat_name, goods_sn, goods_name, b.brand_name, " .
                ...
                "FROM " . $xak->table('goods') . " AS g " .
                "LEFT JOIN " . $xak->table('brand') . " AS b ON g.brand_id = b.brand_id " .
                "LEFT JOIN " . $xak->table('category') . " AS c ON g.cat_id = c.cat_id " .
        ...
        $sql = "SELECT p.user_price, r.rank_name " .
                "FROM " . $xak->table('member_price') . " AS p, " .
                    $xak->table('user_rank') . " AS r " .
        ...
        $sql = "SELECT a.attr_id, a.attr_name, g.goods_attr_id, g.attr_value, g.attr_price, a.attr_input_type, a.attr_type " .
                "FROM " . $xak->table('goods_attr') . " AS g, " .
                    $xak->table('attribute') . " AS a " .
...
elseif ($_REQUEST['act'] == 'remove_order')
{
    ...
    $GLOBALS['db']->query("DELETE FROM ".$GLOBALS['xak']->table('order_info'). " WHERE order_id = '$order_id'");
    $GLOBALS['db']->query("DELETE FROM ".$GLOBALS['xak']->table('order_goods'). " WHERE order_id = '$order_id'");
    $GLOBALS['db']->query("DELETE FROM ".$GLOBALS['xak']->table('order_action'). " WHERE order_id = '$order_id'");
    $action_array = array('delivery', 'back');
    del_delivery($order_id, $action_array);
...
elseif ($_REQUEST['act'] == 'search_users')
{
    ...
    if ($id_name != '')
    {
        $sql = "SELECT user_id, user_name FROM " . $GLOBALS['xak']->table('users') .
...
elseif ($_REQUEST['act'] == 'search_goods')
{
    ...
    if ($keyword != '')
    {
        $sql = "SELECT goods_id, goods_name, goods_sn FROM " . $GLOBALS['xak']->table('goods') .
                " WHERE is_delete = 0" .
                " AND is_on_sale = 1" .
                " AND is_alone_sale = 1" .
...
elseif ($_REQUEST['act'] == 'edit_invoice_no')
{
    ...
    $sql = 'UPDATE ' . $GLOBALS['xak']->table('order_info') . " SET invoice_no='$no' WHERE order_id = '$order_id'";
...
elseif ($_REQUEST['act'] == 'edit_pay_note')
{
    ...
    $sql = 'UPDATE ' . $GLOBALS['xak']->table('order_info') . " SET pay_note='$no' WHERE order_id = '$order_id'";
    if ($GLOBALS['db']->query($sql))
    {
        if (empty($no))
        {
...
elseif ($_REQUEST['act'] == 'get_goods_info')
{
    ...
    while ($row = $db->fetchRow($res))
    {
        ...
        $_goods_thumb = (strpos($_goods_thumb, 'http://') === 0) ? $_goods_thumb : $xak->url() . $_goods_thumb;
...
function return_user_surplus_integral_bonus($order)
{
    ...
    if ($order['user_id'] > 0 && $order['surplus'] > 0)
    {
        ...
        $GLOBALS['db']->query("UPDATE ". $GLOBALS['xak']->table('order_info') . " SET `order_amount` = '0' WHERE `order_id` =". $order['order_id']);
    }
...
function update_order_amount($order_id)
{
    ...
    //更新订单总金额
    $sql = "UPDATE " . $GLOBALS['xak']->table('order_info') .
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT agency_id FROM " . $GLOBALS['xak']->table('admin_user') . " WHERE user_id = '$_SESSION[admin_id]'";
        ...
        elseif (isset($_COOKIE['XAKCP']['page_size']) && intval($_COOKIE['XAKCP']['page_size']) > 0)
        {
            $filter['page_size'] = intval($_COOKIE['XAKCP']['page_size']);
        }
        else
        {
            $filter['page_size'] = 15;
        ...
        if ($filter['user_name'])
        {
            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['xak']->table('order_info') . " AS o ,".
                   $GLOBALS['xak']->table('users') . " AS u " . $where;
        }
        else
        {
            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['xak']->table('order_info') . " AS o ". $where;
        }
        ...
        $sql = "SELECT o.order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid," .
                ...
                " FROM " . $GLOBALS['xak']->table('order_info') . " AS o " .
                " LEFT JOIN " .$GLOBALS['xak']->table('users'). " AS u ON u.user_id=o.user_id ". $where .
...
function update_pay_log($order_id)
{
    ...
    if ($order_id > 0)
    {
        $sql = "SELECT order_amount FROM " . $GLOBALS['xak']->table('order_info') .
                " WHERE order_id = '$order_id'";
        $order_amount = $GLOBALS['db']->getOne($sql);
        if (!is_null($order_amount))
        {
            $sql = "SELECT log_id FROM " . $GLOBALS['xak']->table('pay_log') .
                    " WHERE order_id = '$order_id'" .
            ...
            {
                /* 未付款，更新支付金额 */
                $sql = "UPDATE " . $GLOBALS['xak']->table('pay_log') .
            ...
            {
                /* 已付款，生成新的pay_log */
                $sql = "INSERT INTO " . $GLOBALS['xak']->table('pay_log') .
...
{
    $sql = 'SELECT *
            FROM ' . $GLOBALS['xak']->table('suppliers') . '
            WHERE is_check = 1
...
function get_order_goods($order)
{
    ...
    $sql = "SELECT o.*, g.suppliers_id AS suppliers_id,IF(o.product_id > 0, p.product_number, g.goods_number) AS storage, o.goods_attr, IFNULL(b.brand_name, '') AS brand_name, p.product_sn " .
            "FROM " . $GLOBALS['xak']->table('order_goods') . " AS o ".
            "LEFT JOIN " . $GLOBALS['xak']->table('products') . " AS p ON o.product_id = p.product_id " .
            "LEFT JOIN " . $GLOBALS['xak']->table('goods') . " AS g ON o.goods_id = g.goods_id " .
            "LEFT JOIN " . $GLOBALS['xak']->table('brand') . " AS b ON g.brand_id = b.brand_id " .
            "WHERE o.order_id = '$order[order_id]' ";
    $res = $GLOBALS['db']->query($sql);
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
...
function get_package_goods_list($package_id)
{
    $sql = "SELECT pg.goods_id, g.goods_name, (CASE WHEN pg.product_id > 0 THEN p.product_number ELSE g.goods_number END) AS goods_number, p.goods_attr, p.product_id, pg.goods_number AS
            ...
            FROM " . $GLOBALS['xak']->table('package_goods') . " AS pg
                LEFT JOIN " .$GLOBALS['xak']->table('goods') . " AS g ON pg.goods_id = g.goods_id
                LEFT JOIN " . $GLOBALS['xak']->table('products') . " AS p ON pg.product_id = p.product_id
    ...
    if ($good_product_str != '')
    {
        $sql = "SELECT ga.goods_attr_id, ga.attr_value, ga.attr_price, a.attr_name
                FROM " .$GLOBALS['xak']->table('goods_attr'). " AS ga, " .$GLOBALS['xak']->table('attribute'). " AS a
...
function order_delivery_num($order_id, $goods_id, $product_id = 0)
{
    $sql = 'SELECT SUM(G.send_number) AS sums
            FROM ' . $GLOBALS['xak']->table('delivery_goods') . ' AS G, ' . $GLOBALS['xak']->table('delivery_order') . ' AS O
...
function order_deliveryed($order_id)
{
    ...
    $sql = 'SELECT COUNT(delivery_id)
            FROM ' . $GLOBALS['xak']->table('delivery_order') . '
            WHERE order_id = \''. $order_id . '\'
            AND status = 0';
    $sum = $GLOBALS['db']->getOne($sql);
...
function get_order_finish($order_id)
{
    ...
    $sql = 'SELECT COUNT(rec_id)
            FROM ' . $GLOBALS['xak']->table('order_goods') . '
            WHERE order_id = \'' . $order_id . '\'
...
function del_order_delivery($order_id)
{
    ...
    $sql = 'DELETE O, G
            FROM ' . $GLOBALS['xak']->table('delivery_order') . ' AS O, ' . $GLOBALS['xak']->table('delivery_goods') . ' AS G
            WHERE O.order_id = \'' . $order_id . '\'
            AND O.status = 0
...
function del_delivery($order_id, $action_array)
{
    ...
    {
        $sql = 'DELETE O, G
                FROM ' . $GLOBALS['xak']->table('back_order') . ' AS O, ' . $GLOBALS['xak']->table('back_goods') . ' AS G
                WHERE O.order_id = \'' . $order_id . '\'
...
function delivery_list()
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['xak']->table('delivery_order') . $where;
        $filter['record_count']   = $GLOBALS['db']->getOne($sql);
        $filter['page_count']     = $filter['record_count'] > 0 ? ceil($filter['record_count'] / $filter['page_size']) : 1;
...
function back_list()
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['xak']->table('back_order') . $where;
        $filter['record_count']   = $GLOBALS['db']->getOne($sql);
        $filter['page_count']     = $filter['record_count'] > 0 ? ceil($filter['record_count'] / $filter['page_size']) : 1;
        ...
        $sql = "SELECT back_id, delivery_sn, order_sn, order_id, add_time, action_user, consignee, country,
                ...
                FROM " . $GLOBALS['xak']->table("back_order") . "
                $where
                ORDER BY " . $filter['sort_by'] . " " . $filter['sort_order']. "
                LIMIT " . ($filter['page'] - 1) * $filter['page_size'] . ", " . $filter['page_size'] . " ";
...
function delivery_order_info($delivery_id, $delivery_sn = '')
{
    ...
    $sql = "SELECT * FROM " . $GLOBALS['xak']->table('delivery_order');
    if ($delivery_id > 0)
    {
...
function back_order_info($back_id)
{
    ...
    $sql = "SELECT * FROM " . $GLOBALS['xak']->table('back_order') . "
...
function package_sended($package_id, $goods_id, $order_id, $extension_code, $product_id = 0)
{
    ...
    $sql = "SELECT SUM(DG.send_number)
            FROM " . $GLOBALS['xak']->table('delivery_goods') . " AS DG, " . $GLOBALS['xak']->table('delivery_order') . " AS o
        ...
        if (is_array($value))
        {
            ...
            foreach ($goods_list as $goods)
            {
                ...
                foreach ($goods['package_goods_list'] as $package_goods)
                {
                    ...
                    $sql = "UPDATE " . $GLOBALS['xak']->table('goods') ."
...
function package_virtual_card_shipping($goods, $order_sn)
{
    ...
    foreach ($goods as $virtual_goods_key => $virtual_goods_value)
    {
        ...
        $sql = "SELECT card_id, card_sn, card_password, end_date, crc32
                FROM ".$GLOBALS['xak']->table('virtual_card')."
        ...
        /* 标记已经取出的卡片 */
        $sql = "UPDATE ".$GLOBALS['xak']->table('virtual_card')." SET ".
           "is_saled = 1 ,".
           "order_sn = '$order_sn' ".
        ...
        /* 获取订单信息 */
        $sql = "SELECT order_id, order_sn, consignee, email FROM ".$GLOBALS['xak']->table('order_info'). " WHERE order_sn = '$order_sn'";
...
function delivery_return_goods($delivery_id, $delivery_order)
{
    /* 查询：取得发货单商品 */
    $goods_sql = "SELECT *
                 FROM " . $GLOBALS['xak']->table('delivery_goods') . "
                 WHERE delivery_id = " . $delivery_order['delivery_id'];
    $goods_list = $GLOBALS['db']->getAll($goods_sql);
    /* 更新： */
...
function del_order_invoice_no($order_id, $delivery_invoice_no)
{
    ...
    $sql = "SELECT invoice_no
            FROM " . $GLOBALS['xak']->table('order_info') . "
            WHERE order_id = '$order_id'";
