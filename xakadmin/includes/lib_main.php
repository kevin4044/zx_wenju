...
function admin_log($sn = '', $action, $content)
{
    ...
    $sql = 'INSERT INTO ' . $GLOBALS['xak']->table('admin_log') . ' (log_time, user_id, log_info, ip_address) ' .
...
function insert_config($parent, $code, $value)
{
    ...
    $sql = 'SELECT id FROM ' . $xak->table('shop_config') . " WHERE code = '$parent' AND type = 1";
    $parent_id = $db->getOne($sql);

    $sql = 'INSERT INTO ' . $xak->table('shop_config') . ' (parent_id, code, value) ' .
...
{
    $bonus = array();
    $sql = 'SELECT type_id, type_name, type_money FROM ' . $GLOBALS['xak']->table('bonus_type') .
           ' WHERE send_type = 3';
...
{
    $rank_list = array();
    $sql = 'SELECT rank_id, rank_name, min_points FROM ' . $GLOBALS['xak']->table('user_rank');
    if ($is_special)
    {
...
{
    $user_list = array();
    $sql = 'SELECT user_id, user_name FROM ' . $GLOBALS['xak']->table('users') . $where.
...
function get_position_list()
{
    ...
    $sql = 'SELECT position_id, position_name, ad_width, ad_height '.
           'FROM ' . $GLOBALS['xak']->table('ad_position');
    $res = $GLOBALS['db']->query($sql);
...
function get_goods_list($filter)
{
    ...
    $sql = 'SELECT goods_id, goods_name, shop_price '.
           'FROM ' . $GLOBALS['xak']->table('goods') . ' AS g ' . $where .
           'LIMIT 50';
...
function get_article_list($filter)
{
    ...
    $sql = 'SELECT a.article_id, a.title '.
           'FROM ' .$GLOBALS['xak']->table('article'). ' AS a, ' .$GLOBALS['xak']->table('article_cat'). ' AS c ' . $where;
    $res = $GLOBALS['db']->query($sql);
...
function area_list($region_id)
{
    ...
    $sql = 'SELECT * FROM ' . $GLOBALS['xak']->table('region').
...
{
    $sql = 'SELECT cat_id, cat_name FROM ' . $GLOBALS['xak']->table('goods_type') . ' WHERE enabled = 1';
    $res = $GLOBALS['db']->query($sql);
...
function get_pay_ids()
{
    ...
    $sql = 'SELECT pay_id, is_cod FROM ' .$GLOBALS['xak']->table('payment'). ' WHERE enabled = 1';
    $res = $GLOBALS['db']->query($sql);
...
{
    $sql = 'TRUNCATE TABLE ' .$GLOBALS['xak']->table($table_name);
...
{
    $sql = "SELECT attr_group FROM " . $GLOBALS['xak']->table('goods_type') . " WHERE cat_id='$cat_id'";
...
function set_filter($filter, $sql, $param_str = '')
{
    ...
    setcookie('XAKCP[lastfilterfile]', sprintf('%X', crc32($filterfile)), time() + 600);
    setcookie('XAKCP[lastfilter]',     urlencode(serialize($filter)), time() + 600);
    setcookie('XAKCP[lastfiltersql]',  base64_encode($sql), time() + 600);
}
...
function get_filter($param_str = '')
{
    ...
    if (isset($_GET['uselastfilter']) && isset($_COOKIE['XAKCP']['lastfilterfile'])
        && $_COOKIE['XAKCP']['lastfilterfile'] == sprintf('%X', crc32($filterfile)))
    {
        return array(
            'filter' => unserialize(urldecode($_COOKIE['XAKCP']['lastfilter'])),
            'sql'    => base64_decode($_COOKIE['XAKCP']['lastfiltersql'])
        );
    }
    else
    {
...
{
    $sql = "SELECT COUNT(*) FROM " .$GLOBALS['xak']->table('brand').
...
function admin_info()
{
    $sql = "SELECT * FROM ". $GLOBALS['xak']->table('admin_user')."
