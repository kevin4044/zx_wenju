...
class cls_sql_dump
{
    ...
    function get_table_df($table, $add_drop = false)
    {
        ...
        {
            $table_df .= $tmp_sql . " ENGINE=MyISAM DEFAULT CHARSET=" . str_replace('-', '', XAK_CHARSET) . ";\r\n";
        }
        else
        {
    ...
    function dump_table($path, $vol)
    {
        ...
        $this->dump_sql .= '-- END xakshop v2.x SQL Dump Program ';
    ...
    {
        /* 获取sql文件头部信息 */
        $sql_info = array('date'=>'', 'mysql_ver'=> '', 'php_ver'=>0, 'xak_ver'=>'', 'vol'=>0);
        $fp = fopen($path,'rb');
        $str = fread($fp, 250);
        fclose($fp);
        $arr = explode("\n", $str);
        ...
        foreach ($arr AS $val)
        {
            ...
            if ($pos > 0)
            {
                ...
                elseif ($type == 'Xakshop VERSION')
                {
                    $sql_info['xak_ver'] = $value;
                }
                elseif ($type == 'Vol')
                {
                    $sql_info['vol'] = $value;
