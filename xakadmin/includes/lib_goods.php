...
{
    $sql = "SELECT * FROM " . $GLOBALS['xak']->table('user_rank') .
           " ORDER BY min_points";
...
function get_member_price_list($goods_id)
{
    ...
    $sql = "SELECT user_rank, user_price FROM " .
           $GLOBALS['xak']->table('member_price') .
...
function handle_goods_attr($goods_id, $id_list, $is_spec_list, $value_price_list)
{
    ...
    foreach ($id_list AS $key => $id)
    {
        ...
        // 插入或更新记录
        $sql = "SELECT goods_attr_id FROM " . $GLOBALS['xak']->table('goods_attr') . " WHERE goods_id = '$goods_id' AND attr_id = '$id' AND attr_value = '$value' LIMIT 0, 1";
        $result_id = $GLOBALS['db']->getOne($sql);
        if (!empty($result_id))
        {
            $sql = "UPDATE " . $GLOBALS['xak']->table('goods_attr') . "
        ...
        {
            $sql = "INSERT INTO " . $GLOBALS['xak']->table('goods_attr') . " (goods_id, attr_id, attr_value, attr_price) " .
...
function handle_member_price($goods_id, $rank_list, $price_list)
{
    ...
    foreach ($rank_list AS $key => $rank)
    {
        ...
        // 插入或更新记录
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['xak']->table('member_price') .
        ...
        if ($GLOBALS['db']->getOne($sql) > 0)
        {
            ...
            if ($price < 0)
            {
                $sql = "DELETE FROM " . $GLOBALS['xak']->table('member_price') .
                       " WHERE goods_id = '$goods_id' AND user_rank = '$rank' LIMIT 1";
            }
            else
            {
                $sql = "UPDATE " . $GLOBALS['xak']->table('member_price') .
        ...
        else
        {
            ...
            {
                $sql = "INSERT INTO " . $GLOBALS['xak']->table('member_price') . " (goods_id, user_rank, user_price) " .
...
{
    /* 查询现有的扩展分类 */
    $sql = "SELECT cat_id FROM " . $GLOBALS['xak']->table('goods_cat') .
            " WHERE goods_id = '$goods_id'";
    ...
    if ($delete_list)
    {
        $sql = "DELETE FROM " . $GLOBALS['xak']->table('goods_cat') .
                " WHERE goods_id = '$goods_id' " .
    ...
    {
        // 插入记录
        $sql = "INSERT INTO " . $GLOBALS['xak']->table('goods_cat') .
                " (goods_id, cat_id) " .
...
{
    $sql = "UPDATE " . $GLOBALS['xak']->table('link_goods') . " SET " .
            " goods_id = '$goods_id' " .
            " WHERE goods_id = '0'" .
...
{
    $sql = "UPDATE " . $GLOBALS['xak']->table('group_goods') . " SET " .
            " parent_id = '$goods_id' " .
            " WHERE parent_id = '0'" .
            " AND admin_id = '$_SESSION[admin_id]'";
    $GLOBALS['db']->query($sql);
...
{
    $sql = "UPDATE " . $GLOBALS['xak']->table('goods_article') . " SET " .
            " goods_id = '$goods_id' " .
            " WHERE goods_id = '0'" .
            " AND admin_id = '$_SESSION[admin_id]'";
    $GLOBALS['db']->query($sql);
...
function handle_gallery_image($goods_id, $image_files, $image_descs, $image_urls)
{
    ...
    foreach ($image_descs AS $key => $img_desc)
    {
        ...
        if ($flag)
        {
            ...
            $sql = "INSERT INTO " . $GLOBALS['xak']->table('goods_gallery') . " (goods_id, img_url, img_desc, thumb_url, img_original) " .
                    "VALUES ('$goods_id', '$img_url', '$img_desc', '$thumb_url', '$img_original')";
            $GLOBALS['db']->query($sql);
            /* 不保留商品原图的时候删除原图 */
            if ($proc_thumb && !$GLOBALS['_CFG']['retain_original_img'] && !empty($img_original))
            {
                $GLOBALS['db']->query("UPDATE " . $GLOBALS['xak']->table('goods_gallery') . " SET img_original='' WHERE `goods_id`='{$goods_id}'");
                @unlink('../' . $img_original);
            }
        }
...
function delete_goods($goods_id)
{
    ...
    /* 取得有效商品id */
    $sql = "SELECT DISTINCT goods_id FROM " . $GLOBALS['xak']->table('goods') .
    ...
    /* 删除商品 */
    $sql = "DELETE FROM " . $GLOBALS['xak']->table('goods') .
            " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);

    /* 删除商品的货品记录 */
    $sql = "DELETE FROM " . $GLOBALS['xak']->table('products') .
            " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
    ...
    /* 删除商品相册 */
    $sql = "DELETE FROM " . $GLOBALS['xak']->table('goods_gallery') . " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);

    /* 删除相关表记录 */
    $sql = "DELETE FROM " . $GLOBALS['xak']->table('collect_goods') . " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['xak']->table('goods_article') . " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['xak']->table('goods_attr') . " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['xak']->table('goods_cat') . " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['xak']->table('member_price') . " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['xak']->table('group_goods') . " WHERE parent_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['xak']->table('group_goods') . " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['xak']->table('link_goods') . " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['xak']->table('link_goods') . " WHERE link_goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['xak']->table('tag') . " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['xak']->table('comment') . " WHERE comment_type = 0 AND id_value " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);

    /* 删除相应虚拟商品记录 */
    $sql = "DELETE FROM " . $GLOBALS['xak']->table('virtual_card') . " WHERE goods_id " . db_create_in($goods_id);
...
function generate_goods_sn($goods_id)
{
    ...
    $sql = "SELECT goods_sn FROM " . $GLOBALS['xak']->table('goods') .
...
function check_goods_sn_exist($goods_sn, $goods_id = 0)
{
    ...
    if (empty($goods_id))
    {
        $sql = "SELECT goods_id FROM " . $GLOBALS['xak']->table('goods') ."
...
function get_attr_list($cat_id, $goods_id = 0)
{
    ...
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_input_type, a.attr_type, a.attr_values, v.attr_value, v.attr_price ".
            "FROM " .$GLOBALS['xak']->table('attribute'). " AS a ".
            "LEFT JOIN " .$GLOBALS['xak']->table('goods_attr'). " AS v ".
...
function get_goods_type_specifications()
{
    ...
    $sql = "SELECT DISTINCT cat_id
            FROM " .$GLOBALS['xak']->table('attribute'). "
            WHERE attr_type = 1";
    $row = $GLOBALS['db']->GetAll($sql);
...
function build_attr_html($cat_id, $goods_id = 0)
{
    ...
    foreach ($attr AS $key => $val)
    {
        ...
        {
            $html .= '<input name="attr_value_list[]" class="inputText" type="text" value="' .htmlspecialchars($val['attr_value']). '" size="40" /> ';
        }
        elseif ($val['attr_input_type'] == 2)
        {
            $html .= '<textarea name="attr_value_list[]" rows="3" cols="40" class="inputText">' .htmlspecialchars($val['attr_value']). '</textarea>';
        }
        else
        {
        ...
        $html .= ($val['attr_type'] == 1 || $val['attr_type'] == 2) ?
            $GLOBALS['_LANG']['spec_price'].' <input type="text" class="inputText" name="attr_price_list[]" value="' . $val['attr_price'] . '" size="5" maxlength="10" />' :
...
function get_linked_goods($goods_id)
{
    $sql = "SELECT lg.link_goods_id AS goods_id, g.goods_name, lg.is_double " .
            "FROM " . $GLOBALS['xak']->table('link_goods') . " AS lg, " .
...
function get_group_goods($goods_id)
{
    $sql = "SELECT gg.goods_id, CONCAT(g.goods_name, ' -- [', gg.goods_price, ']') AS goods_name " .
            "FROM " . $GLOBALS['xak']->table('group_goods') . " AS gg, " .
                $GLOBALS['xak']->table('goods') . " AS g " .
...
function get_goods_articles($goods_id)
{
    $sql = "SELECT g.article_id, a.title " .
            "FROM " .$GLOBALS['xak']->table('goods_article') . " AS g, " .
                $GLOBALS['xak']->table('article') . " AS a " .
...
function goods_list($is_delete, $real_goods=1, $conditions = '')
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['xak']->table('goods'). " AS g WHERE is_delete='$is_delete' $where";
        ...
        $sql = "SELECT goods_id, goods_name, goods_type, goods_sn, shop_price, is_on_sale, is_best, is_new, is_hot, sort_order, goods_number, integral, " .
                    ...
                    " FROM " . $GLOBALS['xak']->table('goods') . " AS g WHERE is_delete='$is_delete' $where" .
...
function product_goods_attr_list($goods_id)
{
    ...
    $sql = "SELECT goods_attr_id, attr_value FROM " . $GLOBALS['xak']->table('goods_attr') . " WHERE goods_id = '$goods_id'";
...
function get_goods_specifications_list($goods_id)
{
    ...
    $sql = "SELECT g.goods_attr_id, g.attr_value, g.attr_id, a.attr_name
            FROM " . $GLOBALS['xak']->table('goods_attr') . " AS g
                LEFT JOIN " . $GLOBALS['xak']->table('attribute') . " AS a
...
function product_list($goods_id, $conditions = '')
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['xak']->table('products'). " AS p WHERE goods_id = $goods_id $where";
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);

        $sql = "SELECT product_id, goods_id, goods_attr, product_sn, product_number
                FROM " . $GLOBALS['xak']->table('products') . " AS g
...
function get_product_info($product_id, $filed = '')
{
    ...
    $sql = "SELECT $filed FROM  " . $GLOBALS['xak']->table('products') . " WHERE product_id = '$product_id'";
...
function check_goods_specifications_exist($goods_id)
{
    ...
    $sql = "SELECT COUNT(a.attr_id)
            FROM " .$GLOBALS['xak']->table('attribute'). " AS a, " .$GLOBALS['xak']->table('goods'). " AS g
...
function check_goods_attr_exist($goods_attr, $goods_id, $product_id = 0)
{
    ...
    if (empty($product_id))
    {
        $sql = "SELECT product_id FROM " . $GLOBALS['xak']->table('products') ."
...
function check_product_sn_exist($product_sn, $product_id = 0)
{
    ...
    $sql="SELECT goods_id FROM ". $GLOBALS['xak']->table('goods')."WHERE goods_sn='$product_sn'";
