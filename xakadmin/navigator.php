...
$exc = new exchange($xak->table("nav"), $db, 'id', 'name');
...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    elseif ($_REQUEST['step'] == 2)
    {
        ...
        $vieworder = $db->getOne("SELECT max(vieworder) FROM ". $xak->table('nav') . " WHERE type = '". $item_type ."'");
        ...
        if($item_ifshow == 1 && $item_type == 'middle')
        {
            ...
            if($arr)
            {
                ...
                $sql = "INSERT INTO " . $GLOBALS['xak']->table('nav') . " (name,ctype,cid,ifshow,vieworder,opennew,url,type) VALUES('$item_name','".$arr['type']."','".$arr['id']."','$item_ifshow','$item_vieworder','$item_opennew','$item_url','$item_type')";
            }
        }

        if(empty($sql))
        {
            $sql = "INSERT INTO " . $GLOBALS['xak']->table('nav') . " (name,ifshow,vieworder,opennew,url,type) VALUES('$item_name','$item_ifshow','$item_vieworder','$item_opennew','$item_url','$item_type')";
        }
        $db->query($sql);
        clear_cache_files();
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    if (empty($_REQUEST['step']))
    {
        ...
        $row = $db->getRow("SELECT * FROM " . $GLOBALS['xak']->table('nav') . " WHERE id='$id'");
    ...
    elseif ($_REQUEST['step'] == 2)
    {
        ...
        $row = $db->getRow("SELECT ctype,cid,ifshow,type FROM " . $GLOBALS['xak']->table('nav') . " WHERE id = '$id'");
        ...
        if($arr)
        {
            ...
            $sql = "UPDATE " . $GLOBALS['xak']->table('nav') .
...
{
    $id = (int)$_GET['id'];
    $row = $db->getRow("SELECT ctype,cid,type FROM " . $GLOBALS['xak']->table('nav') . " WHERE id = '$id' LIMIT 1");
    ...
    $sql = " DELETE FROM " . $GLOBALS['xak']->table('nav') . " WHERE id='$id' LIMIT 1";
    $db->query($sql);
    clear_cache_files();
    xak_header("Location: navigator.php?act=list\n");
    exit;
}
...
if ($_REQUEST['act'] == 'toggle_ifshow')
{
    ...
    $row = $db->getRow("SELECT type,ctype,cid FROM " . $GLOBALS['xak']->table('nav') . " WHERE id = '$id' LIMIT 1");
...
function get_nav()
{
    ...
    if($result === false)
    {
        ...
        $sql = "SELECT count(*) FROM " .$GLOBALS['xak']->table('nav');
...
function nav_update($id, $args)
{
    ...
    return $GLOBALS['db']->autoExecute($GLOBALS['xak']->table('nav'), $args, 'update', "id='$id'");
}
...
function is_show_in_nav($type, $id)
{
    if($type == 'c')
    {
        $tablename = $GLOBALS['xak']->table('category');
    }
    else
    {
        $tablename = $GLOBALS['xak']->table('article_cat');
    }
