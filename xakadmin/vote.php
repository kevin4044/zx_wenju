...
$exc = new exchange($xak->table("vote"), $db, 'vote_id', 'vote_name');
$exc_opn = new exchange($xak->table("vote_option"), $db, 'option_id', 'option_name');
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    /* 查看广告名称是否有重复 */
    $sql = "SELECT COUNT(*) FROM " .$xak->table('vote'). " WHERE vote_name='$_POST[vote_name]'";
    if ($db->getOne($sql) == 0)
    {
        /* 插入数据 */
        $sql = "INSERT INTO ".$xak->table('vote')." (vote_name, start_time, end_time, can_multi, vote_count)
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    /* 获取数据 */
    $vote_arr = $db->GetRow("SELECT * FROM ".$xak->table('vote')." WHERE vote_id='$_REQUEST[id]'");
...
elseif ($_REQUEST['act'] == 'new_option')
{
    ...
    {
        /* 查看调查标题是否有重复 */
        $sql = 'SELECT COUNT(*) FROM ' .$xak->table('vote_option').
        ...
        {
            $sql = 'INSERT INTO ' .$xak->table('vote_option'). ' (vote_id, option_name, option_count) '.
...
elseif ($_REQUEST['act'] == 'edit_option_name')
{
    ...
    /* 检查名称是否重复 */
    $vote_id = $db->getOne('SELECT vote_id FROM ' .$xak->table('vote_option'). " WHERE option_id='$id'");
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    {
        /* 同时删除调查选项 */
        $db->query("DELETE FROM " .$xak->table('vote_option'). " WHERE vote_id = '$id'");
        clear_cache_files();
...
function get_votelist()
{
    ...
    /* 记录总数以及页数 */
    $sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['xak']->table('vote');
    $filter['record_count'] = $GLOBALS['db']->getOne($sql);

    $filter = page_and_size($filter);

    /* 查询数据 */
    $sql  = 'SELECT * FROM ' .$GLOBALS['xak']->table('vote'). ' ORDER BY vote_id DESC';
...
function get_optionlist($id)
{
    ...
    $sql  = 'SELECT option_id, vote_id, option_name, option_count, option_order'.
            ' FROM ' .$GLOBALS['xak']->table('vote_option').
