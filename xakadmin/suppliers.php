 ...
 * ----------------------------------------------------------------------------
 * $Author: wanglei $
 * $File: suppliers.php 2009-05-13 09:31:42Z wanglei $
 */
...
elseif ($_REQUEST['act'] == 'edit_suppliers_name')
{
    ...
    {
        /* 保存供货商信息 */
        $sql = "UPDATE " . $xak->table('suppliers') . "
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    if ($suppliers['suppliers_id'])
    {
        /* 判断供货商是否存在订单 */
        $sql = "SELECT COUNT(*)
                FROM " . $xak->table('order_info') . "AS O, " . $xak->table('order_goods') . " AS OG, " . $xak->table('goods') . " AS G
        ...
        $sql = "DELETE FROM " . $xak->table('suppliers') . "
            WHERE suppliers_id = '$id'";
        $db->query($sql);
        ...
        {
            $sql = "DELETE FROM " . $xak->table($value) . " WHERE suppliers_id = '$id'";
            $db->query($sql, 'SILENT');
        }
...
elseif ($_REQUEST['act'] == 'is_check')
{
    ...
    if ($suppliers['suppliers_id'])
    {
        ...
        $db->autoExecute($xak->table('suppliers'), $_suppliers, '', "suppliers_id = '$id'");
        clear_cache_files();
...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    else
    {
        ...
        if (isset($_POST['remove']))
        {
            ...
            foreach ($table_array as $value)
            {
                $sql = "DELETE FROM " . $xak->table($value) . " WHERE suppliers_id " . db_create_in($ids) . " ";
                $db->query($sql, 'SILENT');
            }
...
elseif (in_array($_REQUEST['act'], array('add', 'edit')))
{
    ...
    elseif ($_REQUEST['act'] == 'edit')
    {
        ...
        /* 取得供货商信息 */
        $id = $_REQUEST['id'];
        $sql = "SELECT * FROM " . $xak->table('suppliers') . " WHERE suppliers_id = '$id'";
...
elseif (in_array($_REQUEST['act'], array('insert', 'update')))
{
    ...
    if ($_REQUEST['act'] == 'insert')
    {
        ...
        $db->autoExecute($xak->table('suppliers'), $suppliers, 'INSERT');
        $suppliers['suppliers_id'] = $db->insert_id();

        if (isset($_POST['admins']))
        {
            $sql = "UPDATE " . $xak->table('admin_user') . " SET suppliers_id = '" . $suppliers['suppliers_id'] . "', action_list = '" . SUPPLIERS_ACTION_LIST . "' WHERE user_id " . db_create_in($_POST['admins']);
            $db->query($sql);
        }
    ...
    if ($_REQUEST['act'] == 'update')
    {
        ...
        /* 取得供货商信息 */
        $sql = "SELECT * FROM " . $xak->table('suppliers') . " WHERE suppliers_id = '" . $suppliers['id'] . "'";
        ...
        /* 保存供货商信息 */
        $db->autoExecute($xak->table('suppliers'), $suppliers['new'], 'UPDATE', "suppliers_id = '" . $suppliers['id'] . "'");

        /* 清空供货商的管理员 */
        $sql = "UPDATE " . $xak->table('admin_user') . " SET suppliers_id = 0, action_list = '" . SUPPLIERS_ACTION_LIST . "' WHERE suppliers_id = '" . $suppliers['id'] . "'";
        $db->query($sql);

        /* 添加供货商的管理员 */
        if (isset($_POST['admins']))
        {
            $sql = "UPDATE " . $xak->table('admin_user') . " SET suppliers_id = '" . $suppliers['old']['suppliers_id'] . "' WHERE user_id " . db_create_in($_POST['admins']);
            $db->query($sql);
        }
...
function suppliers_list()
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['xak']->table('suppliers') . $where;
