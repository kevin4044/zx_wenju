...
if ($_REQUEST['act'] == 'list')
{
    ...
    $sql = "SELECT rank_id, rank_name, min_points FROM ".$xak->table('user_rank')." ORDER BY min_points ASC ";
    $rs = $db->query($sql);
...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    /* 取出注册扩展字段 */
    $sql = 'SELECT * FROM ' . $xak->table('reg_fields') . ' WHERE type < 2 AND display = 1 AND id != 6 ORDER BY dis_order, id';
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $db->autoExecute($xak->table('users'), $other, 'UPDATE', "user_name = '$username'");
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT u.user_name, u.sex, u.birthday, u.pay_points, u.rank_points, u.user_rank , u.user_money, u.frozen_money, u.credit_line, u.parent_id, u2.user_name as parent_username, u.qq, u.msn, u.office_phone, u.home_phone, u.mobile_phone".
        " FROM " .$xak->table('users'). " u LEFT JOIN " . $xak->table('users') . " u2 ON u.parent_id = u2.user_id WHERE u.user_id='$_GET[id]'";
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    if(!empty($password))
    {
			$sql="UPDATE ".$xak->table('users'). "SET `ec_salt`='0' WHERE user_name= '".$username."'";
			$db->query($sql);
	}
    /* 更新用户扩展字段的数据 */
    $sql = 'SELECT id FROM ' . $xak->table('reg_fields') . ' WHERE type = 0 AND display = 1 ORDER BY dis_order, id';   //读出所有扩展字段的id
...
elseif ($_REQUEST['act'] == 'batch_remove')
{
    ...
    {
        $sql = "SELECT user_name FROM " . $xak->table('users') . " WHERE user_id " . db_create_in($_POST['checkboxes']);
        $col = $db->getCol($sql);
...
elseif ($_REQUEST['act'] == 'edit_username')
{
    ...
    {
        if ($_CFG['integrate_code'] != 'xakshop')
        {
            /* 更新商城会员表 */
            $db->query('UPDATE ' .$xak->table('users'). " SET user_name = '$username' WHERE user_id = '$id'");
        }
...
elseif ($_REQUEST['act'] == 'edit_email')
{
    ...
    $sql = "SELECT user_name FROM " . $xak->table('users') . " WHERE user_id = '$id'";
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $sql = "SELECT user_name FROM " . $xak->table('users') . " WHERE user_id = '" . $_GET['id'] . "'";
    $username = $db->getOne($sql);
    /* 通过插件来删除用户 */
    $users =& init_users();
...
elseif ($_REQUEST['act'] == 'address_list')
{
    ...
    $sql = "SELECT a.*, c.region_name AS country_name, p.region_name AS province, ct.region_name AS city_name, d.region_name AS district_name ".
           " FROM " .$xak->table('user_address'). " as a ".
           " LEFT JOIN " . $xak->table('region') . " AS c ON c.region_id = a.country " .
           " LEFT JOIN " . $xak->table('region') . " AS p ON p.region_id = a.province " .
           " LEFT JOIN " . $xak->table('region') . " AS ct ON ct.region_id = a.city " .
           " LEFT JOIN " . $xak->table('region') . " AS d ON d.region_id = a.district " .
           " WHERE user_id='$id'";
...
elseif ($_REQUEST['act'] == 'remove_parent')
{
    ...
    $sql = "UPDATE " . $xak->table('users') . " SET parent_id = 0 WHERE user_id = '" . $_GET['id'] . "'";
    $db->query($sql);
    ...
    for ($i = 1; $i<=$num; $i++)
    {
        ...
        if ($count)
        {
            $sql = "SELECT user_id, user_name, '$i' AS level, email, is_validated, user_money, frozen_money, rank_points, pay_points, reg_time ".
                    " FROM " . $GLOBALS['xak']->table('users') . " WHERE user_id IN($up_uid)" .
...
function user_list()
{
    ...
    if ($result === false)
    {
        ...
        if ($filter['rank'])
        {
            $sql = "SELECT min_points, max_points, special_rank FROM ".$GLOBALS['xak']->table('user_rank')." WHERE rank_id = '$filter[rank]'";
        ...
        $filter['record_count'] = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['xak']->table('users') . $ex_where);

        /* 分页大小 */
        $filter = page_and_size($filter);
        $sql = "SELECT user_id, user_name, email, is_validated, user_money, frozen_money, rank_points, pay_points, reg_time ".
                " FROM " . $GLOBALS['xak']->table('users') . $ex_where .
