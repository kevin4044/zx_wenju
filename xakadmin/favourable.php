...
$exc = new exchange($xak->table('favourable_activity'), $db, 'act_id', 'act_name');
...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    else
    {
        ...
        {
            /* 删除记录 */
            $sql = "DELETE FROM " . $xak->table('favourable_activity') .
...
elseif ($_REQUEST['act'] == 'edit_sort_order')
{
    ...
    $sql = "UPDATE " . $xak->table('favourable_activity') .
            " SET sort_order = '$val'" .
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    if ($favourable['act_range'] != FAR_ALL && !empty($favourable['act_range_ext']))
    {
        ...
        {
            $sql = "SELECT cat_id AS id, cat_name AS name FROM " . $xak->table('category') .
                " WHERE cat_id " . db_create_in($favourable['act_range_ext']);
        }
        elseif ($favourable['act_range'] == FAR_BRAND)
        {
            $sql = "SELECT brand_id AS id, brand_name AS name FROM " . $xak->table('brand') .
                " WHERE brand_id " . db_create_in($favourable['act_range_ext']);
        }
        else
        {
            $sql = "SELECT goods_id AS id, goods_name AS name FROM " . $xak->table('goods') .
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if ($is_add)
    {
        $db->autoExecute($xak->table('favourable_activity'), $favourable, 'INSERT');
        $favourable['act_id'] = $db->insert_id();
    }
    else
    {
        $db->autoExecute($xak->table('favourable_activity'), $favourable, 'UPDATE', "act_id = '$favourable[act_id]'");
    }
...
function favourable_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['xak']->table('favourable_activity') .
                " WHERE 1 $where";
