...
$exc = new exchange($xak->table("category"), $db, 'cat_id', 'cat_name');
...
if ($_REQUEST['act'] == 'list')
{
	...
	$smarty->assign('action_link2',  array('href' => 'category.php?act=quickadd', 'text' => '批量添加分类'));
...
//-- 批量添加商品分类
/*------------------------------------------------------ */
if ($_REQUEST['act'] == 'quickadd')
{
    /* 权限检查 */
    admin_priv('cat_manage');

    /* 模板赋值 */
    $smarty->assign('ur_here',      '批量添加商品分类');
    $smarty->assign('action_link',  array('href' => 'category.php?act=list', 'text' => $_LANG['03_category_list']));
	
	$smarty->assign('cat_select',   cat_list(0, 0, true));//商品分类
    $smarty->assign('form_act',     'savequick');
	...
	{
		$catgory_info[$i]['id']=$i;
		$catgory_info[$i]['units']='件';		
	}
	$smarty->assign('catgory_info',$catgory_info);

    /* 显示页面 */
    assign_query_info();
    $smarty->display('category_quick.htm');
}
...
if ($_REQUEST['act'] == 'insert')
{
    ...
    /* 入库的操作 */
    if ($db->autoExecute($xak->table('category'), $cat) !== false)
    {
        $cat_id = $db->insert_id();
        if($cat['show_in_nav'] == 1)
        {
            $vieworder = $db->getOne("SELECT max(vieworder) FROM ". $xak->table('nav') . " WHERE type = 'middle'");
            $vieworder += 2;
            //显示在自定义导航栏中
            $sql = "INSERT INTO " . $xak->table('nav') .
...
//-- 商品分类批量添加时的处理
/*------------------------------------------------------ */
if ($_REQUEST['act'] == 'savequick')
{
	/* 权限检查 */
    admin_priv('cat_manage');
	
	$auto_error = $auto_error2 = $auto_ok = 0;
	//开始检查数据
	for($i=1; $i<21; $i++)
	{
		$parent = !empty($_POST['parent_id'])? intval($_POST['parent_id']):0;
		if(trim($_POST['catgory_name'.$i])!='')
		{
			//初始化分类信息
			$cat=array();
			$cat['parent_id'] = $parent;
			$cat['measure_unit'] = !empty($_POST['catgory_units'.$i]) ? trim($_POST['catgory_units'.$i]) : '件';
			$cat['cat_name']=trim($_POST['catgory_name'.$i]);
			$cat['is_show']= !empty($_POST['catgory_show'.$i]) ? intval($_POST['catgory_show'.$i]): 0;
			$cat['sort_order']=!empty($_POST['catgory_id'.$i]) ? intval($_POST['catgory_id'.$i]): 50;
	...
	clear_cache_files();    // 清除缓存
	if($auto_error ==0 && $auto_error==0 && $auto_ok > 0)
	{
		$link[0]['text'] = $_LANG['back_list'];
		$link[0]['href'] = 'category.php?act=list';
		sys_msg('批量添加商品分类成功', 0, $link);
	}
	elseif($auto_error ==0 && $auto_error==0 && $auto_ok == 0)
	{
		$link[0]['text'] = $_LANG['back_list'];
		$link[0]['href'] = 'category.php?act=list';
		sys_msg('没有添加商品分类', 0, $link);
	}
	else
	{
		$link[0]['text'] = $_LANG['back_list'];
		$link[0]['href'] = 'category.php?act=list';
		sys_msg('部分商品分类没有添加成功,可能是商品名称重复!', 0, $link);
	}
} 
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    if ($cat_info['filter_attr'])
    {
        ...
        {
            $attr_cat_id = $db->getOne("SELECT cat_id FROM " . $xak->table('attribute') . " WHERE attr_id = '" . intval($v) . "'");
    ...
    //分类是否存在首页推荐
    $res = $db->getAll("SELECT recommend_type FROM " . $xak->table("cat_recommend") . " WHERE cat_id=" . $cat_id);
    if (!empty($res))
    {
        $cat_recommend = array();
        foreach($res as $data)
...
if ($_REQUEST['act'] == 'update')
{
    ...
    $dat = $db->getRow("SELECT cat_name, show_in_nav FROM ". $xak->table('category') . " WHERE cat_id = '$cat_id'");

    if ($db->autoExecute($xak->table('category'), $cat, 'UPDATE', "cat_id='$cat_id'"))
    {
        if($cat['cat_name'] != $dat['cat_name'])
        {
            //如果分类名称发生了改变
            $sql = "UPDATE " . $xak->table('nav') . " SET name = '" . $cat['cat_name'] . "' WHERE ctype = 'c' AND cid = '" . $cat_id . "' AND type = 'middle'";
            $db->query($sql);
        }
        if($cat['show_in_nav'] != $dat['show_in_nav'])
        {
            ...
            {
                //显示
                $nid = $db->getOne("SELECT id FROM ". $xak->table('nav') . " WHERE ctype = 'c' AND cid = '" . $cat_id . "' AND type = 'middle'");
                if(empty($nid))
                {
                    //不存在
                    $vieworder = $db->getOne("SELECT max(vieworder) FROM ". $xak->table('nav') . " WHERE type = 'middle'");
                    ...
                    $sql = "INSERT INTO " . $xak->table('nav') . " (name,ctype,cid,ifshow,vieworder,opennew,url,type) VALUES('" . $cat['cat_name'] . "', 'c', '$cat_id','1','$vieworder','0', '" . $uri . "','middle')";
                }
                else
                {
                    $sql = "UPDATE " . $xak->table('nav') . " SET ifshow = 1 WHERE ctype = 'c' AND cid = '" . $cat_id . "' AND type = 'middle'";
                }
                $db->query($sql);
            }
            else
            {
                //去除
                $db->query("UPDATE " . $xak->table('nav') . " SET ifshow = 0 WHERE ctype = 'c' AND cid = '" . $cat_id . "' AND type = 'middle'");
            }
        }
...
if ($_REQUEST['act'] == 'move_cat')
{
    ...
    /* 更新商品分类 */
    $sql = "UPDATE " .$xak->table('goods'). " SET cat_id = '$target_cat_id' ".
           "WHERE cat_id = '$cat_id'";
    if ($db->query($sql))
    {
        /* 清除缓存 */
...
if ($_REQUEST['act'] == 'toggle_show_in_nav')
{
    ...
    if (cat_update($id, array('show_in_nav' => $val)) != false)
    {
        if($val == 1)
        {
            ...
            $vieworder = $db->getOne("SELECT max(vieworder) FROM ". $xak->table('nav') . " WHERE type = 'middle'");
            $vieworder += 2;
            $catname = $db->getOne("SELECT cat_name FROM ". $xak->table('category') . " WHERE cat_id = '$id'");
            //显示在自定义导航栏中
            $_CFG['rewrite'] = 0;
            $uri = build_uri('category', array('cid'=> $id), $catname);

            $nid = $db->getOne("SELECT id FROM ". $xak->table('nav') . " WHERE ctype = 'c' AND cid = '" . $id . "' AND type = 'middle'");
            if(empty($nid))
            {
                //不存在
                $sql = "INSERT INTO " . $xak->table('nav') . " (name,ctype,cid,ifshow,vieworder,opennew,url,type) VALUES('" . $catname . "', 'c', '$id','1','$vieworder','0', '" . $uri . "','middle')";
            }
            else
            {
                $sql = "UPDATE " . $xak->table('nav') . " SET ifshow = 1 WHERE ctype = 'c' AND cid = '" . $id . "' AND type = 'middle'";
            }
            $db->query($sql);
        }
        else
        {
            //去除
            $db->query("UPDATE " . $xak->table('nav') . "SET ifshow = 0 WHERE ctype = 'c' AND cid = '" . $id . "' AND type = 'middle'");
        }
        clear_cache_files();
        make_json_result($val);
    }
...
if ($_REQUEST['act'] == 'remove')
{
    ...
    $cat_name = $db->getOne('SELECT cat_name FROM ' .$xak->table('category'). " WHERE cat_id='$cat_id'");

    /* 当前分类下是否有子分类 */
    $cat_count = $db->getOne('SELECT COUNT(*) FROM ' .$xak->table('category'). " WHERE parent_id='$cat_id'");

    /* 当前分类下是否存在商品 */
    $goods_count = $db->getOne('SELECT COUNT(*) FROM ' .$xak->table('goods'). " WHERE cat_id='$cat_id'");

    /* 如果不存在下级子分类和商品，则删除之 */
    if ($cat_count == 0 && $goods_count == 0)
    {
        /* 删除分类 */
        $sql = 'DELETE FROM ' .$xak->table('category'). " WHERE cat_id = '$cat_id'";
        if ($db->query($sql))
        {
            $db->query("DELETE FROM " . $xak->table('nav') . "WHERE ctype = 'c' AND cid = '" . $cat_id . "' AND type = 'middle'");
            clear_cache_files();
...
{
    $sql = "SELECT * FROM " .$GLOBALS['xak']->table('category'). " WHERE cat_id='$cat_id' LIMIT 1";
...
function cat_update($cat_id, $args)
{
    ...
    return $GLOBALS['db']->autoExecute($GLOBALS['xak']->table('category'), $args, 'update', "cat_id='$cat_id'");
}
...
function get_attr_list()
{
    $sql = "SELECT a.attr_id, a.cat_id, a.attr_name ".
           ...
           $GLOBALS['xak']->table('goods_type') . " AS c ".
...
function insert_cat_recommend($recommend_type, $cat_id)
{
    ...
    {
        //取得之前的分类
        $recommend_res = $GLOBALS['db']->getAll("SELECT recommend_type FROM " . $GLOBALS['xak']->table("cat_recommend") . " WHERE cat_id=" . $cat_id);
        if (empty($recommend_res))
        {
            foreach($recommend_type as $data)
            {
                $data = intval($data);
                $GLOBALS['db']->query("INSERT INTO " . $GLOBALS['xak']->table("cat_recommend") . "(cat_id, recommend_type) VALUES ('$cat_id', '$data')");
            }
        }
        else
        {
            ...
            {
                $GLOBALS['db']->query("DELETE FROM " . $GLOBALS['xak']->table("cat_recommend") . " WHERE cat_id=$cat_id AND recommend_type " . db_create_in($delete_array));
            }
    ...
    {
        $GLOBALS['db']->query("DELETE FROM ". $GLOBALS['xak']->table("cat_recommend") . " WHERE cat_id=" . $cat_id);
    }
}
