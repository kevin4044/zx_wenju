...
$exc = new exchange($xak->table('payment'), $db, 'pay_code', 'pay_name');
...
if ($_REQUEST['act'] == 'list')
{
    /* 查询数据库中启用的支付方式 */
    $pay_list = array();
    $sql = "SELECT * FROM " . $xak->table('payment') . " WHERE enabled = '1' ORDER BY pay_order";
    $res = $db->query($sql);
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT * FROM " . $xak->table('payment') . " WHERE pay_code = '$_REQUEST[code]' AND enabled = '1'";
    $pay = $db->getRow($sql);
    if (empty($pay))
    {
...
elseif (isset($_POST['Submit']))
{
    ...
    $sql = "SELECT COUNT(*) FROM " . $xak->table('payment') .
    ...
    {
        /* 安装，检查该支付方式是否曾经安装过 */
        $sql = "SELECT COUNT(*) FROM " . $xak->table('payment') . " WHERE pay_code = '$_REQUEST[pay_code]'";
        if ($db->GetOne($sql) > 0)
        {
