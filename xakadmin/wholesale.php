...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    /* 删除记录 */
    $sql = "DELETE FROM " . $xak->table('wholesale') .
...
elseif ($_REQUEST['act'] == 'toggle_enabled')
{
    ...
    $sql = "UPDATE " . $xak->table('wholesale') .
            " SET enabled = '$val'" .
            " WHERE act_id = '$id' LIMIT 1";
    $db->query($sql);
...
elseif ($_REQUEST['act'] == 'batch_add')
{
    ...
    $sql = "SELECT rank_id, rank_name FROM " . $xak->table('user_rank') .
...
elseif ($_REQUEST['act'] == 'batch_add_insert')
{
    ...
    $sql = "SELECT goods_name, goods_id FROM " . $xak->table('goods') .
    ...
    if (isset($_POST['rank_id']))
    {
        ...
        {
            $sql = "SELECT COUNT(act_id) AS num, goods_id FROM " . $xak->table('wholesale') .
                    " WHERE goods_id IN ($dst_goods) " .
    ...
    foreach ($_POST['dst_goods_lists'] as $goods_value)
    {
        ...
        /* 保存数据 */
        $db->autoExecute($xak->table('wholesale'), $_wholesale, 'INSERT');
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if (isset($_POST['rank_id']))
    {
        foreach ($_POST['rank_id'] as $rank_id)
        {
            $sql = "SELECT COUNT(*) FROM " . $xak->table('wholesale') .
                    " WHERE goods_id = '$goods_id' " .
                    " AND CONCAT(',', rank_ids, ',') LIKE CONCAT('%,', '$rank_id', ',%')";
            if (!$is_add)
            {
    ...
    $sql = "SELECT a.attr_id " .
            "FROM " . $xak->table('goods') . " AS g, " . $xak->table('attribute') . " AS a " .
    ...
    foreach ($key_list as $key)
    {
        ...
        if (!empty($attr))
        {
            ...
            $sql = "SELECT product_id FROM " . $xak->table('products') . " WHERE goods_attr = '$goods_attr' AND goods_id = '$goods_id'";
            if (!$db->getOne($sql))
            {
                $attr_error = true;
                continue;
    ...
    if ($is_add)
    {
        $db->autoExecute($xak->table('wholesale'), $wholesale, 'INSERT');
        $wholesale['act_id'] = $db->insert_id();
    }
    else
    {
        $db->autoExecute($xak->table('wholesale'), $wholesale, 'UPDATE', "act_id = '$wholesale[act_id]'");
    }
...
function wholesale_list()
{
    /* 查询会员等级 */
    $rank_list = array();
    $sql = "SELECT rank_id, rank_name FROM " . $GLOBALS['xak']->table('user_rank');
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['xak']->table('wholesale') .
                " WHERE 1 $where";
        ...
        /* 查询 */
        $sql = "SELECT * ".
                "FROM " . $GLOBALS['xak']->table('wholesale') .
                " WHERE 1 $where ".
