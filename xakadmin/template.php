...
if ($_REQUEST['act'] == 'list')
{
    ...
    $sql = "DELETE FROM ".$xak->table('template')." WHERE 1 ";
...
if ($_REQUEST['act'] == 'setup')
{
    ...
    $sql = "SELECT region, library, sort_order, id, number, type FROM ".$xak->table('template') ." ".
...
if ($_REQUEST['act'] == 'setting')
{
    ...
    $curr_template = $_CFG['template'];
    $db->query("DELETE FROM " .$xak->table('template'). " WHERE remarks = '' AND filename = '$_POST[template_file]' AND theme = '$curr_template'");
    ...
    foreach ($_POST['regions'] AS $key => $val)
    {
        ...
        {
            $sql = "INSERT INTO " .$xak->table('template').
...
if ($_REQUEST['act'] == 'install')
{
    ...
    $sql = "UPDATE " .$GLOBALS['xak']->table('shop_config'). " SET value = '$tpl_name' WHERE code = 'template'";
    $step_one = $db->query($sql, 'SILENT');
    $sql = "UPDATE " .$GLOBALS['xak']->table('shop_config'). " SET value = '$tpl_fg' WHERE code = 'stylename'";
...
if ($_REQUEST['act'] == 'backup_setting')
{
    ...
    $sql = "SELECT DISTINCT(remarks) FROM " . $xak->table('template') . " WHERE theme = '" . $_CFG['template'] . "' AND remarks > ''";
    $col = $db->getCol($sql);
    $remarks = array();
    foreach ($col as $val)
    {
    ...
    $sql = "SELECT DISTINCT(filename) FROM " . $xak->table('template') . " WHERE theme = '" . $_CFG['template'] . "' AND remarks = ''";
    $col = $db->getCol($sql);
    $files = array();
    foreach ($col as $val)
    {
...
if ($_REQUEST['act'] == 'act_backup_setting')
{
    ...
    $sql = "SELECT COUNT(*) FROM " . $xak->table('template') . " WHERE remarks='$remarks' AND theme = '" . $_CFG['template'] . "'";
    if ($db->getOne($sql) > 0)
    {
...
if ($_REQUEST['act'] == 'del_backup')
{
    ...
    if ($remarks)
    {
        $sql = "DELETE FROM " . $xak->table('template') . " WHERE remarks='$remarks' AND theme = '" . $_CFG['template'] . "'";
        $db->query($sql);
    }
...
if ($_REQUEST['act'] == 'restore_backup')
{
    ...
    if ($remarks)
    {
        ...
        if ($arr)
        {
            ...
            /* 文件修改成功后，恢复数据库 */
            $sql = "DELETE FROM " .$xak->table('template').
                   " WHERE remarks = '' AND  theme = '" . $_CFG['template'] . "'".
