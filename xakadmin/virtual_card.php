...
if ($_REQUEST['act'] == 'replenish')
{
    ...
    {
        $goods_name = $db->GetOne("SELECT goods_name From ".$xak->table('goods')." WHERE goods_id='".$_REQUEST['goods_id']."' AND is_real = 0 AND extension_code='virtual_card' ");
        if (empty($goods_name))
        {
            $link[] = array('text'=>$_LANG['go_back'],'href'=>'virtual_card.php?act=list');
...
elseif ($_REQUEST['act'] == 'edit_replenish')
{
    ...
    $sql = "SELECT T1.card_id, T1.goods_id, T2.goods_name,T1.card_sn, T1.card_password, T1.end_date, T1.crc32 FROM ".
            $xak->table('virtual_card')." AS T1, ".$xak->table('goods')." AS T2 ".
...
elseif ($_REQUEST['act'] == 'action')
{
    ...
    {
        $sql = "SELECT count(*) FROM ".$xak->table('virtual_card')." WHERE goods_id='".$_POST['goods_id']."' AND card_sn='$coded_card_sn'";
    ...
    if (empty($_POST['old_card_sn']))
    {
        ...
        $add_date = gmtime();
        $sql = "INSERT INTO ".$xak->table('virtual_card')." (goods_id, card_sn, card_password, end_date, add_date, crc32) ".
        ...
        if (empty($_POST['old_card_sn']))
        {
            $sql = "UPDATE ".$xak->table('goods')." SET goods_number= goods_number+1 WHERE goods_id='$_POST[goods_id]'";
            $db->query($sql);
        }
    ...
    else
    {
        /* 更新数据 */
        $end_date = strtotime($_POST['end_dateYear'] . "-" . $_POST['end_dateMonth'] . "-" . $_POST['end_dateDay']);
        $sql = "UPDATE ".$xak->table('virtual_card')." SET card_sn='$coded_card_sn', card_password='$coded_card_password', end_date='$end_date' ".
...
elseif ($_REQUEST['act'] == 'batch_drop_card')
{
    ...
    $sql = "DELETE FROM ".$xak->table('virtual_card')." WHERE card_id ".db_create_in(implode(',',$_POST['checkboxes']));
    if ($db->query($sql))
    {
        /* 商品数量减$num */
...
elseif ($_REQUEST['act'] == 'batch_insert')
{
    ...
    foreach ($_POST['checked'] as $key)
    {
        ...
        $db->AutoExecute($xak->table('virtual_card'), $rec, 'INSERT');
        $i++;
    }
...
elseif ($_REQUEST['act'] == 'submit_change')
{
    ...
    if (isset($_POST['old_string']) && isset($_POST['new_string']))
    {
        ...
        $sql = "SELECT card_id, card_sn, card_password FROM " . $xak->table('virtual_card') . " WHERE crc32 = '$old_crc32'";
        $res = $db->query($sql);
        while ($row = $db->fetchRow($res))
        {
            ...
            $row['crc32'] = $new_crc32;
            $db->autoExecute($xak->table('virtual_card'), $row, 'UPDATE', 'card_id = ' . $row['card_id']);
        }
...
elseif ($_REQUEST['act'] == 'toggle_sold')
{
    ...
    $sql = "UPDATE ".$xak->table('virtual_card')." SET is_saled= '$val' WHERE card_id='$id'";

    if ($db->query($sql, 'SILENT'))
    {
        /* 修改商品库存 */
        $sql = "SELECT goods_id FROM " . $xak->table('virtual_card') . " WHERE card_id = '$id' LIMIT 1";
...
elseif ($_REQUEST['act'] == 'remove_card')
{
    ...
    $row = $db->GetRow('SELECT card_sn, goods_id FROM ' . $xak->table('virtual_card') . " WHERE card_id = '$id'");

    $sql = 'DELETE FROM ' . $xak->table('virtual_card') . " WHERE card_id = '$id'";
    if ($db->query($sql, 'SILENT'))
    {
        /* 修改商品数量 */
...
elseif ($_REQUEST['act'] == 'start_change')
{
    ...
    $sql = "SELECT crc32, count(*) AS cnt FROM " . $xak->table('virtual_card') . " GROUP BY crc32";
...
elseif ($_REQUEST['act'] == 'on_change')
{
   ...
   while ($row = $db->fetchRow($res))
   {
       ...
       if (!$db->autoExecute($xak->table('virtual_card'), $row, 'UPDATE', 'card_id = ' . $row['card_id']))
       {
    ...
    // 查询是否还有未更新的
    $sql      = "SELECT COUNT(*) FROM " . $xak->table('virtual_card') . " WHERE crc32 = '$old_crc32' ";
    ...
    else
    {
        ...
        $sql = "SELECT crc32, count(*) AS cnt FROM " . $GLOBALS['xak']->table('virtual_card') . " GROUP BY crc32";
        $res = $GLOBALS['db']->query($sql);
        while ($row = $db->fetchRow($res))
        {
...
function get_replenish_list()
{
    ...
    $sql = "SELECT COUNT(*) FROM " . $GLOBALS['xak']->table('virtual_card') . " WHERE 1 $where";
...
{
    $sql = "SELECT COUNT(*) FROM " . $GLOBALS['xak']->table('virtual_card') . " WHERE goods_id = '$goods_id' AND is_saled = 0";
    $goods_number = $GLOBALS['db']->getOne($sql);

    $sql = "UPDATE " . $GLOBALS['xak']->table('goods') . " SET goods_number = '$goods_number' WHERE goods_id = '$goods_id' AND extension_code='virtual_card'";
