...
elseif ($_REQUEST['act'] == 'del')
{
    ...
    $stat = $db->getOne("SELECT is_separate FROM " . $GLOBALS['xak']->table('order_info') . " WHERE order_id = '$oid'");
    if (empty($stat))
    {
...
elseif ($_REQUEST['act'] == 'rollback')
{
    ...
    $stat = $db->getRow("SELECT * FROM " . $GLOBALS['xak']->table('affiliate_log') . " WHERE log_id = '$logid'");
    if (!empty($stat))
    {
        ...
        $sql = "UPDATE " . $GLOBALS['xak']->table('affiliate_log') .
...
elseif ($_REQUEST['act'] == 'separate')
{
    ...
    $row = $db->getRow("SELECT o.order_sn, o.is_separate, (o.goods_amount - o.discount) AS goods_amount, o.user_id FROM " . $GLOBALS['xak']->table('order_info') . " o".
                    " LEFT JOIN " . $GLOBALS['xak']->table('users') . " u ON o.user_id = u.user_id".
            " WHERE order_id = '$oid'");
    ...
    if (empty($row['is_separate']))
    {
        ...
        if(empty($separate_by))
        {
            ...
            for ($i=0; $i < $num; $i++)
            {
                ...
                $row = $db->getRow("SELECT o.parent_id as user_id,u.user_name FROM " . $GLOBALS['xak']->table('users') . " o" .
                        " LEFT JOIN" . $GLOBALS['xak']->table('users') . " u ON o.parent_id = u.user_id".
        ...
        {
            //推荐订单分成
            $row = $db->getRow("SELECT o.parent_id, u.user_name FROM " . $GLOBALS['xak']->table('order_info') . " o" .
                    " LEFT JOIN" . $GLOBALS['xak']->table('users') . " u ON o.parent_id = u.user_id".
...
function get_affiliate_ck()
{
    ...
    if(!empty($affiliate['on']))
    {
        ...
        {
            //推荐注册分成
            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['xak']->table('order_info') . " o".
                    " LEFT JOIN".$GLOBALS['xak']->table('users')." u ON o.user_id = u.user_id".
                    " LEFT JOIN " . $GLOBALS['xak']->table('affiliate_log') . " a ON o.order_id = a.order_id" .
    ...
    if(!empty($affiliate['on']))
    {
        ...
        {
            //推荐注册分成
            $sql = "SELECT o.*, a.log_id, a.user_id as suid,  a.user_name as auser, a.money, a.point, a.separate_type,u.parent_id as up FROM " . $GLOBALS['xak']->table('order_info') . " o".
                    " LEFT JOIN".$GLOBALS['xak']->table('users')." u ON o.user_id = u.user_id".
                    " LEFT JOIN " . $GLOBALS['xak']->table('affiliate_log') . " a ON o.order_id = a.order_id" .
                    " WHERE o.user_id > 0 AND (u.parent_id > 0 AND o.is_separate = 0 OR o.is_separate > 0) $sqladd".
...
{
    $time = gmtime();
    $sql = "INSERT INTO " . $GLOBALS['xak']->table('affiliate_log') . "( order_id, user_id, user_name, time, money, point, separate_type)".
