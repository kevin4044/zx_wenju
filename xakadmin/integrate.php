...
if ($_REQUEST['act'] == 'list')
{
    ...
    $allow_set_points = $_CFG['integrate_code'] == 'xakshop' ? 0 : 1;
...
if ($_REQUEST['act'] == 'install')
{
    ...
    if ($_GET['code'] == 'xakshop')
    {
        $sql = "UPDATE " .$xak->table('shop_config'). " SET value = 'xakshop' WHERE code = 'integrate_code'";
        $db->query($sql);
        $sql = "UPDATE " . $GLOBALS['xak']->table('shop_config') . " SET value = '' WHERE code = 'points_rule'";
    ...
    {
        $sql = "UPDATE " . $GLOBALS['xak']->table('users') .
               " SET flag = 0, alias=''".
               " WHERE flag > 0";
...
if ($_REQUEST['act'] == 'check_config')
{
    ...
    {
        $test_str = xak_iconv('UTF8', $_POST['cfg']['db_charset']);
    }
    ...
    $sql = "SELECT COUNT(*) FROM " . $xak->table('users');
    $total = $db->getOne($sql);
    ...
    if ($total == 0)
    {
        ...
        xak_header("Location: integrate.php?act=complete\n");
        exit;
    }
...
if ($_REQUEST['act'] == 'check_user')
{
    ...
    $domain = empty($_GET['domain']) ? '@xakshop' : trim($_GET['domain']);
    if ($size <2)
    {
        $size = 2;
    }
    ...
    $sql = "SELECT user_name FROM " . $xak->table('users') . " LIMIT $start, $size";
    ...
    if ($post_user_list)
    {
        ...
        if ($method == 2)
        {
            $sql = "UPDATE " . $GLOBALS['xak']->table('users') . " SET flag = '$method', alias = CONCAT(user_name, '$domain') WHERE " . db_create_in($post_user_list, 'user_name');
        }
        else
        {
            $sql = "UPDATE " . $GLOBALS['xak']->table('users') . " SET flag = '$method' WHERE " . db_create_in($post_user_list, 'user_name');
        }
        ...
        if ($method == 2 )
        {
            ...
            if ($error_user_list)
            {
                ...
                /* 将用户标记为改名失败 */
                $sql = "UPDATE " . $GLOBALS['xak']->table('users') . " SET flag = '1' WHERE " . db_create_in($error_user_list, 'user_name');
            }

            /* 检查改名后用户是否与商城用户重名 */
            $sql = "SELECT user_name FROM " .$GLOBALS['xak']->table('users') . " WHERE " . db_create_in($test_user_list, 'user_name');
    ...
    else
    {
        ...
        $sql = "SELECT COUNT(*) FROM " . $xak->table('users') . " WHERE flag > 0 ";
        if ($db->getOne($sql) > 0)
        {
...
if ($_REQUEST['act'] == 'import_user')
{
    ...
    $query = $db->query("SELECT * FROM " . $xak->table('users') . " ORDER BY `user_id` ASC");
    ...
    {
        $db->query("UPDATE " . $xak->table($table) . " SET `user_id`=`user_id`+ $maxuid ORDER BY `user_id` DESC");
        foreach ($uc_uid as $uid)
        {
            $db->query("UPDATE " . $xak->table($table) . " SET `user_id`='" . $uid['uid'] . "' WHERE `user_id`='" . ($uid['user_id'] + $maxuid) . "'");
        }
    }
    foreach ($truncate_user_table as $table)
    {
        $db->query("TRUNCATE TABLE " . $xak->table($table));
    }
    // 保存重复的用户信息
...
{
    /* 检查是否有改名失败的用户 */
    $sql = "SELECT COUNT(*) FROM " . $xak->table('users') . " WHERE flag = 1";
    if ($db->getOne($sql) > 0)
    {
        $_REQUEST['flag'] = 1;
...
if ($_REQUEST['act'] == 'act_modify')
{
    ...
    {
        /* 检查改名后用户名是否会重名 */
        $sql = 'SELECT user_name FROM ' . $GLOBALS['xak']->table('users') . ' WHERE ' . db_create_in($alias, 'user_name');
        $xak_error_list = $db->getCol($sql);
        ...
        $error_list = array_unique(array_merge($xak_error_list, $bbs_error_list));
        ...
        if ($error_list)
        {
            ...
            foreach ($_POST['opt'] AS $user_id=>$val)
            {
                if ($val = 2)
                {
                    ...
                    {
                        /* 重名用户，需要标记 */
                        $sql = "UPDATE " . $GLOBALS['xak']->table('users') . " SET flag = 1,  alias='' WHERE user_id = '$user_id'";
                    }
                    else
                    {
                        /* 用户名无重复，可以正常改名 */
    ...
    foreach ($_POST['opt'] as $user_id=>$val)
    {
        if ($val == 3 || $val == 4)
        {
            $sql = "UPDATE " . $GLOBALS['xak']->table('users') . " SET flag='$val' WHERE user_id='$user_id'";
            $db->query($sql);
        }
    }

    /* 跳转  */
    xak_header("Location: integrate.php?act=modify");
    exit;
}
...
{
    $size = 100;
    $total = $db->getOne("SELECT COUNT(*) FROM " . $xak->table("users"));
    $task_del = $db->getOne("SELECT COUNT(*) FROM " . $xak->table("users") . " WHERE flag = 3");
    $task_rename = $db->getOne("SELECT COUNT(*) FROM " . $xak->table("users") . " WHERE flag = 2");
    $task_ignore = $db->getOne("SELECT COUNT(*) FROM " . $xak->table("users") . " WHERE flag = 4");
    ...
    if ($task_del > 0)
    {
        ...
        $sql = "SELECT user_name FROM " . $xak->table('users') . " WHERE flag = 2";
    ...
    if ($task_rename > 0)
    {
        ...
        $sql = "SELECT user_name, alias FROM " . $xak->table('users') . " WHERE flag = 3";
        $rename_list = $db->getAll($sql);
    }

    if ($task_ignore >0)
    {
        $sql = "SELECT user_name FROM " . $xak->table('users') . " WHERE flag = 4";
...
if ($_REQUEST['act'] == 'task')
{
    ...
    if ($_SESSION['task']['del']['start'] < $_SESSION['task']['del']['total'])
    {
        /* 执行操作 */
        /* 查找要删除用户 */
        $arr = $db->getCol("SELECT user_name FROM " . $xak->table('users') . " WHERE flag = 3 LIMIT " . $_SESSION['task']['del']['start'] . ',' . $result['size']);
        $db->query("DELETE FROM " . $xak->table('users') . " WHERE " . db_create_in($arr,'user_name'));
    ...
    {
        /* 查找要改名用户 */
        $arr = $db->getCol("SELECT user_name FROM " . $xak->table('users') . " WHERE flag = 2 LIMIT " . $_SESSION['task']['del']['start'] . ',' . $result['size']);
        $db->query("UPDATE " . $xak->table('users') . " SET user_name=alias, alias='' WHERE " . db_create_in($arr,'user_name'));
    ...
    else if ($_SESSION['task']['sync']['start'] < $_SESSION['task']['sync']['total'])
    {
        ...
        $sql = "SELECT user_name, password, email, sex, birthday, reg_time ".
                "FROM " . $xak->table('users') . " LIMIT " . $_SESSION['task']['del']['start'] . ',' . $result['size'];
        $arr = $db->getAll($sql);
        foreach ($arr as $user)
        {
    ...
    else
    {
        ...
        /* 插入code到shop_config表 */
        $sql = "SELECT COUNT(*) FROM " .$xak->table('shop_config'). " WHERE code = 'integrate_code'";

        if ($db->GetOne($sql) == 0)
        {
            $sql = "INSERT INTO " .$xak->table('shop_config'). " (code, value) ".
                    "VALUES ('integrate_code', '$_SESSION[code]')";
        }
        else
        {
            $sql = "UPDATE " .$xak->table('shop_config'). " SET value = '$_SESSION[code]' WHERE code = 'integrate_code'";
        }
        $db->query($sql);
        ...
        $sql = "UPDATE " . $xak->table('users') . " set flag = 0, alias = '' WHERE flag > 0";
        $db->query($sql);
        die($json->encode($result));
    }
}
...
if ($_REQUEST['act'] == 'setup_ucenter')
{
    ...
    $app_type   = 'XKASHOP';
    $app_name   = $db->getOne('SELECT value FROM ' . $xak->table('shop_config') . " WHERE code = 'shop_name'");
    $app_url    = $GLOBALS['xak']->url();
    $app_charset = XAK_CHARSET;
    $app_dbcharset = strtolower((str_replace('-', '', XAK_CHARSET)));
...
if ($_REQUEST['act'] == 'edit_points')
{
    ...
    $sql = "UPDATE " . $xak->table('shop_config') . " SET value ='" . serialize($rule) . "' WHERE code='points_rule'";

    $db->query($sql);

    clear_cache_files();

    xak_header("Location: integrate.php?act=points_set\n");
    exit;
}
...
if ($_REQUEST['act'] == 'save_points')
{
    ...
    $sql = "SELECT COUNT(*) FROM " . $xak->table('shop_config') . " WHERE code='points_set'";
    if ($db->getOne($sql) == 0)
    {
        $sql = "INSERT INTO " . $xak->table('shop_config') . " (parent_id, type, code, value) VALUES (6, 'hidden', 'points_set', '" . serialize($cfg) . "')";
    }
    else
    {
        $sql = "UPDATE " . $xak->table('shop_config') . " SET value ='" . serialize($cfg) . "' WHERE code='points_set'";
    }
    $db->query($sql);
    clear_cache_files();
    sys_msg($_LANG['save_ok'],0, array(array('text'=>$_LANG['06_list_integrate'],'href'=>'integrate.php?act=list')));
...
function conflict_userlist()
{
    ...
    $sql = "SELECT COUNT(*) FROM " . $GLOBALS['xak']->table('users') . $where;
    ...
    $sql = "SELECT user_id, user_name, email, reg_time, flag, alias ".
            " FROM " . $GLOBALS['xak']->table('users') . $where .
...
{
    $sql = "SELECT COUNT(*) FROM " .$GLOBALS['xak']->table('shop_config'). " WHERE code = 'integrate_code'";
    ...
    {
        $sql = "SELECT value FROM " . $GLOBALS['xak']->table('shop_config') . " WHERE code = 'integrate_code'";
        ...
        $sql = "UPDATE " .$GLOBALS['xak']->table('shop_config'). " SET value = '$code' WHERE code = 'integrate_code'";
    }
    ...
    $sql = "SELECT COUNT(*) FROM " . $GLOBALS['xak']->table('shop_config') . " WHERE code = 'integrate_config'";
    if ($GLOBALS['db']->GetOne($sql) == 0)
    {
        $sql =  "INSERT INTO " . $GLOBALS['xak']->table('shop_config') . " (code, value) ".
                "VALUES ('integrate_config', '" . serialize($cfg) . "')";
    }
    else
    {
        $sql = "UPDATE " . $GLOBALS['xak']->table('shop_config') . " SET value='". serialize($cfg) ."' ".
