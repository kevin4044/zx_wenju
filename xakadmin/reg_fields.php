...
$exc = new exchange($xak->table("reg_fields"), $db, 'id', 'reg_field_name');
...
{
    $fields = array();
    $fields = $db->getAll("SELECT * FROM " . $xak->table('reg_fields') . " ORDER BY dis_order, id");
...
{
    $fields = array();
    $fields = $db->getAll("SELECT * FROM " .$xak->table('reg_fields') . "ORDER BY id");
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "INSERT INTO " .$xak->table('reg_fields') ."( ".
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT id AS reg_field_id, reg_field_name, dis_order AS reg_field_order, display AS reg_field_display, is_need AS reg_field_need FROM ".
           $xak->table('reg_fields'). " WHERE id='$_REQUEST[id]'";
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    $sql = "UPDATE " .$xak->table('reg_fields') . " SET `reg_field_name` = '$_POST[reg_field_name]', `dis_order` = '$_POST[reg_field_order]', `display` = '$_POST[reg_field_display]', `is_need` = '$_POST[reg_field_need]' WHERE `id` = '$_POST[id]'";
    $db->query($sql);
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    {
        /* 删除会员扩展信息表的相应信息 */
        $sql = "DELETE FROM " . $GLOBALS['xak']->table('reg_extend_info') . " WHERE reg_field_id = '" . $field_id . "'";
