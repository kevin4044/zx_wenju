...
elseif (!empty($_GET['get_goods']))
{
    ...
    $sql = 'SELECT `goods_id`, `goods_name` FROM ' . $xak->table('goods') . ' AS g WHERE 1 ' . $goods_where . ' LIMIT 50';
...
else
{
    ...
    {
        $album_where = ', ' . $xak->table('goods'). " AS g WHERE album.img_original > '' AND album.goods_id = g.goods_id " . $goods_where;
    }
    else
    {
    ...
    if (isset($_GET['start']))
    {
        ...
        {
            $count = $db->GetOne("SELECT COUNT(*) FROM ".$xak->table('goods'). " AS g WHERE g.original_img <> ''" . $goods_where);
            $title = sprintf($_LANG['goods_format'], $count, $page_size);
        }

        if (isset($_GET['total_album']))
        {
            $count = $GLOBALS['db']->GetOne("SELECT COUNT(*) FROM ".$GLOBALS['xak']->table('goods_gallery'). ' AS album ' . $album_where);
    ...
    else
    {
        ...
        {
            $count = $GLOBALS['db']->GetOne("SELECT COUNT(*) FROM ".$xak->table('goods'). " AS g WHERE g.original_img > ''" . $goods_where);
            /* 页数在许可范围内 */
...
function process_image($page = 1, $page_size = 100, $type = 0, $thumb= true, $watermark = true, $change = false, $silent = true)
{
    if ($type == 0)
    {
        $sql = "SELECT g.goods_id, g.original_img, g.goods_img, g.goods_thumb FROM ".$GLOBALS['xak']->table('goods'). " AS g WHERE g.original_img > ''" . $GLOBALS['goods_where'];
        ...
        while ($row = $GLOBALS['db']->fetchRow($res))
        {
            ...
            if ($watermark)
            {
                ...
                if ($change || empty($row['goods_img']))
                {
                    ...
                    {
                        $sql = "UPDATE " .$GLOBALS['xak']->table('goods'). " SET goods_img = '$image' WHERE goods_id = '" . $row['goods_id'] . "'";
            ...
            if ($thumb)
            {
                ...
                if ($change || empty($row['goods_thumb']))
                {
                    ...
                    {
                        $sql = "UPDATE " .$GLOBALS['xak']->table('goods'). " SET goods_thumb = '$goods_thumb' WHERE goods_id = '" . $row['goods_id'] . "'";
                        $GLOBALS['db']->query($sql);
                        /* 防止原图被删除 */
    ...
    {
        /* 遍历商品相册 */
        $sql = "SELECT album.goods_id, album.img_id, album.img_url, album.thumb_url, album.img_original FROM ".$GLOBALS['xak']->table('goods_gallery'). " AS album " . $GLOBALS['album_where'];
        $res = $GLOBALS['db']->SelectLimit($sql, $page_size, ($page - 1) * $page_size);
        ...
        while ($row = $GLOBALS['db']->fetchRow($res))
        {
            ...
            if ($watermark && file_exists(ROOT_PATH . $row['img_original']))
            {
                ...
                if ($change || empty($row['img_url']) || $row['img_original'] == $row['img_url'])
                {
                    ...
                    {
                         $sql = "UPDATE " .$GLOBALS['xak']->table('goods_gallery'). " SET img_url='$image' WHERE img_id='$row[img_id]'";
                         $GLOBALS['db']->query($sql);
            ...
            if ($thumb)
            {
                ...
                if ($change || empty($row['thumb_url']))
                {
                    ...
                    {
                        $sql = "UPDATE " .$GLOBALS['xak']->table('goods_gallery'). " SET thumb_url='$thumb_url' WHERE img_id='$row[img_id]'";
                        $GLOBALS['db']->query($sql);
...
function process_image_ex($page = 1, $page_size = 100, $type = 0, $thumb= true, $watermark = true, $change = false, $silent = true)
{
    if ($type == 0)
    {
        $sql = "SELECT g.goods_id, g.original_img, g.goods_img, g.goods_thumb FROM ".$GLOBALS['xak']->table('goods'). " AS g WHERE g.original_img > ''" . $goods_where;
        $res = $GLOBALS['db']->SelectLimit($sql, $page_size, ($page-1)*$page_size);
