...
/* 初始化$exc对象 */
$exc = new exchange($xak->table('bonus_type'), $db, 'type_id', 'type_name');
...
if ($_REQUEST['act'] == 'remove')
{
    ...
    /* 更新商品信息 */
    $db->query("UPDATE " .$xak->table('goods'). " SET bonus_type_id = 0 WHERE bonus_type_id = '$id'");

    /* 删除用户的红包 */
    $db->query("DELETE FROM " .$xak->table('user_bonus'). " WHERE bonus_type_id = '$id'");
...
if ($_REQUEST['act'] == 'insert')
{
    ...
    /* 检查类型是否有重复 */
    $sql = "SELECT COUNT(*) FROM " .$xak->table('bonus_type'). " WHERE type_name='$type_name'";
    if ($db->getOne($sql) > 0)
    {
    ...
    /* 插入数据库。 */
    $sql = "INSERT INTO ".$xak->table('bonus_type')." (type_name, type_money,send_start_date,send_end_date,use_start_date,use_end_date,send_type,min_amount,min_goods_amount)
    VALUES ('$type_name',
            '$_POST[type_money]',
            '$send_startdate',
            '$send_enddate',
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    $bonus_arr = $db->getRow("SELECT * FROM " .$xak->table('bonus_type'). " WHERE type_id = '$type_id'");
...
if ($_REQUEST['act'] == 'update')
{
    ...
    $sql = "UPDATE " .$xak->table('bonus_type'). " SET ".
           "type_name       = '$type_name', ".
...
if ($_REQUEST['act'] == 'send')
{
    ...
    {
        /* 查询此红包类型信息 */
        $bonus_type = $db->GetRow("SELECT type_id, type_name FROM ".$xak->table('bonus_type').
        ...
        /* 查询其他红包类型的商品 */
        $sql = "SELECT goods_id FROM " .$xak->table('goods').
...
if ($_REQUEST['act'] == 'send_by_user')
{
    ...
    if (isset($_REQUEST['send_rank']))
    {
        ...
        if ($rank_id > 0)
        {
            $sql = "SELECT min_points, max_points, special_rank FROM " . $xak->table('user_rank') . " WHERE rank_id = '$rank_id'";
            $row = $db->getRow($sql);
            if ($row['special_rank'])
            {
                /* 特殊会员组处理 */
                $sql = 'SELECT COUNT(*) FROM ' . $xak->table('users'). " WHERE user_rank = '$rank_id'";
                $send_count = $db->getOne($sql);
                if($validated_email)
                {
                    $sql = 'SELECT user_id, email, user_name FROM ' . $xak->table('users').
    ...
    foreach ($user_list AS $key => $val)
    {
        ...
        {
             /* 向会员红包表录入数据 */
            $sql = "INSERT INTO " . $xak->table('user_bonus') .
...
if ($_REQUEST['act'] == 'send_by_print')
{
    ...
    /* 生成红包序列号 */
    $num = $db->getOne("SELECT MAX(bonus_sn) FROM ". $xak->table('user_bonus'));
    $num = $num ? floor($num / 10000) : 100000;

    for ($i = 0, $j = 0; $i < $bonus_sum; $i++)
    {
        $bonus_sn = ($num + $i) . str_pad(mt_rand(0, 9999), 4, '0', STR_PAD_LEFT);
        $db->query("INSERT INTO ".$xak->table('user_bonus')." (bonus_type_id, bonus_sn) VALUES('$bonus_typeid', '$bonus_sn')");
...
if ($_REQUEST['act'] == 'gen_excel')
{
    ...
    $type_name = $db->getOne("SELECT type_name FROM ".$xak->table('bonus_type')." WHERE type_id = '$tid'");

    /* 文件名称 */
    $bonus_filename = $type_name .'_bonus_list';
    if (XAK_CHARSET != 'gbk')
    {
        $bonus_filename = xak_iconv('UTF8', 'GB2312',$bonus_filename);
    }
    ...
    if (XAK_CHARSET != 'gbk')
    {
        echo xak_iconv('UTF8', 'GB2312', $_LANG['bonus_excel_file']) . "\t\n";
        /* 红包序列号, 红包金额, 类型名称(红包名称), 使用结束日期 */
        echo xak_iconv('UTF8', 'GB2312', $_LANG['bonus_sn']) ."\t";
        echo xak_iconv('UTF8', 'GB2312', $_LANG['type_money']) ."\t";
        echo xak_iconv('UTF8', 'GB2312', $_LANG['type_name']) ."\t";
        echo xak_iconv('UTF8', 'GB2312', $_LANG['use_enddate']) ."\t\n";
    }
    else
    {
    ...
    $sql = "SELECT ub.bonus_id, ub.bonus_type_id, ub.bonus_sn, bt.type_name, bt.type_money, bt.use_end_date ".
           "FROM ".$xak->table('user_bonus')." AS ub, ".$xak->table('bonus_type')." AS bt ".
    ...
    while ($val = $db->fetchRow($res))
    {
        ...
        if (!isset($code_table[$val['type_name']]))
        {
            if (XAK_CHARSET != 'gbk')
            {
                $code_table[$val['type_name']] = xak_iconv('UTF8', 'GB2312', $val['type_name']);
            }
            else
            {
...
if ($_REQUEST['act'] == 'add_bonus_goods')
{
    ...
    {
        $sql = "UPDATE " .$xak->table('goods'). " SET bonus_type_id='$type_id' WHERE goods_id='$val'";
...
if ($_REQUEST['act'] == 'drop_bonus_goods')
{
    ...
    $db->query("UPDATE ".$xak->table('goods')." SET bonus_type_id = 0 ".
...
if ($_REQUEST['act'] == 'search_users')
{
    ...
    $sql = "SELECT user_id, user_name FROM " . $xak->table('users') .
...
elseif ($_REQUEST['act'] == 'remove_bonus')
{
    ...
    $db->query("DELETE FROM " .$xak->table('user_bonus'). " WHERE bonus_id='$id'");
...
if ($_REQUEST['act'] == 'batch')
{
    ...
    if (isset($_POST['checkboxes']))
    {
        ...
        if (isset($_POST['drop']))
        {
            $sql = "DELETE FROM " . $xak->table('user_bonus'). " WHERE bonus_id " . db_create_in($bonus_id_list);
            $db->query($sql);
...
function get_type_list()
{
    ...
    $sql = "SELECT bonus_type_id, COUNT(*) AS sent_count".
            " FROM " .$GLOBALS['xak']->table('user_bonus') .
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT COUNT(*) FROM ".$GLOBALS['xak']->table('bonus_type');
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);

        /* 分页大小 */
        $filter = page_and_size($filter);

        $sql = "SELECT * FROM " .$GLOBALS['xak']->table('bonus_type'). " ORDER BY $filter[sort_by] $filter[sort_order]";
...
{
    $sql = "SELECT goods_id, goods_name FROM " .$GLOBALS['xak']->table('goods').
...
function get_bonus_list()
{
    ...
    $sql = "SELECT COUNT(*) FROM ".$GLOBALS['xak']->table('user_bonus'). $where;
    $filter['record_count'] = $GLOBALS['db']->getOne($sql);
    ...
    $sql = "SELECT ub.*, u.user_name, u.email, o.order_sn, bt.type_name ".
          ...
          " LEFT JOIN " .$GLOBALS['xak']->table('bonus_type'). " AS bt ON bt.type_id=ub.bonus_type_id ".
          " LEFT JOIN " .$GLOBALS['xak']->table('users'). " AS u ON u.user_id=ub.user_id ".
          " LEFT JOIN " .$GLOBALS['xak']->table('order_info'). " AS o ON o.order_id=ub.order_id $where ".
...
{
    $sql = "SELECT * FROM " . $GLOBALS['xak']->table('bonus_type') .
...
function send_bonus_mail($bonus_type_id, $bonus_id_list)
{
    ...
    $sql = "SELECT b.bonus_id, u.user_name, u.email " .
            "FROM " . $GLOBALS['xak']->table('user_bonus') . " AS b, " .
...
function add_to_maillist($username, $email, $subject, $content, $is_html)
{
    ...
    $template_id = $GLOBALS['db']->getOne("SELECT template_id FROM " . $GLOBALS['xak']->table('mail_templates') . " WHERE template_code = 'send_bonus'");
    $sql = "INSERT INTO "  . $GLOBALS['xak']->table('email_sendlist') . " ( email, template_id, email_content, pri, last_send) VALUES ('$email', $template_id, '$content', 1, '$time')";
    $GLOBALS['db']->query($sql);
    return true;
}
