...
{
    $sql = "SELECT email FROM " . $xak->table('email_list') . "WHERE stat = 1";
...
elseif ($_REQUEST['act'] == 'batch_remove')
{
    ...
    $sql = "DELETE FROM " . $xak->table('email_list') .
...
elseif ($_REQUEST['act'] == 'batch_unremove')
{
    ...
    $sql = "UPDATE " . $xak->table('email_list') .
...
function get_email_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['xak']->table('email_list');
        ...
        $sql = "SELECT * FROM " . $GLOBALS['xak']->table('email_list') .
