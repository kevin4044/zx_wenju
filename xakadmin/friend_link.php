...
$exc = new exchange($xak->table('friend_link'), $db, 'link_id', 'link_name');
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    if ($exc->num("link_name", $link_name) == 0)
    {
        ...
        /* 插入数据 */
        $sql    = "INSERT INTO ".$xak->table('friend_link')." (link_name, link_url, link_logo, show_order) ".
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT link_id, link_name, link_url, link_logo, show_order ".
           "FROM " .$xak->table('friend_link'). " WHERE link_id = '".intval($_REQUEST['id'])."'";
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    {
        //获取链子LOGO,并删除
        $old_logo = $db->getOne("SELECT link_logo FROM " .$xak->table('friend_link'). " WHERE link_id = '$id'");
    ...
    /* 更新信息 */
    $sql = "UPDATE " .$xak->table('friend_link'). " SET ".
            "link_name = '$link_name', ".
            "link_url = '$link_url' ".
            $link_logo.','.
...
function get_links_list()
{
    ...
    if ($result === false)
    {
        ...
        /* 获得总记录数据 */
        $sql = 'SELECT COUNT(*) FROM ' .$GLOBALS['xak']->table('friend_link');
        ...
        /* 获取数据 */
        $sql  = 'SELECT link_id, link_name, link_url, link_logo, show_order'.
               ' FROM ' .$GLOBALS['xak']->table('friend_link').
