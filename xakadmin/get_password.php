...
if ($_SERVER['REQUEST_METHOD'] == 'GET')
{
    ...
    if (!empty($_GET['act']) && $_GET['act'] == 'reset_pwd')
    {
        ...
        /* 以用户的原密码，与code的值匹配 */
        $sql = 'SELECT password FROM ' .$xak->table('admin_user'). " WHERE user_id = '$adminid'";
...
else
{
    ...
    if (!empty($_POST['action']) && $_POST['action'] == 'get_pwd')
    {
        ...
        $sql = 'SELECT user_id, password FROM ' .$xak->table('admin_user').
        ...
        if (!empty($admin_info))
        {
            ...
            $reset_email = $xak->url() . ADMIN_PATH . '/get_password.php?act=reset_pwd&uid='.$admin_id.'&code='.$code;
    ...
    elseif (!empty($_POST['action']) && $_POST['action'] == 'reset_pwd')
    {
        ...
        //更新管理员的密码
		$ec_salt=rand(1,9999);
        $sql = "UPDATE " .$xak->table('admin_user'). "SET password = '".md5(md5($new_password).$ec_salt)."',`ec_salt`='$ec_salt' ".
