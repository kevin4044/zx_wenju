...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    elseif ($_POST['step'] == 2)
    {
        ...
        $time = gmtime();
        $sql = "INSERT INTO " . $xak->table('mail_templates') . " (template_code, is_html,template_subject, template_content, last_modify, type) VALUES('" . md5($magazine_name.$time) . "',1, '$magazine_name', '$magazine_content', '$time', 'magazine')";
        $db->query($sql);
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    if (empty($_POST['step']))
    {
        $rt = $db->getRow("SELECT * FROM "  . $xak->table('mail_templates') . " WHERE type = 'magazine' AND template_id = '$id'");
    ...
    elseif ($_POST['step'] == 2)
    {
        ...
        $magazine_content = str_replace('src=\"','src=\"http://'.$_SERVER['HTTP_HOST'],$magazine_content);
        $time = gmtime();
        $db->query("UPDATE " . $xak->table('mail_templates') . " SET is_html = 1, template_subject = '$magazine_name', template_content = '$magazine_content', last_modify = '$time' WHERE type = 'magazine' AND template_id = '$id'");
        $links[] = array('text' => $_LANG['magazine_list'], 'href' => 'magazine_list.php?act=list');
        sys_msg($_LANG['edit_ok'], 0, $links);
    }
}
...
{
    $id = intval($_REQUEST['id']);
    $db->query("DELETE  FROM "  . $xak->table('mail_templates') . " WHERE type = 'magazine' AND template_id = '$id' LIMIT 1");
    $links[] = array('text' => $_LANG['magazine_list'], 'href' => 'magazine_list.php?act=list');
    sys_msg($_LANG['edit_ok'], 0, $links);
}
elseif ($_REQUEST['act'] == 'addtolist')
{
    ...
    $template_id = $db->getOne("SELECT template_id FROM "  . $xak->table('mail_templates') . " WHERE type = 'magazine' AND template_id = '$id'");
    if (!empty($template_id))
    {
        if (SEND_LIST == $rank_array['0'])
        {
            $count = $db->getOne("SELECT COUNT(*) FROM " . $xak->table('email_list') . "WHERE stat = 1");
            if ($count > $start)
            {
                $sql = "SELECT email FROM " . $xak->table('email_list') . "WHERE stat = 1 LIMIT $start,100";
                $query = $db->query($sql);
                $add = '';
                ...
                if ($add)
                {
                    $sql = "INSERT INTO "  . $xak->table('email_sendlist') . " (email,template_id,pri,last_send) VALUES " . $add;
                    $db->query($sql);
                }
                if($i == 100)
                {
            ...
            {
                $db->query("UPDATE "  . $xak->table('mail_templates') . " SET last_send = " . time() . " WHERE type = 'magazine' AND template_id = '$id'");
                $links[] = array('text' => $_LANG['magazine_list'], 'href' => 'magazine_list.php?act=list');
                sys_msg($_LANG['edit_ok'], 0, $links);
            }
        }
        ...
        {
            $sql = "SELECT special_rank FROM " . $xak->table('user_rank') . " WHERE rank_id = '" . $rank_array['1'] . "'";
            $row = $db->getRow($sql);
            if (SEND_USER == $rank_array['0'])
            {
                $count_sql = 'SELECT COUNT(*) FROM ' . $xak->table('users') . 'WHERE is_validated = 1';
                $email_sql = 'SELECT email FROM ' . $xak->table('users') . "WHERE is_validated = 1 LIMIT $start,100";
            }
            elseif ($row['special_rank'])
            {
                $count_sql = 'SELECT COUNT(*) FROM ' . $xak->table('users') . 'WHERE is_validated = 1 AND user_rank = ' . $rank_array['1'];
                $email_sql = 'SELECT email FROM ' . $xak->table('users') . 'WHERE is_validated = 1 AND user_rank = ' . $rank_array['1'] . " LIMIT $start,100";
            }
            else
            {

                $count_sql = 'SELECT COUNT(*) ' .
                             'FROM ' . $xak->table('users') . ' AS u LEFT JOIN ' . $xak->table('user_rank') . ' AS ur ' .
...
function get_magazine()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT count(*) FROM " .$GLOBALS['xak']->table('mail_templates') . " WHERE type = 'magazine'";
        ...
        /* 查询 */
        $sql = "SELECT * ".
               " FROM ".$GLOBALS['xak']->table('mail_templates') .
               " WHERE type = 'magazine'" .
