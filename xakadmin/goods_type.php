...
$exc = new exchange($xak->table("goods_type"), $db, 'cat_id', 'cat_name');
...
if ($_REQUEST['act'] == 'manage')
{
    ...
    $query = $db->query("SELECT a.cat_id FROM " . $xak->table('attribute') . " AS a RIGHT JOIN " . $xak->table('goods_attr') . " AS g ON g.attr_id = a.attr_id GROUP BY a.cat_id");
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    if ($db->autoExecute($xak->table('goods_type'), $goods_type) !== false)
    {
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    if ($db->autoExecute($xak->table('goods_type'), $goods_type, 'UPDATE', "cat_id='$cat_id'") !== false)
    {
        /* 对比原来的分组 */
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    if ($exc->drop($id))
    {
        ...
        /* 清除该类型下的所有属性 */
        $sql = "SELECT attr_id FROM " .$xak->table('attribute'). " WHERE cat_id = '$id'";
        $arr = $db->getCol($sql);

        $GLOBALS['db']->query("DELETE FROM " .$xak->table('attribute'). " WHERE attr_id " . db_create_in($arr));
        $GLOBALS['db']->query("DELETE FROM " .$xak->table('goods_attr'). " WHERE attr_id " . db_create_in($arr));
...
function get_goodstype()
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数以及页数 */
        $sql = "SELECT COUNT(*) FROM ".$GLOBALS['xak']->table('goods_type');
        ...
        $sql = "SELECT t.*, COUNT(a.cat_id) AS attr_count ".
               "FROM ". $GLOBALS['xak']->table('goods_type'). " AS t ".
               "LEFT JOIN ". $GLOBALS['xak']->table('attribute'). " AS a ON a.cat_id=t.cat_id ".
               "GROUP BY t.cat_id " .
...
{
    $sql = "SELECT * FROM " .$GLOBALS['xak']->table('goods_type'). " WHERE cat_id='$cat_id'";
...
{
    $sql = "UPDATE " . $GLOBALS['xak']->table('attribute') .
