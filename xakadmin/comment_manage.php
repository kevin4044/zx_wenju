...
if ($_REQUEST['act']=='reply')
{
    ...
    /* 获取评论详细信息并进行字符处理 */
    $sql = "SELECT * FROM " .$xak->table('comment'). " WHERE comment_id = '$_REQUEST[id]'";
    ...
    /* 获得评论回复内容 */
    $sql = "SELECT * FROM ".$xak->table('comment'). " WHERE parent_id = '$_REQUEST[id]'";
    ...
    /* 获取管理员的用户名和Email地址 */
    $sql = "SELECT user_name, email FROM ". $xak->table('admin_user').
    ...
    {
        $sql = "SELECT goods_name FROM ".$xak->table('goods').
               " WHERE goods_id = '$comment_info[id_value]'";
        $id_value = $db->getOne($sql);
    }
    else
    {
        $sql = "SELECT title FROM ".$xak->table('article').
...
if ($_REQUEST['act']=='action')
{
    ...
    /* 获得评论是否有回复 */
    $sql = "SELECT comment_id, content, parent_id FROM ".$xak->table('comment').
    ...
    {
        /* 更新回复的内容 */
        $sql = "UPDATE ".$xak->table('comment')." SET ".
               "email     = '$_POST[email]', ".
    ...
    {
        /* 插入回复的评论内容 */
        $sql = "INSERT INTO ".$xak->table('comment')." (comment_type, id_value, email, user_name , ".
    ...
    $sql = "UPDATE " .$xak->table('comment'). " SET status = 1 WHERE comment_id = '$_POST[comment_id]'";
    $db->query($sql);
    ...
    xak_header("Location: comment_manage.php?act=reply&id=$_REQUEST[comment_id]&send_ok=$send_ok\n");
    exit;
}
/*------------------------------------------------------ */
//-- 更新评论的状态为显示或者禁止
...
if ($_REQUEST['act'] == 'check')
{
    ...
    {
        /* 允许评论显示 */
        $sql = "UPDATE " .$xak->table('comment'). " SET status = 1 WHERE comment_id = '$_REQUEST[id]'";
        $db->query($sql);
        ...
        xak_header("Location: comment_manage.php?act=reply&id=$_REQUEST[id]\n");
        exit;
    }
    else
    {
        /* 禁止评论显示 */
        $sql = "UPDATE " .$xak->table('comment'). " SET status = 0 WHERE comment_id = '$_REQUEST[id]'";
        $db->query($sql);
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $sql = "DELETE FROM " .$xak->table('comment'). " WHERE comment_id = '$id'";
    $res = $db->query($sql);
    if ($res)
    {
        $db->query("DELETE FROM " .$xak->table('comment'). " WHERE parent_id = '$id'");
    }
...
if ($_REQUEST['act'] == 'batch')
{
    ...
    if (isset($_POST['checkboxes']))
    {
        ...
        {
            case 'remove':
                $db->query("DELETE FROM " . $xak->table('comment') . " WHERE " . db_create_in($_POST['checkboxes'], 'comment_id'));
                $db->query("DELETE FROM " . $xak->table('comment') . " WHERE " . db_create_in($_POST['checkboxes'], 'parent_id'));
                break;

           case 'allow' :
               $db->query("UPDATE " . $xak->table('comment') . " SET status = 1  WHERE " . db_create_in($_POST['checkboxes'], 'comment_id'));
               break;

           case 'deny' :
               $db->query("UPDATE " . $xak->table('comment') . " SET status = 0  WHERE " . db_create_in($_POST['checkboxes'], 'comment_id'));
               break;
...
function get_comment_list()
{
    ...
    $sql = "SELECT count(*) FROM " .$GLOBALS['xak']->table('comment'). " WHERE parent_id = 0 $where";
    ...
    /* 获取评论数据 */
    $arr = array();
    $sql  = "SELECT * FROM " .$GLOBALS['xak']->table('comment'). " WHERE parent_id = 0 $where " .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        $sql = ($row['comment_type'] == 0) ?
            "SELECT goods_name FROM " .$GLOBALS['xak']->table('goods'). " WHERE goods_id='$row[id_value]'" :
            "SELECT title FROM ".$GLOBALS['xak']->table('article'). " WHERE article_id='$row[id_value]'";
        $row['title'] = $GLOBALS['db']->getOne($sql);

        /* 标记是否回复过 */
//        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['xak']->table('comment'). " WHERE parent_id = '$row[comment_id]'";
