...
elseif ($_REQUEST['act'] == 'act_export_taobao')
{
    ...
    {
        $sql = "SELECT region_id,  region_name FROM " . $xak->table('region') . " WHERE region_id IN ('$_CFG[shop_province]',  '$_CFG[shop_city]')";
        $arr = $db->getAll($sql);
    ...
    " FROM " . $xak->table('goods') . " AS g ". $where;
    ...
    {
        $content = xak_iconv(XAK_CHARSET, 'utf-8', $content);
    }
...
elseif($_REQUEST['act'] == 'act_export_xakshop')
{
    /* 检查权限 */
    admin_priv('goods_export');
    ...
    $sql = "SELECT g.*, b.brand_name as brandname " .
           " FROM " . $xak->table('goods') . " AS g LEFT JOIN " . $xak->table('brand') . " AS b " .
    ...
    $zip->add_file(xak_iconv(XAK_CHARSET, $charset, $content), 'goods_list.csv');
...
elseif ($_REQUEST['act'] == 'act_export_paipai')
{
    ...
    if (XAK_CHARSET == 'utf-8')
    {
        $zip->add_file(xak_iconv('UTF8', 'GB2312', $content), 'goods_list.csv');
    }
    else
    {
...
elseif ($_REQUEST['act'] == 'act_export_custom')
{
    ...
    while ($row = $db->fetchRow($res))
    {
        ...
        $sql = "SELECT `attr_id`, `attr_value` FROM " . $xak->table('goods_attr') . " WHERE `goods_id` = '" . $row['goods_id'] . "'";
        $query = $db->query($sql);
...
{
    $sql = "SELECT `attr_id`, `cat_id`, `attr_name` FROM " . $GLOBALS['xak']->table('attribute') . " ";
    if (!empty($cat_id))
    {
...
function set_goods_field_name($array, $lang)
{
    ...
    foreach ($array as $key => $value)
    {
        ...
        {
            $tmp_fields[$key] = $GLOBALS['db']->getOne("SELECT `attr_name` FROM " . $GLOBALS['xak']->table('attribute') . " WHERE `attr_id` = '" . intval($value) . "'");
        }
    }
    return $tmp_fields;
}
