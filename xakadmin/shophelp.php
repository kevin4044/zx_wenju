...
/*初始化数据交换对象 */
$exc_article = new exchange($xak->table("article"), $db, 'article_id', 'title');
$exc_cat     = new exchange($xak->table("article_cat"), $db, 'cat_id', 'cat_name');
...
if ($_REQUEST['act'] == 'insert')
{
    ...
    /* 插入数据 */
    $add_time = gmtime();
    $sql = "INSERT INTO ".$xak->table('article')."(title, cat_id, article_type, content, add_time, author) VALUES('$_POST[title]', '$_POST[cat_id]', '$_POST[article_type]','$_POST[FCKeditor1]','$add_time', '_SHOPHELP' )";
    $db ->query($sql);
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    /* 取文章数据 */
    $sql = "SELECT article_id,title, cat_id, article_type, is_open, author, author_email, keywords, content FROM " .$xak->table('article'). " WHERE article_id='$_REQUEST[id]'";
...
elseif ($_REQUEST['act'] == 'remove_art')
{
    ...
    $id     = intval($_GET['id']);
    $cat_id = $db->getOne('SELECT cat_id FROM ' .$xak->table('article'). " WHERE article_id='$id'");
...
elseif ($_REQUEST['act'] == 'add_catname')
{
    ...
    if (!empty($cat_name))
    {
        ...
        {
            $sql = "INSERT INTO " .$xak->table('article_cat'). " (cat_name, cat_type) VALUES ('$cat_name', 0)";
            $db->query($sql);

            admin_log($cat_name, 'add', 'shophelpcat');

            xak_header("Location: shophelp.php?act=query\n");
            exit;
        }
    }
    else
    ...
    xak_header("Location: shophelp.php?act=list_cat\n");
    exit;
}
...
function get_shophelp_list()
{
    ...
    $sql = 'SELECT cat_id, cat_name, sort_order'.
           ' FROM ' .$GLOBALS['xak']->table('article_cat').
           ' WHERE cat_type = 0 ORDER BY sort_order';
    $res = $GLOBALS['db']->query($sql);
    while ($rows = $GLOBALS['db']->fetchRow($res))
    {
        $sql = 'SELECT COUNT(*) FROM ' .$GLOBALS['xak']->table('article'). " WHERE cat_id = '$rows[cat_id]'";
