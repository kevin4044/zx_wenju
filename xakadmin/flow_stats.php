...
if ($_REQUEST['act'] == 'view')
{
    ...
    if(!$is_multi)
    {
        ...
        $sql = "SELECT COUNT(*) AS access_count, area FROM " . $xak->table('stats') .
                " WHERE access_time >= '$start_date' AND access_time < " .($end_date + 86400).
    ...
    else
    {
        ...
        $sql = "SELECT access_time, area FROM " . $xak->table('stats') .
                " WHERE $where";
        $res = $db->query($sql);
        $area_arr = array();
        while ($val = $db->fetchRow($res))
    ...
    if(!$is_multi)
    {
        ...
        $sql = "SELECT COUNT(*) AS access_count, referer_domain FROM " . $xak->table('stats') .
                " WHERE access_time >= '$start_date' AND access_time <= " .($end_date + 86400).
    ...
    else
    {
        ...
        $sql = "SELECT access_time, referer_domain FROM " . $xak->table('stats') .
                " WHERE $where";
...
elseif ($act = 'download')
{
    ...
    $sql = "SELECT COUNT(*) AS access_count, area FROM " . $GLOBALS['xak']->table('stats') .
            " WHERE access_time >= '$start_date' AND access_time <= " .($end_date + 86400).
            " GROUP BY area ORDER BY access_count DESC LIMIT 20";
    ...
    $sql = "SELECT COUNT(*) AS access_count, referer_domain FROM " . $GLOBALS['xak']->table('stats') .
            " WHERE access_time >= '$start_date' AND access_time <= " .($end_date + 86400).
            " GROUP BY referer_domain ORDER BY access_count DESC LIMIT 20";
    ...
    if (XAK_CHARSET != 'gbk')
    {
        echo xak_iconv(XAK_CHARSET, 'gbk', $data) . "\t";
    }
    else
    {
        echo $data. "\t";
