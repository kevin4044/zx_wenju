...
elseif ($_REQUEST['act'] == 'addtolist')
{
    ...
    $sql = "SELECT count(*) FROM " . $GLOBALS['xak']->table('goods') . " g"
        ." LEFT JOIN " . $GLOBALS['xak']->table('collect_goods') . " c"
            . " ON g.goods_id = c.goods_id"
        ." LEFT JOIN " . $GLOBALS['xak']->table('users') . " u"
    ...
    if ($count > $start)
    {
        $sql = "SELECT u.user_name, u.email, g.goods_name, g.goods_id FROM " . $GLOBALS['xak']->table('goods') . " g LEFT JOIN " . $GLOBALS['xak']->table('collect_goods') . " c ON g.goods_id = c.goods_id LEFT JOIN " . $GLOBALS['xak']->table('users') . " u ON c.user_id = u.user_id" .
               " WHERE c.is_attention = 1 AND g.is_delete = 0 AND c.goods_id = '$id' LIMIT $start,100";
        $query = $db->query($sql);
        $add = '';        
        $template = $db->getRow("SELECT * FROM " . $xak->table('mail_templates') . " WHERE  template_code = 'attention_list' AND type = 'template'");

        $i = 0;
        while ($rt = $db->fetch_array($query))
        {
            $time = time();
            $goods_url = $xak->url() . build_uri('goods', array('gid' => $id), $rt['goods_name']);
        ...
        if ($add)
        {
            $sql = "INSERT INTO "  . $xak->table('email_sendlist') . " (email,template_id,email_content,pri,last_send) VALUES " . $add;
            $db->query($sql);
        }
        if($i == 100)
        {
...
elseif ($_REQUEST['act'] == 'batch_addtolist')
{
    ...
    if ($count > $start)
    {
        ...
        while ($rt = $db->fetch_array($query))
        {
            ...
            $goods_url = $xak->url() . build_uri('goods', array('gid' => $rt['goods_id']), $rt['user_name']);
...
function get_attention()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT COUNT(DISTINCT c.goods_id) FROM " . $GLOBALS['xak']->table('collect_goods') . " c " .
                "LEFT JOIN " . $GLOBALS['xak']->table('goods') . " g ON c.goods_id = g.goods_id " .
                    $where;
        ...
        /* 查询 */
        $sql = "SELECT DISTINCT c.goods_id, g.goods_name, g.last_update FROM " . $GLOBALS['xak']->table('collect_goods') . " c " .
                "LEFT JOIN " . $GLOBALS['xak']->table('goods') . " g ON c.goods_id = g.goods_id " .
                    $where .
