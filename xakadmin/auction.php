...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    else
    {
        ...
        {
            /* 查询哪些拍卖活动已经有人出价 */
            $sql = "SELECT DISTINCT act_id FROM " . $xak->table('auction_log') .
                    " WHERE act_id " . db_create_in($ids);
            $ids = array_diff($ids, $db->getCol($sql));
            if (!empty($ids))
            {
                /* 删除记录 */
                $sql = "DELETE FROM " . $xak->table('goods_activity') .
                        " WHERE act_id " . db_create_in($ids) .
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if ($is_add)
    {
        ...
        $db->autoExecute($xak->table('goods_activity'), $auction, 'INSERT');
        $auction['act_id'] = $db->insert_id();
    }
    else
    {
        $db->autoExecute($xak->table('goods_activity'), $auction, 'UPDATE', "act_id = '$auction[act_id]'");
    }
