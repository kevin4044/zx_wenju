...
if ($_REQUEST['act'] == 'list')
{
    ...
    /* 获得支付方式列表 */
    $payment = array();
    $sql = "SELECT pay_id, pay_name FROM ".$xak->table('payment').
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    {
        /* 取得余额信息 */
        $user_account = $db->getRow("SELECT * FROM " .$xak->table('user_account') . " WHERE id = '$id'");

        // 如果是负数，去掉前面的符号
        $user_account['amount'] = str_replace('-', '', $user_account['amount']);

        /* 取得会员名称 */
        $sql = "SELECT user_name FROM " .$xak->table('users'). " WHERE user_id = '$user_account[user_id]'";
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    $user_id = $db->getOne("SELECT user_id FROM " .$xak->table('users'). " WHERE user_name = '$user_name'");
    ...
    if ($_REQUEST['act'] == 'insert')
    {
        ...
        $sql = "INSERT INTO " .$xak->table('user_account').
    ...
    {
        /* 更新数据表 */
        $sql = "UPDATE " .$xak->table('user_account'). " SET ".
    ...
    if ($process_type == 0 && $is_paid == 0)
    {
        ...
        /* 取支付方式信息 */
        $payment_info = array();
        $payment_info = $db->getRow('SELECT * FROM ' . $xak->table('payment').
...
elseif ($_REQUEST['act'] == 'check')
{
    ...
    if ($id == 0)
    {
        xak_header("Location: user_account.php?act=list\n");
        exit;
    }

    /* 查询当前的预付款信息 */
    $account = array();
    $account = $db->getRow("SELECT * FROM " .$xak->table('user_account'). " WHERE id = '$id'");
    ...
    $sql = "SELECT user_name FROM " .$xak->table('users'). " WHERE user_id = '$account[user_id]'";
    $user_name = $db->getOne($sql);
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $sql = "SELECT u.user_name FROM " . $xak->table('users') . " AS u, " .
           $xak->table('user_account') . " AS ua " .
           " WHERE u.user_id = ua.user_id AND ua.id = '$id' ";
    $user_name = $db->getOne($sql);
    $sql = "DELETE FROM " . $xak->table('user_account') . " WHERE id = '$id'";
...
function account_list()
{
    ...
    if ($result === false)
    {
        ...
        if ($filter['keywords'])
        {
            ...
            $sql = "SELECT COUNT(*) FROM " .$GLOBALS['xak']->table('user_account'). " AS ua, ".
        ...
        $sql  = 'SELECT ua.*, u.user_name FROM ' .
            $GLOBALS['xak']->table('user_account'). ' AS ua LEFT JOIN ' .
            $GLOBALS['xak']->table('users'). ' AS u ON ua.user_id = u.user_id'.
