...
else
{
    ...
    $db->query("UPDATE " .$xak->table('shop_config'). " SET VALUE='$config' WHERE code='sitemap'");

    /* 商品分类 */
    $sql = "SELECT cat_id,cat_name FROM " .$xak->table('category'). " ORDER BY parent_id";
    $res = $db->query($sql);
    ...
    /* 文章分类 */
    $sql = "SELECT cat_id,cat_name FROM " .$xak->table('article_cat'). " WHERE cat_type=1";
    $res = $db->query($sql);
    ...
    /* 商品 */
    $sql = "SELECT goods_id, goods_name FROM " .$xak->table('goods'). " WHERE is_delete = 0";
    $res = $db->query($sql);
    ...
    /* 文章 */
    $sql = "SELECT article_id,title,file_url,open_type FROM " .$xak->table('article'). " WHERE is_open=1";
    $res = $db->query($sql);
    ...
    if ($sm->build($sm_file))
    {
        sys_msg(sprintf($_LANG['generate_success'], $xak->url()."sitemaps.xml"));
    }
    else
    {
        ...
        if ($sm->build($sm_file))
        {
            sys_msg(sprintf($_LANG['generate_success'], $xak->url(). DATA_DIR . '/sitemaps.xml'));
        }
        else
        {
