...
if ($_REQUEST['act'] == 'list')
{
    ...
    $crons_enable = $db->getOne("SELECT enable FROM " . $GLOBALS['xak']->table('crons') . " WHERE cron_code='ipdel'");
...
elseif ($_REQUEST['act'] == 'del')
{
    ...
    $sql = "DELETE FROM " . $xak->table('auto_manage') . " WHERE item_id = '$goods_id' AND type = 'article'";
    $db->query($sql);
...
elseif ($_REQUEST['act'] == 'edit_starttime')
{
    ...
    $db->autoReplace($xak->table('auto_manage'), array('item_id' => $id,'type' => 'article',
...
function get_auto_goods()
{
    ...
    $sql = "SELECT COUNT(*) FROM " . $GLOBALS['xak']->table('article') . " g" . $where;
    $filter['record_count'] = $GLOBALS['db']->getOne($sql);
    $goodsdb = array();
    $filter = page_and_size($filter);
    $sql = "SELECT g.*,a.starttime,a.endtime FROM " . $GLOBALS['xak']->table('article') . " g LEFT JOIN " . $GLOBALS['xak']->table('auto_manage') . " a ON g.article_id = a.item_id AND a.type='article'" . $where .
