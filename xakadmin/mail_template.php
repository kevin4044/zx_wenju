...
if ($_REQUEST['act'] == 'list')
{
    ...
    /* 获得所有邮件模板 */
    $sql = "SELECT template_id, template_code FROM " .$xak->table('mail_templates') . " WHERE  type = 'template'";
    $res = $db->query($sql);
    $cur = null;
...
elseif ($_REQUEST['act'] == 'save_template')
{
    ...
    $sql = "UPDATE " .$xak->table('mail_templates'). " SET ".
...
function load_template($temp_id)
{
    $sql = "SELECT template_subject, template_content, is_html ".
            "FROM " .$GLOBALS['xak']->table('mail_templates'). " WHERE template_id='$temp_id'";
