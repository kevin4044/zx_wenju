...
{
    /* 获取管理员列表 */
    $admin_list = $db->getAll('SELECT user_id, user_name FROM ' .$xak->table('admin_user'));
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    {
        /* 获取管理员信息 */
        $result = $db->query('SELECT user_id FROM ' .$xak->table('admin_user') . 'WHERE user_id !=' . $_SESSION['admin_id']);
        while ($rows = $db->FetchRow($result))
        {
            $sql = "INSERT INTO " .$xak->table('admin_message'). " (sender_id, receiver_id, sent_time, " .
    ...
    else
    {
        ...
        {
            $sql = "INSERT INTO " .$xak->table('admin_message'). " (sender_id, receiver_id, ".
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = 'SELECT message_id, receiver_id, title, message'.
           'FROM ' .$xak->table('admin_message'). " WHERE message_id='$id'";
...
elseif ($_REQUEST['act'] == 'update')
{
    /* 获得留言数据*/
    $msg_arr = array();
    $msg_arr = $db->getRow('SELECT * FROM ' .$xak->table('admin_message')." WHERE message_id='$_POST[id]'");

    $sql = "UPDATE " .$xak->table('admin_message'). " SET ".
           "title = '$_POST[title]',".
...
elseif ($_REQUEST['act'] == 'view')
{
    ...
    $sql     = "SELECT a.*, b.user_name ".
               "FROM " .$xak->table('admin_message')." AS a ".
               "LEFT JOIN " .$xak->table('admin_user')." AS b ON b.user_id = a.sender_id ".
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $sql = "UPDATE ".$xak->table('admin_message')." SET deleted=1 ".
            " WHERE message_id=$id AND (sender_id='$_SESSION[admin_id]' OR receiver_id='$_SESSION[admin_id]')";
    $db->query($sql);
...
function get_message_list()
{
    ...
    $sql = "SELECT COUNT(*) FROM ".$GLOBALS['xak']->table('admin_message')." AS a WHERE 1 AND ". $where;
    ...
    $sql = "SELECT a.message_id,a.sender_id,a.receiver_id,a.sent_time,a.read_time,a.deleted,a.title,a.message,b.user_name".
            " FROM ".$GLOBALS['xak']->table('admin_message')." AS a,".$GLOBALS['xak']->table('admin_user')." AS b ".
