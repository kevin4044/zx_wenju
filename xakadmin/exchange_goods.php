...
/*初始化数据交换对象 */
$exc   = new exchange($xak->table("exchange_goods"), $db, 'goods_id', 'exchange_integral');
//$image = new cls_image();
...
if ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "INSERT INTO ".$xak->table('exchange_goods')."(goods_id, exchange_integral, is_exchange, is_hot) ".
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT eg.goods_id, eg.exchange_integral,eg.is_exchange, eg.is_hot, g.goods_name ".
           " FROM " . $xak->table('exchange_goods') . " AS eg ".
           "  LEFT JOIN " . $xak->table('goods') . " AS g ON g.goods_id = eg.goods_id ".
