...
if ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    if (!$isadd)
    {
        $sql = "SELECT * FROM " . $xak->table('topic') . " WHERE topic_id = '$topic_id'";
        $topic = $db->getRow($sql);
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if ($is_insert)
    {
        $sql = "INSERT INTO " . $xak->table('topic') . " (title,start_time,end_time,data,intro,template,css,topic_img,title_pic,base_style, htmls,keywords,description)" .
...
elseif ($_REQUEST["act"] == "delete")
{
    ...
    $sql = "DELETE FROM " . $xak->table('topic') . " WHERE ";
...
function get_topic_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT COUNT(*) FROM ".$GLOBALS['xak']->table('topic');
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);

        /* 分页大小 */
        $filter = page_and_size($filter);

        $sql = "SELECT * FROM " .$GLOBALS['xak']->table('topic'). " ORDER BY $filter[sort_by] $filter[sort_order]";
    ...
    while($topic = $GLOBALS['db']->fetch_array($query)){
        $topic['start_time'] = local_date('Y-m-d',$topic['start_time']);
        $topic['end_time']   = local_date('Y-m-d',$topic['end_time']);
        $topic['url']        = $GLOBALS['xak']->url() . 'topic.php?topic_id=' . $topic['topic_id'];
        $res[] = $topic;
    }
