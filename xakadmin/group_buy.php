...
elseif ($_REQUEST['act'] =='insert_update')
{
    ...
    if (isset($_POST['finish']))
    {
        ...
        /* 结束团购活动，修改结束时间为当前时间 */
        $sql = "UPDATE " . $xak->table('goods_activity') .
                " SET end_time = '" . gmtime() . "' " .
    ...
    elseif (isset($_POST['mail']))
    {
        ...
        while ($order = $db->fetchRow($res))
        {
            ...
            $smarty->assign('shop_url',     $xak->url() . 'user.php?act=order_detail&order_id='.$order['order_id']);
    ...
    else
    {
        ...
        $goods_name = $db->getOne("SELECT goods_name FROM " . $xak->table('goods') . " WHERE goods_id = '$goods_id'");
        ...
        {
            /* update */
            $db->autoExecute($xak->table('goods_activity'), $group_buy, 'UPDATE', "act_id = '$group_buy_id'");
        ...
        {
            /* insert */
            $db->autoExecute($xak->table('goods_activity'), $group_buy, 'INSERT');
...
elseif ($_REQUEST['act'] == 'batch_drop')
{
    if (isset($_POST['checkboxes']))
    {
        ...
        foreach ($_POST['checkboxes'] AS $key => $id)
        {
            ...
            {
                /* 删除团购活动 */
                $sql = "DELETE FROM " . $GLOBALS['xak']->table('goods_activity') .
...
elseif ($_REQUEST['act'] == 'edit_deposit')
{
    ...
    $sql = "SELECT ext_info FROM " . $xak->table('goods_activity') .
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    /* 删除团购活动 */
    $sql = "DELETE FROM " . $xak->table('goods_activity') . " WHERE act_id = '$id' LIMIT 1";
    $db->query($sql);
...
function group_buy_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['xak']->table('goods_activity') .
...
{
    $sql = "SELECT * FROM " . $GLOBALS['xak']->table('goods_activity') .
