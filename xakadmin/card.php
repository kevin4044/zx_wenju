...
$exc = new exchange($xak->table("card"), $db, 'card_id', 'card_name');
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    /*插入数据*/
    $sql = "INSERT INTO ".$xak->table('card')."(card_name, card_fee, free_money, card_desc, card_img)
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT card_id, card_name, card_fee, free_money, card_desc, card_img FROM " .$xak->table('card'). " WHERE card_id='$_REQUEST[id]'";
    $card = $db->GetRow($sql);
...
elseif ($_REQUEST['act'] == 'drop_card_img')
{
    ...
    /* 取得logo名称 */
    $sql = "SELECT card_img FROM " .$xak->table('card'). " WHERE card_id = '$card_id'";
    $img_name = $db->getOne($sql);

    if (!empty($img_name))
    {
        @unlink(ROOT_PATH . DATA_DIR . '/cardimg/' .$img_name);
        $sql = "UPDATE " .$xak->table('card'). " SET card_img = '' WHERE card_id = '$card_id'";
        $db->query($sql);
    }
...
function cards_list()
{
    ...
    if ($result === false)
    {
        ...
        /* 分页大小 */
        $sql = "SELECT count(*) FROM " .$GLOBALS['xak']->table('card');
