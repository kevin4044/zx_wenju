...
/* 初始化 $exc 对象 */
$exc = new exchange($xak->table("admin_user"), $db, 'user_id', 'user_name');
...
{
    /* 清除cookie */
    setcookie('XAKCP[admin_id]',   '', 1);
    setcookie('XAKCP[admin_pass]', '', 1);
...
elseif ($_REQUEST['act'] == 'signin')
{
    ...
    $sql="SELECT `ec_salt` FROM ". $xak->table('admin_user') ."WHERE user_name = '" . $_POST['username']."'";
    ...
    if ($row)
    {
        ...
        if (isset($_POST['remember']))
        {
            ...
            setcookie('XAKCP[admin_id]',   $row['user_id'],                            $time);
            setcookie('XAKCP[admin_pass]', md5($row['password'] . $_CFG['hash_code']), $time);
        }
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    {
        $sql = "SELECT action_list FROM " . $xak->table('role') . " WHERE role_id = '".$_POST['select_role']."'";
        $row = $db->getRow($sql);
        $action_list = $row['action_list'];
        $role_id = $_POST['select_role'];
    }

        $sql = "SELECT nav_list FROM " . $xak->table('admin_user') . " WHERE action_list = 'all'";
        $row = $db->getRow($sql);


    $sql = "INSERT INTO ".$xak->table('admin_user')." (user_name, email, password, add_time, nav_list, action_list, role_id) ".
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    /* 获取管理员信息 */
    $sql = "SELECT user_id, user_name, email, password, agency_id, role_id FROM " .$xak->table('admin_user').
    ...
    {
        $sql = "SELECT agency_name FROM " . $xak->table('agency') . " WHERE agency_id = '$user_info[agency_id]'";
    ...
    /* 获得该管理员的权限 */
    $priv_str = $db->getOne("SELECT action_list FROM " .$xak->table('admin_user'). " WHERE user_id = '$_GET[id]'");
...
elseif ($_REQUEST['act'] == 'update' || $_REQUEST['act'] == 'update_self')
{
    ...
    {
        /* 查询旧密码并与输入的旧密码比较是否相同 */
        $sql = "SELECT password FROM ".$xak->table('admin_user')." WHERE user_id = '$admin_id'";
        $old_password = $db->getOne($sql);
		$sql ="SELECT ec_salt FROM ".$xak->table('admin_user')." WHERE user_id = '$admin_id'";
    ...
    if($pwd_modified)
    {
        $sql = "UPDATE " .$xak->table('admin_user'). " SET ".
...
elseif ($_REQUEST['act'] == 'modif')
{
    ...
    /* 包含插件菜单语言项 */
    $sql = "SELECT code FROM ".$xak->table('plugins');
    $rs = $db->query($sql);
    ...
    $sql = "SELECT user_id, user_name, email, nav_list ".
           "FROM " .$xak->table('admin_user'). " WHERE user_id = '".$_SESSION['admin_id']."'";
    $user_info = $db->getRow($sql);
...
elseif ($_REQUEST['act'] == 'allot')
{
    ...
    /* 获取权限的分组数据 */
    $sql_query = "SELECT action_id, parent_id, action_code,relevance FROM " .$xak->table('admin_action').
                 " WHERE parent_id = 0";
    ...
    /* 按权限组查询底级的权限名称 */
    $sql = "SELECT action_id, parent_id, action_code,relevance FROM " .$xak->table('admin_action').
...
elseif ($_REQUEST['act'] == 'update_allot')
{
    ...
    /* 取得当前管理员用户名 */
    $admin_name = $db->getOne("SELECT user_name FROM " .$xak->table('admin_user'). " WHERE user_id = '$_POST[id]'");

    /* 更新管理员的权限 */
    $act_list = @join(",", $_POST['action_code']);
    $sql = "UPDATE " .$xak->table('admin_user'). " SET action_list = '$act_list', role_id = '' ".
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    /* 获得管理员用户名 */
    $admin_name = $db->getOne('SELECT user_name FROM '.$xak->table('admin_user')." WHERE user_id='$id'");
...
function get_admin_userlist()
{
    ...
    $sql  = 'SELECT user_id, user_name, email, add_time, last_login '.
            'FROM ' .$GLOBALS['xak']->table('admin_user').' ORDER BY user_id DESC';
...
function clear_cart()
{
    ...
    $sql = "SELECT DISTINCT session_id " .
            "FROM " . $GLOBALS['xak']->table('cart') . " AS c, " .
                $GLOBALS['xak']->table('sessions') . " AS s " .
