...
{
    $tables = $db->GetCol("SHOW TABLES LIKE '" . mysql_like_quote($xak->prefix) . "%'");
...
if ($_REQUEST['act'] == 'restore')
{
    ...
    else
    {
        ...
        foreach ($real_list AS $file)
        {
            ...
            $list[] = array('name' => $file, 'ver' => $info['xak_ver'], 'add_time' => $info['date'], 'vol' => $info['vol'], 'file_size' => num_bitunit($file_size), 'mark' => $mark);
        }
    }
...
if ($_REQUEST['act'] == 'dumpsql')
{
    ...
    {
        case 'full':
            $except = array($xak->prefix.'sessions', $xak->prefix.'sessions_data');
            $temp = $db->GetCol("SHOW TABLES LIKE '" . mysql_like_quote($xak->prefix) . "%'");
...
if ($_REQUEST['act'] == 'import')
{
    ...
    if (preg_match('/_[0-9]+\.sql$/', $file_name))
    {
        ...
        {
            $info = cls_sql_dump::get_head($path . $file);
            if ($info['xak_ver'] != VERSION )
            {
                sys_msg(sprintf($_LANG['version_error'], VERSION, $sql_info['xak_ver']));
            }
...
if ($_REQUEST['act'] == 'upload_sql')
{
    ...
    if (empty($sql_info['xak_ver']))
    {
        sys_msg($_LANG['unrecognize_version'], 1);
    }
    else
    {
        if ($sql_info['xak_ver']!= VERSION)
        {
            sys_msg(sprintf($_LANG['version_error'], VERSION, $sql_info['xak_ver']));
        }
    }
...
if ($_REQUEST['act'] == 'optimize')
{
    ...
    $db_ver = $db_ver_arr;
    $ret = $db ->query("SHOW TABLE STATUS LIKE '" . mysql_like_quote($xak->prefix) . "%'");
...
function sql_import($sql_file)
{
    ...
    if ($db_ver > '4.1')
    {
        for($i = 0; $i < $ret_count; $i++)
        {
            ...
            if (!empty($ret[$i]))
            {
                if ((strpos($ret[$i], 'CREATE TABLE') !== false) && (strpos($ret[$i], 'DEFAULT CHARSET='. str_replace('-', '', XAK_CHARSET) )=== false))
                {
                    /* 建表时缺 DEFAULT CHARSET=utf8 */
                    $ret[$i] = $ret[$i] . 'DEFAULT CHARSET='. str_replace('-', '', XAK_CHARSET);
                }
    ...
    else
    {
        for($i = 0; $i < $ret_count; $i++)
        {
            ...
            if ((strpos($ret[$i], 'CREATE TABLE') !== false) && (strpos($ret[$i], 'DEFAULT CHARSET='. str_replace('-', '', XAK_CHARSET) )!== false))
            {
                $ret[$i] = str_replace('DEFAULT CHARSET='. str_replace('-', '', XAK_CHARSET), '', $ret[$i]);
            }
            if (!empty($ret[$i]))
            {
                $GLOBALS['db']->query($ret[$i]);
