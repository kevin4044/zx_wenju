...
$exc = new exchange($xak->table('shipping'), $db, 'shipping_code', 'shipping_name');
...
if ($_REQUEST['act'] == 'list')
{
    ...
    for ($i = 0; $i < count($modules); $i++)
    {
        ...
        /* 检查该插件是否已经安装 */
        $sql = "SELECT shipping_id, shipping_name, shipping_desc, insure, support_cod,shipping_order FROM " .$xak->table('shipping'). " WHERE shipping_code='" .$modules[$i]['code']. "' ORDER BY shipping_order";
        $row = $db->GetRow($sql);
...
elseif ($_REQUEST['act'] == 'install')
{
    ...
    /* 检查该配送方式是否已经安装 */
    $sql = "SELECT shipping_id FROM " .$xak->table('shipping'). " WHERE shipping_code = '$_GET[code]'";
    $id = $db->GetOne($sql);

    if ($id > 0)
    {
        /* 该配送方式已经安装过, 将该配送方式的状态设置为 enable */
        $db->query("UPDATE " .$xak->table('shipping'). " SET enabled = 1 WHERE shipping_code = '$_GET[code]' LIMIT 1");
    }
    else
    {
...
elseif ($_REQUEST['act'] == 'uninstall')
{
    ...
    /* 获得该配送方式的ID */
    $row = $db->GetRow("SELECT shipping_id, shipping_name, print_bg FROM " .$xak->table('shipping'). " WHERE shipping_code='$_GET[code]'");
    ...
    if ($row)
    {
        $all = $db->getCol("SELECT shipping_area_id FROM " .$xak->table('shipping_area'). " WHERE shipping_id='$shipping_id'");
        $in  = db_create_in(join(',', $all));

        $db->query("DELETE FROM " .$xak->table('area_region'). " WHERE shipping_area_id $in");
        $db->query("DELETE FROM " .$xak->table('shipping_area'). " WHERE shipping_id='$shipping_id'");
        $db->query("DELETE FROM " .$xak->table('shipping'). " WHERE shipping_id='$shipping_id'");
...
elseif ($_REQUEST['act'] == 'print_index')
{
    ...
    /* 检查该插件是否已经安装 取值 */
    $sql = "SELECT * FROM " .$xak->table('shipping'). " WHERE shipping_id = '$shipping_id' LIMIT 0,1";
    $row = $db->GetRow($sql);
    if ($row)
    {
...
elseif ($_REQUEST['act'] == 'recovery_default_template')
{
    ...
    /* 取配送代码 */
    $sql = "SELECT shipping_code FROM " .$xak->table('shipping'). " WHERE shipping_id = '$shipping_id'";
    $code = $db->GetOne($sql);
    ...
    /* 恢复默认 */
    $db->query("UPDATE " .$xak->table('shipping'). " SET print_bg = '" . addslashes($modules[0]['print_bg']) . "',  config_lable = '" . addslashes($modules[0]['config_lable']) . "' WHERE shipping_code = '$code' LIMIT 1");
...
elseif ($_REQUEST['act'] == 'print_upload')
{
    ...
    //保存
    $sql = "UPDATE " .$xak->table('shipping'). " SET print_bg = '$src' WHERE shipping_id = '$shipping_id'";
    $res = $db->query($sql);
    if ($res)
    {
        echo '<script language="javascript">';
...
elseif ($_REQUEST['act'] == 'print_del')
{
    ...
    /* 检查该插件是否已经安装 取值 */
    $sql = "SELECT print_bg FROM " .$xak->table('shipping'). " WHERE shipping_id = '$shipping_id' LIMIT 0,1";
    $row = $db->GetRow($sql);
    if ($row)
    {
        if (($row['print_bg'] != '') && (!is_print_bg_default($row['print_bg'])))
        ...
        $sql = "UPDATE " .$xak->table('shipping'). " SET print_bg = '' WHERE shipping_id = '$shipping_id'";
        $res = $db->query($sql);
    }
    else
    {
...
elseif ($_REQUEST['act'] == 'edit_print_template')
{
    ...
    /* 检查该插件是否已经安装 */
    $sql = "SELECT * FROM " .$xak->table('shipping'). " WHERE shipping_id=$shipping_id";
    $row = $db->GetRow($sql);
    if ($row)
    {
        include_once(ROOT_PATH . 'includes/modules/shipping/' . $row['shipping_code'] . '.php');
...
elseif ($_REQUEST['act'] == 'do_edit_print_template')
{
    ...
    {
        //所见即所得模式
        $db->query("UPDATE " . $xak->table('shipping'). " SET config_lable = '" . $_POST['config_lable'] . "', print_model = '$print_model'  WHERE shipping_id = '$shipping_id'");
    }
    elseif ($print_model == 1)
    {
        ...
        $db->query("UPDATE " . $xak->table('shipping'). " SET shipping_print = '" . $template . "', print_model = '$print_model' WHERE shipping_id = '$shipping_id'");
    }
