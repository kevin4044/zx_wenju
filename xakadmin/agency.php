...
$exc = new exchange($xak->table('agency'), $db, 'agency_id', 'agency_name');
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    {
        $sql = "UPDATE " . $xak->table($value) . " SET agency_id = 0 WHERE agency_id = '$id'";
        $db->query($sql);
    }
...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    else
    {
        ...
        {
            /* 删除记录 */
            $sql = "DELETE FROM " . $xak->table('agency') .
            ...
            foreach ($table_array as $value)
            {
                $sql = "UPDATE " . $xak->table($value) . " SET agency_id = 0 WHERE agency_id " . db_create_in($ids) . " ";
                $db->query($sql);
            }
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    else
    {
        ...
        $id = $_GET['id'];
        $sql = "SELECT * FROM " . $xak->table('agency') . " WHERE agency_id = '$id'";
        ...
        /* 关联的地区 */
        $sql = "SELECT region_id, region_name FROM " . $xak->table('region') .
                " WHERE agency_id = '$id'";
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if ($is_add)
    {
        $db->autoExecute($xak->table('agency'), $agency, 'INSERT');
        $agency['agency_id'] = $db->insert_id();
    }
    else
    {
        $db->autoExecute($xak->table('agency'), $agency, 'UPDATE', "agency_id = '$agency[agency_id]'");
    }

    /* 更新管理员表和地区表 */
    if (!$is_add)
    {
        $sql = "UPDATE " . $xak->table('admin_user') . " SET agency_id = 0 WHERE agency_id = '$agency[agency_id]'";
        $db->query($sql);

        $sql = "UPDATE " . $xak->table('region') . " SET agency_id = 0 WHERE agency_id = '$agency[agency_id]'";
        $db->query($sql);
    }

    if (isset($_POST['admins']))
    {
        $sql = "UPDATE " . $xak->table('admin_user') . " SET agency_id = '$agency[agency_id]' WHERE user_id " . db_create_in($_POST['admins']);
        $db->query($sql);
    }

    if (isset($_POST['regions']))
    {
        $sql = "UPDATE " . $xak->table('region') . " SET agency_id = '$agency[agency_id]' WHERE region_id " . db_create_in($_POST['regions']);
        $db->query($sql);
    }
...
function get_agencylist()
{
    ...
    if ($result === false)
    {
        ...
        /* 查询记录总数，计算分页数 */
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['xak']->table('agency');
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);
        $filter = page_and_size($filter);

        /* 查询记录 */
        $sql = "SELECT * FROM " . $GLOBALS['xak']->table('agency') . " ORDER BY $filter[sort_by] $filter[sort_order]";
