...
/*初始化数据交换对象 */
$exc = new exchange($xak->table("feedback"), $db, 'msg_id', 'msg_title');
...
if ($_REQUEST['act']=='add')
{
    ...
    $order_sn = $db->getOne("SELECT order_sn FROM "  . $xak->table('order_info') . " WHERE order_id = '$order_id'");

    /* 获取关于订单所有信息 */
    $sql = "SELECT msg_id, user_name, msg_title, msg_type, msg_time, msg_content".
           " FROM " . $xak->table('feedback').
...
{
    $sql = "INSERT INTO " . $xak->table('feedback') . "(parent_id, user_id, user_name, user_email, msg_title, msg_type, msg_content, msg_time, message_img, order_id)" .
            " VALUES (0, '$_POST[user_id]', '$_SESSION[admin_name]', ' ', ".
            " '$_POST[msg_title]', 5, '$_POST[msg_content]', '" . gmtime() . "', '', '$_POST[order_id]')";

    $db->query($sql);

    xak_header("Location: user_msg.php?act=add&order_id=$_POST[order_id]&user_id=$_POST[user_id]\n");
    exit;
}
...
if ($_REQUEST['act'] == 'remove_msg')
{
    ...
    $order_id = empty($_GET['order_id']) ? 0 : intval($_GET['order_id']);
    $user_id = empty($_GET['user_id']) ? 0 : intval($_GET['user_id']);
    $sql = "SELECT user_id, order_id, message_img FROM " . $xak->table('feedback') . " WHERE msg_id='$msg_id'";
    $row = $db->getRow($sql);
    if ($row)
    {
        if ($row['user_id'] == $user_id && $row['order_id'] == $order_id)
        {
            ...
            $sql = "DELETE FROM " . $xak->table('feedback') . " WHERE msg_id=$msg_id LIMIT 1";
            $db->query($sql);
        }
    }

    xak_header("Location: user_msg.php?act=add&order_id=$_GET[order_id]&user_id=$_GET[user_id]\n");
    exit;
}
/*------------------------------------------------------ */
//-- 更新留言的状态为显示或者禁止
...
if ($_REQUEST['act'] == 'check')
{
    ...
    {
        /* 允许留言显示 */
        $sql = "UPDATE " .$xak->table('feedback'). " SET msg_status = 1 WHERE msg_id = '$_REQUEST[id]'";
        $db->query($sql);

        /* 清除缓存 */
        clear_cache_files();

        xak_header("Location: user_msg.php?act=view&id=$_REQUEST[id]\n");
        exit;
    }
    else
    {
        /* 禁止留言显示 */
        $sql = "UPDATE " .$xak->table('feedback'). " SET msg_status = 0 WHERE msg_id = '$_REQUEST[id]'";
        $db->query($sql);
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    if ($exc->drop($msg_id))
    {
        ...
        $sql = "DELETE FROM " . $xak->table('feedback') . " WHERE parent_id = '$msg_id' LIMIT 1";
        $db->query($sql, 'SILENT');
...
if ($_REQUEST['act'] == 'batch')
{
    ...
    if (isset($_POST['checkboxes']))
    {
        ...
        {
           case 'remove':
                $db->query("DELETE FROM " . $xak->table('feedback') . " WHERE " . db_create_in($_POST['checkboxes'], 'msg_id'));
                $db->query("DELETE FROM " . $xak->table('feedback') . " WHERE " . db_create_in($_POST['checkboxes'], 'parent_id'));
                break;

           case 'allow' :
               $db->query("UPDATE " . $xak->table('feedback') . " SET msg_status = 1  WHERE " . db_create_in($_POST['checkboxes'], 'msg_id'));
               break;

           case 'deny' :
               $db->query("UPDATE " . $xak->table('feedback') . " SET msg_status = 0,msg_area =1  WHERE " . db_create_in($_POST['checkboxes'], 'msg_id'));
               break;
...
elseif ($_REQUEST['act']=='action')
{
    ...
    {
        $sql = "INSERT INTO ".$xak->table('feedback')." (msg_title, msg_time, user_id, user_name , ".
    ...
    {
        $sql = "UPDATE ".$xak->table('feedback')." SET user_email = '".$_POST['user_email']."', msg_content='".$_POST['msg_content']."', msg_time = '".gmtime()."' WHERE msg_id = '".$_REQUEST['parent_id']."'";
        $db->query($sql);
    }
    ...
    if (!empty($_POST['send_email_notice']) or isset($_POST['remail']))
    {
        ...
        $smarty->assign('shop_name',   "<a href='".$xak->url()."'>" . $_CFG['shop_name'] . '</a>');
    ...
    xak_header("Location: ?act=view&id=".$_REQUEST['msg_id']."&send_ok=$send_ok\n");
    exit;
...
elseif ($_REQUEST['act'] == 'drop_file')
{
    ...
    /* 更新数据库 */
    $db->query("UPDATE ".$xak->table('feedback')." SET message_img = '' WHERE msg_id = '$_GET[id]'");

    xak_header("Location: user_msg.php?act=view&amp;id=".$_GET['id']."\n");
    exit;
}
...
function msg_list()
{
    ...
    $sql = "SELECT count(*) FROM " .$GLOBALS['xak']->table('feedback'). " AS f" .
    ...
    $sql = "SELECT f.msg_id, f.user_name, f.msg_title, f.msg_type, f.order_id, f.msg_status, f.msg_time, f.msg_area, COUNT(r.msg_id) AS reply " .
            "FROM " . $GLOBALS['xak']->table('feedback') . " AS f ".
            "LEFT JOIN " . $GLOBALS['xak']->table('feedback') . " AS r ON r.parent_id=f.msg_id ".
    ...
    {   if($value['order_id'] > 0)
        {
            $msg_list[$key]['order_sn'] = $GLOBALS['db']->getOne("SELECT order_sn FROM " . $GLOBALS['xak']->table('order_info') ." WHERE order_id= " .$value['order_id']);
        }
...
function get_feedback_detail($id)
{
    ...
    $sql = "SELECT T1.*, T2.msg_id AS reply_id, T2.user_name  AS reply_name, u.email AS reply_email, ".
            ...
            "FROM ".$xak->table('feedback'). " AS T1 ".
            "LEFT JOIN " .$xak->table('admin_user'). " AS u ON u.user_id='" .$_SESSION['admin_id']. "' ".
            "LEFT JOIN ".$xak->table('feedback'). " AS T2 ON T2.parent_id=T1.msg_id ".
            "WHERE T1.msg_id = '$id'";
    $msg = $db->GetRow($sql);
