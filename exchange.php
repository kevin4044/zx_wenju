...
if ($_REQUEST['act'] == 'list')
{
    ...
    $display = (isset($_REQUEST['display']) && in_array(trim(strtolower($_REQUEST['display'])), array('list', 'grid', 'text'))) ? trim($_REQUEST['display'])  : (isset($_COOKIE['XAK']['display']) ? $_COOKIE['XAK']['display'] : $default_display_type);
    $display  = in_array($display, array('list', 'grid', 'text')) ? $display : 'text';
    setcookie('XAK[display]', $display, gmtime() + 86400 * 7);
...
elseif ($_REQUEST['act'] == 'view')
{
    ...
    if (!$smarty->is_cached('exchange_goods.dwt', $cache_id))
    {
        ...
        else
        {
            ...
            /* 上一个商品下一个商品 */
            $sql = "SELECT eg.goods_id FROM " .$xak->table('exchange_goods'). " AS eg," . $GLOBALS['xak']->table('goods') . " AS g WHERE eg.goods_id = g.goods_id AND eg.goods_id > " . $goods['goods_id'] . " AND eg.is_exchange = 1 AND g.is_delete = 0 LIMIT 1";
            ...
            $sql = "SELECT max(eg.goods_id) FROM " . $xak->table('exchange_goods') . " AS eg," . $GLOBALS['xak']->table('goods') . " AS g WHERE eg.goods_id = g.goods_id AND eg.goods_id < ".$goods['goods_id'] . " AND eg.is_exchange = 1 AND g.is_delete = 0";
...
elseif ($_REQUEST['act'] == 'buy')
{
    ...
    $db->autoExecute($xak->table('cart'), $cart, 'INSERT');
...
{
    return $GLOBALS['db']->getRow('SELECT keywords, cat_desc, style, grade, filter_attr, parent_id FROM ' . $GLOBALS['xak']->table('category') .
        " WHERE cat_id = '$cat_id'");
}
...
function exchange_get_goods($children, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, eg.exchange_integral, ' .
            ...
            'FROM ' . $GLOBALS['xak']->table('exchange_goods') . ' AS eg, ' .$GLOBALS['xak']->table('goods') . ' AS g ' .
...
function get_exchange_goods_count($children, $min = 0, $max = 0, $ext='')
{
    ...
    $sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['xak']->table('exchange_goods') . ' AS eg, ' .
           $GLOBALS['xak']->table('goods') . " AS g WHERE eg.goods_id = g.goods_id AND $where $ext";
...
function get_exchange_recommend_goods($type = '', $cats = '', $min =0,  $max = 0, $ext='')
{
    ...
    $sql =  'SELECT g.goods_id, g.goods_name, g.goods_name_style, eg.exchange_integral, ' .
            ...
            'FROM ' . $GLOBALS['xak']->table('exchange_goods') . ' AS eg ' .
            'LEFT JOIN ' . $GLOBALS['xak']->table('goods') . ' AS g ON g.goods_id = eg.goods_id ' .
...
function get_exchange_goods_info($goods_id)
{
    ...
    $sql = 'SELECT g.*, c.measure_unit, b.brand_id, b.brand_name AS goods_brand, eg.exchange_integral, eg.is_exchange ' .
            ...
            'LEFT JOIN ' . $GLOBALS['xak']->table('exchange_goods') . ' AS eg ON g.goods_id = eg.goods_id ' .
