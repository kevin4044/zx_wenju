...
if ($_GET['act'] == 'js')
{
    ...
    $sql = 'SELECT ad.ad_id, ad.ad_name, ad.ad_link, ad.ad_code '.
           'FROM ' . $xak->table('ad') . ' AS ad ' .
           'LEFT JOIN ' . $xak->table('ad_position') . ' AS p ON ad.position_id = p.position_id '.
    ...
    if (!empty($ad_info))
    {
        ...
        {
            $ad_info['ad_name'] = xak_iconv('UTF8', $_GET['charset'], $ad_info['ad_name']);
            $ad_info['ad_code'] = xak_iconv('UTF8', $_GET['charset'], $ad_info['ad_code']);
        }
...
else
{
    ...
    if ($ad_id == '-1')
    {
        $sql = "SELECT count(*) FROM " . $xak->table('adsense') . " WHERE from_ad = '-1' AND referer = '" . $site_name . "'";
        if($db->getOne($sql) > 0)
        {
            $sql = "UPDATE " . $xak->table('adsense') . " SET clicks = clicks + 1 WHERE from_ad = '-1' AND referer = '" . $site_name . "'";
        }
        else
        {
            $sql = "INSERT INTO " . $xak->table('adsense') . "(from_ad, referer, clicks) VALUES ('-1', '" . $site_name . "', '1')";
        }
        $db->query($sql);
        //$db->autoReplace($xak->table('adsense'), array('from_ad' => -1, 'referer' => $site_name, 'clicks' => 1), array('clicks' => 1));
        $sql = "SELECT goods_name FROM " .$xak->table('goods'). " WHERE goods_id = $goods_id";
        $res = $db->query($sql);
    ...
    {
        /* 更新站内广告的点击次数 */
        $db->query('UPDATE ' . $xak->table('ad') . " SET click_count = click_count + 1 WHERE ad_id = '$ad_id'");

        $sql = "SELECT count(*) FROM " . $xak->table('adsense') . " WHERE from_ad = '" . $ad_id . "' AND referer = '" . $site_name . "'";
        if($db->getOne($sql) > 0)
        {
            $sql = "UPDATE " . $xak->table('adsense') . " SET clicks = clicks + 1 WHERE from_ad = '" . $ad_id . "' AND referer = '" . $site_name . "'";
        }
        else
        {
            $sql = "INSERT INTO " . $xak->table('adsense') . "(from_ad, referer, clicks) VALUES ('" . $ad_id . "', '" . $site_name . "', '1')";
        }
        $db->query($sql);
        ...
        {
            $uri = (strpos($ad_info['ad_link'], 'http://') === false && strpos($ad_info['ad_link'], 'https://') === false) ? $xak->http() . urldecode($ad_info['ad_link']) : urldecode($ad_info['ad_link']);
        }
        else
        {
            $uri = $xak->url();
