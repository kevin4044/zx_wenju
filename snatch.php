...
if ($_REQUEST['act'] == 'bid')
{
    ...
    /* 获取活动基本信息用于校验 */
    $sql = 'SELECT act_name AS snatch_name, end_time, ext_info FROM ' . $GLOBALS['xak']->table('goods_activity') . " WHERE act_id ='$id'";
    ...
    /* 检查用户是否已经出同一价格 */
    $sql = 'SELECT COUNT(*) FROM '.$GLOBALS['xak']->table('snatch_log'). " WHERE snatch_id = '$id' AND user_id = '$_SESSION[user_id]' AND bid_price = '$price'";
    ...
    /* 检查用户积分是否足够 */
    $sql = 'SELECT pay_points FROM ' .$xak->table('users'). " WHERE user_id = '" . $_SESSION['user_id']. "'";
    ...
    $sql = 'INSERT INTO ' .$xak->table('snatch_log'). '(snatch_id, user_id, bid_price, bid_time) VALUES'.
...
function get_myprice($id)
{
    ...
    {
        /* 取得用户所有价格 */
        $sql = 'SELECT bid_price FROM '.$GLOBALS['xak']->table('snatch_log'). " WHERE snatch_id = '$id' AND user_id = '$_SESSION[user_id]' ORDER BY bid_time DESC";
        $my_price = $GLOBALS['db']->GetCol($sql);

        if ($my_price)
        {
            /* 取得用户唯一价格 */
            $sql = 'SELECT bid_price , count(*) AS num FROM '.$GLOBALS['xak']->table('snatch_log'). "  WHERE snatch_id ='$id' AND bid_price " . db_create_in(join(',', $my_price)). ' GROUP BY bid_price HAVING num = 1';
        ...
        $sql = 'SELECT pay_points FROM '. $GLOBALS['xak']->table('users')." WHERE user_id = '$_SESSION[user_id]'";
        $pay_points = $GLOBALS['db']->GetOne($sql);
        $pay_points = $pay_points.$GLOBALS['_CFG']['integral_name'];
    }

    /* 活动结束时间 */
    $sql = 'SELECT end_time FROM ' .$GLOBALS['xak']->table('goods_activity').
...
{
    $sql = 'SELECT t1.log_id, t1.bid_price, t2.user_name FROM '.$GLOBALS['xak']->table('snatch_log').' AS t1, '.$GLOBALS['xak']->table('users')." AS t2 WHERE snatch_id = '$id' AND t1.user_id = t2.user_id ORDER BY t1.log_id DESC LIMIT $num";
...
function get_snatch($id)
{
    $sql = "SELECT g.goods_id, g.goods_sn, g.is_real, g.goods_name, g.extension_code, g.market_price, g.shop_price AS org_price, product_id, " .
                ...
                "FROM " .$GLOBALS['xak']->table('goods_activity'). " AS ga " .
...
{
    $now = gmtime();
    $sql = 'SELECT act_id FROM ' . $GLOBALS['xak']->table('goods_activity').
