...
if (!$smarty->is_cached('category.dwt', $cache_id))
{
    ...
    if ($brand > 0)
    {
        $sql = "SELECT brand_name FROM " .$GLOBALS['xak']->table('brand'). " WHERE brand_id = '$brand'";
    ...
    $sql = "SELECT b.brand_id, b.brand_name, COUNT(*) AS goods_num ".
            "FROM " . $GLOBALS['xak']->table('brand') . "AS b, ".
                $GLOBALS['xak']->table('goods') . " AS g LEFT JOIN ". $GLOBALS['xak']->table('goods_cat') . " AS gc ON g.goods_id = gc.goods_id " .
    ...
    if ($cat['filter_attr'] > 0)
    {
        ...
        {
            $sql = "SELECT a.attr_name FROM " . $xak->table('attribute') . " AS a, " . $xak->table('goods_attr') . " AS ga, " . $xak->table('goods') . " AS g WHERE ($children OR " . get_extension_goods($children) . ") AND a.attr_id = ga.attr_id AND g.goods_id = ga.goods_id AND g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND a.attr_id='$value'";
            if($temp_name = $db->getOne($sql))
            {
                $all_attr_list[$key]['filter_attr_name'] = $temp_name;

                $sql = "SELECT a.attr_id, MIN(a.goods_attr_id ) AS goods_id, a.attr_value AS attr_value FROM " . $xak->table('goods_attr') . " AS a, " . $xak->table('goods') .
                       " AS g" .
        ...
        {
            $ext_sql = "SELECT DISTINCT(b.goods_id) FROM " . $xak->table('goods_attr') . " AS a, " . $xak->table('goods_attr') . " AS b " .  "WHERE ";
...
{
    return $GLOBALS['db']->getRow('SELECT cat_name, keywords, cat_desc, style, grade, filter_attr, parent_id FROM ' . $GLOBALS['xak']->table('category') .
        " WHERE cat_id = '$cat_id'");
}
...
function get_cagtegory_goods_count($children, $brand = 0, $min = 0, $max = 0, $ext='')
{
    ...
    /* 返回商品总数 */
    return $GLOBALS['db']->getOne('SELECT COUNT(*) FROM ' . $GLOBALS['xak']->table('goods') . " AS g WHERE $where $ext");
}
