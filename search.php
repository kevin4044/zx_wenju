...
else
{
    ...
    if (!empty($_REQUEST['keywords']))
    {
        ...
        foreach ($arr AS $key => $val)
        {
            ...
            $sql = 'SELECT DISTINCT goods_id FROM ' . $xak->table('tag') . " WHERE tag_words LIKE '%$val%' ";
            $res = $db->query($sql);
            ...
            $db->autoReplace($xak->table('keywords'), array('date' => local_date('Y-m-d'),
                'searchengine' => 'xakshop', 'keyword' => addslashes(str_replace('%', '', $val)), 'count' => 1), array('count' => 1));
        }
        $keywords .= ')';
    ...
    {
        $sql = "SELECT goods_id, COUNT(*) AS num FROM " . $xak->table("goods_attr") . " WHERE 0 ";
    ...
    /* 获得符合条件的商品总数 */
    $sql   = "SELECT COUNT(*) FROM " .$xak->table('goods'). " AS g ".
...
function get_seachable_attributes($cat_id = 0)
{
    ...
    /* 获得可用的商品类型 */
    $sql = "SELECT t.cat_id, cat_name FROM " .$GLOBALS['xak']->table('goods_type'). " AS t, ".
