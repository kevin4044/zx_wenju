...
if (!$smarty->is_cached('index.dwt', $cache_id))
{
    ...
    {
        $sql = 'SELECT ad_type, content, url FROM ' . $xak->table("ad_custom") . ' WHERE ad_status = 1';
...
{
    $sql = 'SELECT o.order_sn, o.invoice_no, s.shipping_code FROM ' . $GLOBALS['xak']->table('order_info') . ' AS o' .
            ' LEFT JOIN ' . $GLOBALS['xak']->table('shipping') . ' AS s ON s.shipping_id = o.shipping_id' .
...
function index_get_group_buy()
{
    ...
    if ($limit > 0)
    {
        $sql = 'SELECT gb.act_id AS group_buy_id, gb.goods_id, gb.ext_info, gb.goods_name, g.goods_thumb, g.goods_img ' .
                'FROM ' . $GLOBALS['xak']->table('goods_activity') . ' AS gb, ' .
...
{
    $sql = 'SELECT link_logo, link_name, link_url FROM ' . $GLOBALS['xak']->table('friend_link') . ' ORDER BY show_order';
    $res = $GLOBALS['db']->getAll($sql);
