...
else
{
    ...
    /* 商品 */
    $sql = "SELECT goods_id, goods_name, last_update FROM " .$xak->table('goods'). " WHERE is_delete = 0 LIMIT 300";
    $res = $db->query($sql);
    ...
    /* 文章 */
    $sql = "SELECT article_id,title,file_url,open_type, add_time FROM " .$xak->table('article'). " WHERE is_open=1";
    $res = $db->query($sql);
