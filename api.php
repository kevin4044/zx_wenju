 ...
 * ----------------------------------------------------------------------------
 * $Author: sxc_shop $
 * $File: goods.php 15921 2009-05-07 05:35:58Z sxc_shop $
*/
...
function search_goods_detail()
{
    ...
    if (!empty($_POST['goods_id']) && is_numeric($_POST['goods_id']))
    {
        ...
        ' FROM ' . $GLOBALS['xak']->table('category') . ' AS c, ' . $GLOBALS['xak']->table('goods') . ' AS g LEFT JOIN ' . $GLOBALS['xak']->table('brand') . ' AS b ON g.brand_id = b.brand_id'.
...
function search_site_info()
{
    ...
    $sql = 'SELECT code, value'.
           ' FROM ' . $GLOBALS['xak']->table('shop_config') .
...
function check_auth()
{
    ...
    $certi['app_id'] = 'xakshop_b2c'; // 说明客户端来源
