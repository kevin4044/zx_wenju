...
if (!empty($_REQUEST['goods']) && is_array($_REQUEST['goods']) && count($_REQUEST['goods']) > 1)
{
    ...
    $sql = "SELECT g.goods_id, g.goods_type, g.goods_name, g.shop_price, g.goods_weight, g.goods_thumb, g.goods_brief, ".
            ...
            "LEFT JOIN " . $xak->table('goods_attr'). " AS v ON v.goods_id = g.goods_id ".
            "LEFT JOIN " . $xak->table('attribute') . " AS a ON a.attr_id = v.attr_id " .
            "LEFT JOIN " . $xak->table('brand') . " AS b ON g.brand_id = b.brand_id " .
            "LEFT JOIN " . $xak->table('member_price') . " AS mp ".
    ...
    $sql = "SELECT attr_id,attr_name FROM " . $xak->table('attribute') . " WHERE cat_id='$type_id' ORDER BY attr_id";
