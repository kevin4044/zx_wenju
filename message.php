...
if ($action == 'default')
{
    ...
    $sql = "SELECT COUNT(*) FROM " .$GLOBALS['xak']->table('comment')." WHERE STATUS =1 AND comment_type =0 ";
    $record_count = $db->getOne($sql);
    $sql = "SELECT COUNT(*) FROM " .$GLOBALS['xak']->table('feedback')." WHERE `msg_area`='1' AND `msg_status` = '1' ";
    $record_count += $db->getOne($sql);
...
function get_msg_list($num, $start)
{
    ...
    if($mysql_ver > '3.2.3')
    {
        ...
        $sql .= " FROM " .$GLOBALS['xak']->table('comment');
        ...
        $sql .= " FROM " .$GLOBALS['xak']->table('feedback');
    ...
    else 
    {
        ...
        $con_sql .= " FROM " .$GLOBALS['xak']->table('comment');
        ...
        $fee_sql .= " FROM " .$GLOBALS['xak']->table('feedback');
        ...
        $sql = "SELECT * FROM  " .$GLOBALS['xak']->table('tmp_table') . " ORDER BY msg_time DESC ";
    }
    ...
    while ($rows = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $msg[$rows['msg_time']]['tablename'] = $rows['tablename'];
            ...
            if($rows['id_value'])
            {
                $sql_goods = "SELECT goods_name FROM ".$GLOBALS['xak']->table('goods');
        ...
        if(isset($msg[$rows['msg_time']]['tablename']))
        {
            ...
            else
            {
                ...
                FROM ' . $GLOBALS['xak']->table('comment') .
                " WHERE parent_id = $id ";
