...
{
    $article_id = $db->getOne("SELECT article_id FROM " . $xak->table('article') . " WHERE cat_id = '".intval($_REQUEST['cat_id'])."' ");
}
...
if (!$smarty->is_cached('article.dwt', $cache_id))
{
    ...
    {
        xak_header("location:$article[link]\n");
        exit;
    }
    ...
    $sql = "SELECT a.goods_id, g.goods_name " .
            "FROM " . $xak->table('goods_article') . " AS a, " . $xak->table('goods') . " AS g " .
            "WHERE a.goods_id = g.goods_id " .
            "AND a.article_id = '$_REQUEST[id]' ";
    $smarty->assign('goods_list', $db->getAll($sql));

    /* 上一篇下一篇文章 */
    $next_article = $db->getRow("SELECT article_id, title FROM " .$xak->table('article'). " WHERE article_id > $article_id AND cat_id=$article[cat_id] AND is_open=1 LIMIT 1");
    ...
    $prev_aid = $db->getOne("SELECT max(article_id) FROM " . $xak->table('article') . " WHERE article_id < $article_id AND cat_id=$article[cat_id] AND is_open=1");
    if (!empty($prev_aid))
    {
        $prev_article = $db->getRow("SELECT article_id, title FROM " .$xak->table('article'). " WHERE article_id = $prev_aid");
...
function get_article_info($article_id)
{
    ...
    $sql = "SELECT a.*, IFNULL(AVG(r.comment_rank), 0) AS comment_rank ".
            ...
            "LEFT JOIN " .$GLOBALS['xak']->table('comment'). " AS r ON r.id_value = a.article_id AND comment_type = 1 ".
...
function article_related_goods($id)
{
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_thumb, g.goods_img, g.shop_price AS org_price, ' .
            ...
            'FROM ' . $GLOBALS['xak']->table('goods_article') . ' ga ' .
