...
$sql = "SELECT * FROM " . $xak->table('favourable_activity'). " ORDER BY `sort_order` ASC,`end_time` DESC";
$res = $db->query($sql);
    ...
    switch($row['act_type'])
    {
        case 0:
            ...
            if(is_array($row['gift']))
            {
                ...
                {
                    $row['gift'][$k]['thumb'] = get_image_path($v['id'], $db->getOne("SELECT goods_thumb FROM " . $xak->table('goods') . " WHERE goods_id = '" . $v['id'] . "'"), true);
                }
            }
            break;
        case 1:
