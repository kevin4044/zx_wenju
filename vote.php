...
else
{
    ...
    $pattern = '/(?:<(\w+)[^>]*> .*?)?<div\s+id="XAK_VOTE">(.*)<\/div>(?:.*?<\/\1>)?/is';
...
{
    $sql = "SELECT COUNT(*) FROM ".$GLOBALS['xak']->table('vote_log')." ".
...
{
    $sql = "INSERT INTO " . $GLOBALS['xak']->table('vote_log') . " (vote_id, ip_address, vote_time) " .
           "VALUES ('$vote_id', '$ip_address', " . gmtime() .")";
    $res = $GLOBALS['db']->query($sql);

    /* 更新投票主题的数量 */
    $sql = "UPDATE " .$GLOBALS['xak']->table('vote'). " SET ".
           "vote_count = vote_count + 1 ".
           "WHERE vote_id = '$vote_id'";
    $GLOBALS['db']->query($sql);

    /* 更新投票选项的数量 */
    $sql = "UPDATE " . $GLOBALS['xak']->table('vote_option') . " SET " .
