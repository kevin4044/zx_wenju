...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'gotopage')
{
    ...
    if (!empty($goods_id))
    {
        ...
        $sql = 'SELECT u.user_name, og.goods_number, oi.add_time, IF(oi.order_status IN (2, 3, 4), 0, 1) AS order_status ' .
               'FROM ' . $xak->table('order_info') . ' AS oi LEFT JOIN ' . $xak->table('users') . ' AS u ON oi.user_id = u.user_id, ' . $xak->table('order_goods') . ' AS og ' .
...
if (!$smarty->is_cached('goods.dwt', $cache_id))
{
    ...
    else
    {
        ...
        {
            $time = gmtime();
            $sql = "SELECT type_money FROM " . $xak->table('bonus_type') .
        ...
        assign_template('c', $catlist);
         ...
         /* 上一个商品下一个商品 */
        $prev_gid = $db->getOne("SELECT goods_id FROM " .$xak->table('goods'). " WHERE cat_id=" . $goods['cat_id'] . " AND goods_id > " . $goods['goods_id'] . " AND is_on_sale = 1 AND is_alone_sale = 1 AND is_delete = 0 LIMIT 1");
        if (!empty($prev_gid))
        {
        ...
        $next_gid = $db->getOne("SELECT max(goods_id) FROM " . $xak->table('goods') . " WHERE cat_id=".$goods['cat_id']." AND goods_id < ".$goods['goods_id'] . " AND is_on_sale = 1 AND is_alone_sale = 1 AND is_delete = 0");
        if (!empty($next_gid))
        {
...
/* 记录浏览历史 */
if (!empty($_COOKIE['XAK']['history']))
{
    $history = explode(',', $_COOKIE['XAK']['history']);
    ...
    setcookie('XAK[history]', implode(',', $history), gmtime() + 3600 * 24 * 30);
}
else
{
    setcookie('XAK[history]', $goods_id, gmtime() + 3600 * 24 * 30);
}


/* 更新点击次数 */
$db->query('UPDATE ' . $xak->table('goods') . " SET click_count = click_count + 1 WHERE goods_id = '$_REQUEST[id]'");
...
function get_linked_goods($goods_id)
{
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_thumb, g.goods_img, g.shop_price AS org_price, ' .
            ...
            'FROM ' . $GLOBALS['xak']->table('link_goods') . ' lg ' .
            'LEFT JOIN ' . $GLOBALS['xak']->table('goods') . ' AS g ON g.goods_id = lg.link_goods_id ' .
...
function get_user_rank_prices($goods_id, $shop_price)
{
    $sql = "SELECT rank_id, IFNULL(mp.user_price, r.discount * $shop_price / 100) AS price, r.rank_name, r.discount " .
            'FROM ' . $GLOBALS['xak']->table('user_rank') . ' AS r ' .
            'LEFT JOIN ' . $GLOBALS['xak']->table('member_price') . " AS mp ".
...
function get_also_bought($goods_id)
{
    $sql = 'SELECT COUNT(b.goods_id ) AS num, g.goods_id, g.goods_name, g.goods_thumb, g.goods_img, g.shop_price, g.promote_price, g.promote_start_date, g.promote_end_date ' .
            'FROM ' . $GLOBALS['xak']->table('order_goods') . ' AS a ' .
            'LEFT JOIN ' . $GLOBALS['xak']->table('order_goods') . ' AS b ON b.order_id = a.order_id ' .
            'LEFT JOIN ' . $GLOBALS['xak']->table('goods') . ' AS g ON g.goods_id = b.goods_id ' .
...
{
    $sql = "SELECT SUM(attr_price) FROM " . $GLOBALS['xak']->table('goods_attr') .
...
function get_package_goods_list($goods_id)
{
    ...
    $sql = "SELECT pg.goods_id, ga.act_id, ga.act_name, ga.act_desc, ga.goods_name, ga.start_time,
            ...
            FROM " . $GLOBALS['xak']->table('goods_activity') . " AS ga, " . $GLOBALS['xak']->table('package_goods') . " AS pg
    ...
    foreach ($res as $tempkey => $value)
    {
        ...
        $sql = "SELECT pg.package_id, pg.goods_id, pg.goods_number, pg.admin_id, p.goods_attr, g.goods_sn, g.goods_name, g.market_price, g.goods_thumb, IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS rank_price
                FROM " . $GLOBALS['xak']->table('package_goods') . " AS pg
                    ...
                    LEFT JOIN ". $GLOBALS['xak']->table('products') . " AS p
