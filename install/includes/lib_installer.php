...
function create_config_file($db_host, $db_port, $db_user, $db_pass, $db_name, $prefix, $timezone)
{
    ...
    $content .= "define('XAK_CHARSET','".XAK_CHARSET."');\n\n";
    $content .= "define('ADMIN_PATH','xakadmin');\n\n";
    $content .= "define('AUTH_KEY', 'xak998998998');\n\n";
...
function install_data($sql_files)
{
    ...
    $se = new sql_executor($db, EC_DB_CHARSET, 'xsp_', $prefix);
...
function deal_aftermath()
{
    ...
    include(ROOT_PATH . 'data/config.php');
    include_once(ROOT_PATH . 'includes/cls_xak.php');
    include_once(ROOT_PATH . 'includes/cls_mysql.php');
    ...
    /* 更新 XKASHOP 版本 */
    $sql = "UPDATE $prefix"."shop_config SET value='" .VERSION. "' WHERE code='xak_version'";
    if (!$db->query($sql, 'SILENT'))
    {
        $err->add($db->errno() .' '. $db->error());
        return false;
    ...
    if (!@fwrite($fp, "XKASHOP INSTALLED"))
    {
...
function url()
{
    ...
    $xakerver = 'http://'.$_SERVER['HTTP_HOST'].($_SERVER['SERVER_PORT'] && $_SERVER['SERVER_PORT'] != 80 ? ':'.$_SERVER['SERVER_PORT'] : '');
    $default_appurl = $xakerver.substr($PHP_SELF, 0, strpos($PHP_SELF, 'install/') - 1);

    return $default_appurl;
}

/** 获得 XKASHOP 当前环境的 HTTP 协议方式
 *
 * @access  public
 *
 * @return  void
