...
function shipping_list()
{
    $sql = 'SELECT shipping_id, shipping_name ' .
            'FROM ' . $GLOBALS['xak']->table('shipping') .
            ' WHERE enabled = 1';
...
{
    $sql = 'SELECT * FROM ' . $GLOBALS['xak']->table('shipping') .
...
function available_shipping_list($region_id_list)
{
    $sql = 'SELECT s.shipping_id, s.shipping_code, s.shipping_name, ' .
            ...
            'FROM ' . $GLOBALS['xak']->table('shipping') . ' AS s, ' .
                $GLOBALS['xak']->table('shipping_area') . ' AS a, ' .
                $GLOBALS['xak']->table('area_region') . ' AS r ' .
...
function payment_list()
{
    $sql = 'SELECT pay_id, pay_name ' .
            'FROM ' . $GLOBALS['xak']->table('payment') .
            ' WHERE enabled = 1';
...
{
    $sql = 'SELECT * FROM ' . $GLOBALS['xak']->table('payment') .
...
function pack_list()
{
    $sql = 'SELECT * FROM ' . $GLOBALS['xak']->table('pack');
    $res = $GLOBALS['db']->query($sql);
...
function card_list()
{
    $sql = "SELECT * FROM " . $GLOBALS['xak']->table('card');
    $res = $GLOBALS['db']->query($sql);
...
function order_info($order_id, $order_sn = '')
{
    ...
    if ($order_id > 0)
    {
        $sql = "SELECT *, " . $total_fee . " FROM " . $GLOBALS['xak']->table('order_info') .
...
function order_fee($order, $goods, $consignee)
{
    ...
    if ($order['shipping_id'] > 0 && $total['real_goods_count'] > 0)
    {
        ...
        if (!empty($shipping_info))
        {
            ...
            $sql = 'SELECT count(*) FROM ' . $GLOBALS['xak']->table('cart') . " WHERE  `session_id` = '" . SESS_ID. "' AND `extension_code` != 'package_buy' AND `is_shipping` = 0";
    ...
    if ($order['extension_code'] == 'exchange_goods')
    {
        $sql = 'SELECT SUM(eg.exchange_integral) '.
               'FROM ' . $GLOBALS['xak']->table('cart') . ' AS c,' . $GLOBALS['xak']->table('exchange_goods') . 'AS eg '.
...
{
    return $GLOBALS['db']->autoExecute($GLOBALS['xak']->table('order_info'),
...
{
    /* 检查该商品是否已经存在在购物车中 */
    $sql = "SELECT COUNT(*) FROM " .$GLOBALS['xak']->table('cart').
...
function cart_weight_price($type = CART_GENERAL_GOODS)
{
    ...
    /* 计算超值礼包内商品的相关配送参数 */
    $sql = 'SELECT goods_id, goods_number, goods_price FROM ' . $GLOBALS['xak']->table('cart') . " WHERE extension_code = 'package_buy' AND session_id = '" . SESS_ID . "'";
    ...
    if ($row)
    {
        ...
        foreach ($row as $val)
        {
            ...
            $sql = 'SELECT count(*) FROM ' .
                    $GLOBALS['xak']->table('package_goods') . ' AS pg, ' .
                    $GLOBALS['xak']->table('goods') . ' AS g ' .
    ...
    $sql    = 'SELECT SUM(g.goods_weight * c.goods_number) AS weight, ' .
                ...
                'FROM ' . $GLOBALS['xak']->table('cart') . ' AS c '.
                'LEFT JOIN ' . $GLOBALS['xak']->table('goods') . ' AS g ON g.goods_id = c.goods_id '.
...
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0)
{
    ...
    $sql = "SELECT g.goods_name, g.goods_sn, g.is_on_sale, g.is_real, ".
            ...
            " FROM " .$GLOBALS['xak']->table('goods'). " AS g ".
            " LEFT JOIN " . $GLOBALS['xak']->table('member_price') . " AS mp ".
    ...
    $sql = "SELECT parent_id, goods_price " .
            "FROM " . $GLOBALS['xak']->table('group_goods') .
    ...
    foreach ($basic_list as $parent_id => $fitting_price)
    {
        ...
        /* 添加 */
        $GLOBALS['db']->autoExecute($GLOBALS['xak']->table('cart'), $parent, 'INSERT');
    ...
    {
        /* 检查该商品是否已经存在在购物车中 */
        $sql = "SELECT goods_number FROM " .$GLOBALS['xak']->table('cart').
        ...
        if($row) //如果购物车已经有此物品，则更新
        {
            ...
            {
                $goods_price = get_final_price($goods_id, $num, true, $spec);
                $sql = "UPDATE " . $GLOBALS['xak']->table('cart') . " SET goods_number = '$num'" .
    ...
    /* 把赠品删除 */
    $sql = "DELETE FROM " . $GLOBALS['xak']->table('cart') . " WHERE session_id = '" . SESS_ID . "' AND is_gift <> 0";
    $GLOBALS['db']->query($sql);
...
{
    $sql = "DELETE FROM " . $GLOBALS['xak']->table('cart') .
...
function get_goods_attr_info($arr, $type = 'pice')
{
    ...
    if (!empty($arr))
    {
        ...
        $sql = "SELECT a.attr_name, ga.attr_value, ga.attr_price ".
                "FROM ".$GLOBALS['xak']->table('goods_attr')." AS ga, ".
                    $GLOBALS['xak']->table('attribute')." AS a ".
...
{
    $sql = "SELECT * FROM " . $GLOBALS['xak']->table('users') .
...
{
    return $GLOBALS['db']->autoExecute($GLOBALS['xak']->table('users'),
...
{
    $sql = "SELECT * FROM " . $GLOBALS['xak']->table('user_address') .
            " WHERE user_id = '$user_id'";
...
function user_bonus($user_id, $goods_amount = 0)
{
    ...
    $sql = "SELECT t.type_id, t.type_name, t.type_money, b.bonus_id " .
            "FROM " . $GLOBALS['xak']->table('bonus_type') . " AS t," .
                $GLOBALS['xak']->table('user_bonus') . " AS b " .
...
{
    $sql = "SELECT order_id FROM " . $GLOBALS['xak']->table('user_bonus') .
...
{
    $sql = "UPDATE " . $GLOBALS['xak']->table('user_bonus') .
...
function order_refund($order, $refund_type, $refund_note, $refund_amount = 0)
{
    ...
    elseif (2 == $refund_type)
    {
        ...
        $GLOBALS['db']->autoExecute($GLOBALS['xak']->table('user_account'), $account, 'INSERT');
...
function get_cart_goods()
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        if (trim($row['goods_attr']) != '')
        {
            ...
            $sql = "SELECT attr_value FROM " . $GLOBALS['xak']->table('goods_attr') . " WHERE goods_attr_id " .
        ...
        {
            $goods_thumb = $GLOBALS['db']->getOne("SELECT `goods_thumb` FROM " . $GLOBALS['xak']->table('goods') . " WHERE `goods_id`='{$row['goods_id']}'");
...
function get_consignee($user_id)
{
    ...
    else
    {
        ...
        if ($user_id > 0)
        {
            /* 取默认地址 */
            $sql = "SELECT ua.*".
                    " FROM " . $GLOBALS['xak']->table('user_address') . "AS ua, ".$GLOBALS['xak']->table('users').' AS u '.
...
function exist_real_goods($order_id = 0, $flow_type = CART_GENERAL_GOODS)
{
    ...
    {
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['xak']->table('order_goods') .
...
function get_total_bonus()
{
    ...
    $sql = "SELECT SUM(c.goods_number * t.type_money)" .
            "FROM " . $GLOBALS['xak']->table('cart') . " AS c, "
                    . $GLOBALS['xak']->table('bonus_type') . " AS t, "
                    . $GLOBALS['xak']->table('goods') . " AS g " .
            "WHERE c.session_id = '" . SESS_ID . "' " .
            "AND c.is_gift = 0 " .
    ...
    $sql = "SELECT FLOOR('$amount' / min_amount) * type_money " .
            "FROM " . $GLOBALS['xak']->table('bonus_type') .
...
function change_user_bonus($bonus_id, $order_id, $is_used = true)
{
    if ($is_used)
    {
        $sql = 'UPDATE ' . $GLOBALS['xak']->table('user_bonus') . ' SET ' .
                'used_time = ' . gmtime() . ', ' .
                "order_id = '$order_id' " .
                "WHERE bonus_id = '$bonus_id'";
    }
...
function merge_order($from_order_sn, $to_order_sn)
{
    ...
    do
    {
        ...
        if ($GLOBALS['db']->autoExecute($GLOBALS['xak']->table('order_info'), addslashes_deep($order), 'INSERT'))
        {
            break;
        }
        else
    ...
    /* 删除原订单 */
    $sql = 'DELETE FROM ' . $GLOBALS['xak']->table('order_info') .
            " WHERE order_id " . db_create_in(array($from_order['order_id'], $to_order['order_id']));
    $GLOBALS['db']->query($sql);

    /* 删除原订单支付日志 */
    $sql = 'DELETE FROM ' . $GLOBALS['xak']->table('pay_log') .
            " WHERE order_id " . db_create_in(array($from_order['order_id'], $to_order['order_id']));
    $GLOBALS['db']->query($sql);
...
function change_order_goods_storage($order_id, $is_dec = true, $storage = 0)
{
    ...
    {
        case 0 :
            $sql = "SELECT goods_id, SUM(send_number) AS num, MAX(extension_code) AS extension_code, product_id FROM " . $GLOBALS['xak']->table('order_goods') .
                    " WHERE order_id = '$order_id' AND is_real = 1 GROUP BY goods_id, product_id";
        break;

        case 1 :
            $sql = "SELECT goods_id, SUM(goods_number) AS num, MAX(extension_code) AS extension_code, product_id FROM " . $GLOBALS['xak']->table('order_goods') .
                    " WHERE order_id = '$order_id' AND is_real = 1 GROUP BY goods_id, product_id";
        break;
    }
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        else
        {
            $sql = "SELECT goods_id, goods_number" .
                   " FROM " . $GLOBALS['xak']->table('package_goods') .
...
{
    $sql = "SELECT pay_id FROM " . $GLOBALS['xak']->table('payment');
    if ($is_cod)
    {
        $sql .= " WHERE is_cod = 1";
    }
...
function compute_discount()
{
    ...
    $sql = "SELECT *" .
            "FROM " . $GLOBALS['xak']->table('favourable_activity') .
    ...
    $sql = "SELECT c.goods_id, c.goods_price * c.goods_number AS subtotal, g.cat_id, g.brand_id " .
            "FROM " . $GLOBALS['xak']->table('cart') . " AS c, " . $GLOBALS['xak']->table('goods') . " AS g " .
...
function integral_to_give($order)
{
    ...
    else
    {
        $sql = "SELECT SUM(og.goods_number * IF(g.give_integral > -1, g.give_integral, og.goods_price)) AS custom_points, SUM(og.goods_number * IF(g.rank_integral > -1, g.rank_integral, og.goods_price)) AS rank_points " .
                "FROM " . $GLOBALS['xak']->table('order_goods') . " AS og, " .
                          $GLOBALS['xak']->table('goods') . " AS g " .
...
function send_order_bonus($order_id)
{
    ...
    if ($bonus_list)
    {
        ...
        foreach ($bonus_list AS $bonus)
        {
            ...
            /* 修改用户红包 */
            $sql = "INSERT INTO " . $GLOBALS['xak']->table('user_bonus') . " (bonus_type_id, user_id) " .
...
function return_order_bonus($order_id)
{
    ...
    if ($bonus_list)
    {
        ...
        foreach ($bonus_list AS $bonus)
        {
            $sql = "DELETE FROM " . $GLOBALS['xak']->table('user_bonus') .
...
function order_bonus($order_id)
{
    ...
    $sql = "SELECT b.type_id, b.type_money, SUM(o.goods_number) AS number " .
            "FROM " . $GLOBALS['xak']->table('order_goods') . " AS o, " .
                      $GLOBALS['xak']->table('goods') . " AS g, " .
                      $GLOBALS['xak']->table('bonus_type') . " AS b " .
            " WHERE o.order_id = '$order_id' " .
            " AND o.is_gift = 0 " .
            " AND o.goods_id = g.goods_id " .
...
function judge_package_stock($package_id, $package_num = 1)
{
    ...
    if ($goods['product_ids'] != '')
    {
        $sql = "SELECT p.product_id
                FROM " . $GLOBALS['xak']->table('products') . " AS p, " . $GLOBALS['xak']->table('package_goods') . " AS pg
    ...
    if ($goods['goods_ids'] != '')
    {
        $sql = "SELECT g.goods_id
                FROM " . $GLOBALS['xak']->table('goods') . "AS g, " . $GLOBALS['xak']->table('package_goods') . " AS pg
