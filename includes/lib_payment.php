...
{
    return $GLOBALS['xak']->url() . 'respond.php?code=' . $code;
}
...
function get_order_id_by_sn($order_sn, $voucher = 'false')
{
    if ($voucher == 'true')
    {
        ...
        {
              return $GLOBALS['db']->getOne("SELECT log_id FROM " . $GLOBALS['xak']->table('pay_log') . " WHERE order_id=" . $order_sn . ' AND order_type=1');
        }
        else
        {
            return "";
    ...
    else
    {
        if(is_numeric($order_sn))
        {
            $sql = 'SELECT order_id FROM ' . $GLOBALS['xak']->table('order_info'). " WHERE order_sn = '$order_sn'";
            $order_id = $GLOBALS['db']->getOne($sql);
        }
        if (!empty($order_id))
        {
            $pay_log_id = $GLOBALS['db']->getOne("SELECT log_id FROM " . $GLOBALS['xak']->table('pay_log') . " WHERE order_id='" . $order_id . "'");
            return $pay_log_id;
        }
        else
        {
...
{
    $sql = 'SELECT goods_name FROM ' . $GLOBALS['xak']->table('order_goods'). " WHERE order_id = '$order_id'";
...
function check_money($log_id, $money)
{
    ...
    {
        $sql = 'SELECT order_amount FROM ' . $GLOBALS['xak']->table('pay_log') .
              " WHERE log_id = '$log_id'";
...
function order_paid($log_id, $pay_status = PS_PAYED, $note = '')
{
    ...
    {
        /* 取得要修改的支付记录信息 */
        $sql = "SELECT * FROM " . $GLOBALS['xak']->table('pay_log') .
                " WHERE log_id = '$log_id'";
        ...
        if ($pay_log && $pay_log['is_paid'] == 0)
        {
            ...
            {
                $sql = 'SELECT `id` FROM ' . $GLOBALS['xak']->table('user_account') .  " WHERE `id` = '$pay_log[order_id]' AND `is_paid` = 1  LIMIT 1";
                $res_id=$GLOBALS['db']->getOne($sql);
                if(empty($res_id))
                {
                    /* 更新会员预付款的到款状态 */
                    $sql = 'UPDATE ' . $GLOBALS['xak']->table('user_account') .
                           " SET paid_time = '" .gmtime(). "', is_paid = 1" .
                           " WHERE id = '$pay_log[order_id]' LIMIT 1";
                    $GLOBALS['db']->query($sql);

                    /* 取得添加预付款的用户以及金额 */
                    $sql = "SELECT user_id, amount FROM " . $GLOBALS['xak']->table('user_account') .
        ...
        else
        {
            ...
            if (!empty($post_virtual_goods))
            {
                $msg = '';
                /* 检查两次刷新时间有无超过12小时 */
                $sql = 'SELECT pay_time, order_sn FROM ' . $GLOBALS['xak']->table('order_info') . " WHERE order_id = '$pay_log[order_id]'";
