...
function get_collection_goods($user_id, $num = 10, $start = 0)
{
    $sql = 'SELECT g.goods_id, g.goods_name, g.market_price, g.shop_price AS org_price, '.
            ...
            ' FROM ' . $GLOBALS['xak']->table('collect_goods') . ' AS c' .
...
{
    $sql = 'SELECT COUNT(*) '.
           'FROM ' .$GLOBALS['xak']->table('booking_goods').
...
function get_message_list($user_id, $user_name, $num, $start, $order_id = 0)
{
    /* 获取留言数据 */
    $msg = array();
    $sql = "SELECT * FROM " .$GLOBALS['xak']->table('feedback');
    if ($order_id)
    {
    ...
    while ($rows = $GLOBALS['db']->fetchRow($res))
    {
        ...
        //{
            ...
            $sql   = "SELECT user_name, user_email, msg_time, msg_content".
                     " FROM " .$GLOBALS['xak']->table('feedback') .
...
function add_message($message)
{
    ...
    $sql = "INSERT INTO " . $GLOBALS['xak']->table('feedback') .
...
{
     $sql = "DELETE FROM ".$GLOBALS['xak']->table('tag').
...
function get_booking_list($user_id, $num, $start)
{
    ...
    $sql = "SELECT bg.rec_id, bg.goods_id, bg.goods_number, bg.booking_time, bg.dispose_note, g.goods_name ".
           "FROM " .$GLOBALS['xak']->table('booking_goods')." AS bg , " .$GLOBALS['xak']->table('goods')." AS g". " WHERE bg.goods_id = g.goods_id AND bg.user_id = '$user_id' ORDER BY bg.booking_time DESC";
    $res = $GLOBALS['db']->SelectLimit($sql, $num, $start);
...
{
    $info = array();
    $sql  = "SELECT goods_name FROM " .$GLOBALS['xak']->table('goods'). " WHERE goods_id = '$goods_id'";
...
{
    $sql = 'DELETE FROM ' .$GLOBALS['xak']->table('booking_goods').
...
{
    $sql = "INSERT INTO " .$GLOBALS['xak']->table('booking_goods').
...
{
    $sql = 'INSERT INTO ' .$GLOBALS['xak']->table('user_account').
...
{
    $sql = 'UPDATE ' .$GLOBALS['xak']->table('user_account'). ' SET '.
...
{
    $sql = 'INSERT INTO ' .$GLOBALS['xak']->table('pay_log')." (order_id, order_amount, order_type, is_paid)".
...
{
    $sql = 'SELECT * FROM ' .$GLOBALS['xak']->table('user_account').
           " WHERE id = '$surplus_id'";
...
{
    $sql = 'DELETE FROM ' .$GLOBALS['xak']->table('user_account').
...
{
    $sql = "SELECT SUM(user_money) FROM " .$GLOBALS['xak']->table('account_log').
           " WHERE user_id = '$user_id'";
...
function get_user_default($user_id)
{
    ...
    $sql = "SELECT pay_points, user_money, credit_line, last_login, is_validated FROM " .$GLOBALS['xak']->table('users'). " WHERE user_id = '$user_id'";
    $row = $GLOBALS['db']->getRow($sql);
    $info = array();
    ...
    $sql = "SELECT COUNT(*) FROM " .$GLOBALS['xak']->table('order_info').
...
function add_tag($id, $tag)
{
    ...
    {
        /* 检查是否重复 */
        $sql = "SELECT COUNT(*) FROM ". $GLOBALS['xak']->table("tag").
                " WHERE user_id = '".$_SESSION['user_id']."' AND goods_id = '$id' AND tag_words = '$val'";

        if ($GLOBALS['db']->getOne($sql) == 0)
        {
            $sql = "INSERT INTO ".$GLOBALS['xak']->table("tag")." (user_id, goods_id, tag_words) ".
...
function color_tag(&$tags)
{
    ...
    foreach ($tags AS $key => $val)
    {
        ...
        if ($rewrite)
        {
            if (strtolower(XAK_CHARSET) !== 'utf-8')
            {
...
function get_rank_info()
{
    ...
    {
        $sql = "SELECT rank_name, special_rank FROM " . $xak->table('user_rank') . " WHERE rank_id = '$_SESSION[user_rank]'";
        $row = $db->getRow($sql);
        if (empty($row))
        {
            return array();
        ...
        {
            $user_rank = $db->getOne("SELECT rank_points FROM " . $xak->table('users') . " WHERE user_id = '$_SESSION[user_id]'");
            $sql = "SELECT rank_name,min_points FROM " . $xak->table('user_rank') . " WHERE min_points > '$user_rank' ORDER BY min_points ASC LIMIT 1";
            $rt  = $db->getRow($sql);
...
function get_comment_list($user_id, $page_size, $start)
{
    $sql = "SELECT c.*, g.goods_name AS cmt_name, r.content AS reply_content, r.add_time AS reply_time ".
           " FROM " . $GLOBALS['xak']->table('comment') . " AS c ".
    ...
    if ($to_article)
    {
        $sql = "SELECT article_id , title FROM " . $GLOBALS['xak']->table('article') . " WHERE " . db_create_in($to_article, 'article_id');
