...
function get_categories_tree($cat_id = 0)
{
    if ($cat_id > 0)
    {
        $sql = 'SELECT parent_id FROM ' . $GLOBALS['xak']->table('category') . " WHERE cat_id = '$cat_id'";
    ...
    /*
     ...
     如果不是取当前分类及其下的子分类
    */
        $sql = 'SELECT count(*) FROM ' . $GLOBALS['xak']->table('category') . " WHERE parent_id = '$parent_id' AND is_show = 1 ";
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0)
    {
        /* 获取当前分类及其子分类 */
        $sql = 'SELECT cat_id,cat_name ,parent_id,is_show,keywords ' .
                'FROM ' . $GLOBALS['xak']->table('category') .
...
{
    $three_arr = array();
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['xak']->table('category') . " WHERE parent_id = '$tree_id' AND is_show = 1 ";
...
function get_top10($cats = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.shop_price, g.goods_thumb, SUM(og.goods_number) as goods_number ' .
           'FROM ' . $GLOBALS['xak']->table('goods') . ' AS g, ' .
                $GLOBALS['xak']->table('order_info') . ' AS o, ' .
                $GLOBALS['xak']->table('order_goods') . ' AS og ' .
    ...
    if (empty($type_goods[$type]))
    {
        ...
        if ($data === false)
        {
            $sql = 'SELECT g.goods_id, g.is_best, g.is_new, g.is_hot, g.is_promote, b.brand_name,g.sort_order ' .
               ' FROM ' . $GLOBALS['xak']->table('goods') . ' AS g ' .
               ' LEFT JOIN ' . $GLOBALS['xak']->table('brand') . ' AS b ON b.brand_id = g.brand_id ' .
...
function get_goods_info($goods_id)
{
    ...
    $sql = 'SELECT g.*, c.measure_unit, b.brand_id, b.brand_name AS goods_brand, m.type_money AS bonus_money, ' .
            ...
            'FROM ' . $GLOBALS['xak']->table('goods') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['xak']->table('category') . ' AS c ON g.cat_id = c.cat_id ' .
            'LEFT JOIN ' . $GLOBALS['xak']->table('brand') . ' AS b ON g.brand_id = b.brand_id ' .
            'LEFT JOIN ' . $GLOBALS['xak']->table('comment') . ' AS r '.
                'ON r.id_value = g.goods_id AND comment_type = 0 AND r.parent_id = 0 AND r.status = 1 ' .
            'LEFT JOIN ' . $GLOBALS['xak']->table('bonus_type') . ' AS m ' .
...
function get_goods_properties($goods_id)
{
    ...
    $sql = "SELECT attr_group ".
            "FROM " . $GLOBALS['xak']->table('goods_type') . " AS gt, " . $GLOBALS['xak']->table('goods') . " AS g ".
    ...
    if (!empty($attr))
    {
        foreach ($attr['lnk'] AS $key => $val)
        {
            ...
            $sql = 'SELECT g.goods_id, g.goods_name, g.goods_thumb, g.goods_img, g.shop_price AS org_price, ' .
                    ...
                    'FROM ' . $GLOBALS['xak']->table('goods') . ' AS g ' .
                    'LEFT JOIN ' . $GLOBALS['xak']->table('goods_attr') . ' as a ON g.goods_id = a.goods_id ' .
                    "LEFT JOIN " . $GLOBALS['xak']->table('member_price') . " AS mp ".
                        "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
...
function assign_cat_goods($cat_id, $num = 0, $from = 'web', $order_rule = '')
{
    ...
    /* 分类信息 */
    $sql = 'SELECT cat_name FROM ' . $GLOBALS['xak']->table('category') . " WHERE cat_id = '$cat_id'";
...
function assign_brand_goods($brand_id, $num = 0, $cat_id = 0,$order_rule = '')
{
    ...
    /* 分类信息 */
    $sql = 'SELECT brand_name FROM ' . $GLOBALS['xak']->table('brand') . " WHERE brand_id = '$brand_id'";
...
function get_extension_goods($cats)
{
    ...
    $sql = 'SELECT goods_id FROM ' . $GLOBALS['xak']->table('goods_cat') . " AS g WHERE $cats";
...
function spec_price($spec)
{
    if (!empty($spec))
    {
        ...
        $sql = 'SELECT SUM(attr_price) AS attr_price FROM ' . $GLOBALS['xak']->table('goods_attr') . " WHERE $where";
...
function group_buy_stat($group_buy_id, $deposit)
{
    ...
    $sql = "SELECT COUNT(*) AS total_order, SUM(g.goods_number) AS total_goods " .
            "FROM " . $GLOBALS['xak']->table('order_info') . " AS o, " .
                $GLOBALS['xak']->table('order_goods') . " AS g " .
...
{
    $sql = "SELECT * FROM " . $GLOBALS['xak']->table('goods_activity') . " WHERE act_id = '$act_id'";
    ...
    /* 查询出价用户数和最后出价 */
    $sql = "SELECT COUNT(DISTINCT bid_user) FROM " . $GLOBALS['xak']->table('auction_log') .
            " WHERE act_id = '$act_id'";
    $auction['bid_user_count'] = $GLOBALS['db']->getOne($sql);
    if ($auction['bid_user_count'] > 0)
    {
        $sql = "SELECT a.*, u.user_name " .
                "FROM " . $GLOBALS['xak']->table('auction_log') . " AS a, " .
...
{
    $sql = "SELECT * FROM " . $GLOBALS['xak']->table('favourable_activity') .
            " WHERE act_id = '$act_id'";
    $row = $GLOBALS['db']->getRow($sql);
    if (!empty($row))
    {
...
{
    $sql = "SELECT * FROM " . $GLOBALS['xak']->table('wholesale') .
            " WHERE act_id = '$act_id'";
    $row = $GLOBALS['db']->getRow($sql);
    if (!empty($row))
    {
...
function get_goods_attr($goods_id)
{
    ...
    $sql = "SELECT a.attr_id, a.attr_name " .
            "FROM " . $GLOBALS['xak']->table('goods') . " AS g, " . $GLOBALS['xak']->table('attribute') . " AS a " .
            "WHERE g.goods_id = '$goods_id' " .
...
function get_goods_fittings($goods_list = array())
{
    ...
    $sql = 'SELECT gg.parent_id, ggg.goods_name AS parent_name, gg.goods_id, gg.goods_price, g.goods_name, g.goods_thumb, g.goods_img, g.shop_price AS org_price, ' .
            ...
            'LEFT JOIN ' . $GLOBALS['xak']->table('goods') . 'AS g ON g.goods_id = gg.goods_id ' .
            "LEFT JOIN " . $GLOBALS['xak']->table('member_price') . " AS mp ".
                    "ON mp.goods_id = gg.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
            "LEFT JOIN " . $GLOBALS['xak']->table('goods') . " AS ggg ON ggg.goods_id = gg.parent_id ".
...
function get_products_info($goods_id, $spec_goods_attr_id)
{
    ...
    if(isset($goods_attr_array['sort']))
    {
        ...
        $sql = "SELECT * FROM " .$GLOBALS['xak']->table('products'). " WHERE goods_id = '$goods_id' AND goods_attr = '$goods_attr' LIMIT 0, 1";
