...
class integrate
{
    ...
    function integrate($cfg)
    {
        ...
        {
            $this->db_name = $GLOBALS['xak']->db_name;
            $this->prefix = $GLOBALS['xak']->prefix;
            $this->db = &$GLOBALS['db'];
        }
        else
        {
    ...
    function edit_user($cfg)
    {
        ...
        if ((!empty($cfg['email'])) && $this->field_email != 'NULL')
        {
            ...
            {
                // 新的E-mail
                $sql = "UPDATE " . $GLOBALS['xak']->table('users') . " SET is_validated = 0 WHERE user_name = '$cfg[post_username]'";
                $this->db->query($sql);
            }
    ...
    function remove_user($id)
    {
        ...
        if ($this->need_sync || (isset($this->is_xakshop) && $this->is_xakshop))
        {
            /* 如果需要同步或是默认登录插件执行这部分代码 */
    ...
    function set_cookie($username='', $remember= null )
    {
        ...
        elseif ($remember)
        {
            ...
            setcookie("XAK[username]", $username, $time, $this->cookie_path, $this->cookie_domain);
    ...
    function sync ($username, $password='', $md5password='')
    {
        ...
        if (empty($profile))
        {
            ...
            {
               $sql = "INSERT INTO " . $GLOBALS['xak']->table('users').
