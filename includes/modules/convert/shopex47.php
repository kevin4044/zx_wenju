...
class shopex47
{
    ...
    function shopex47(&$sdb, $sprefix, $sroot, $scharset = 'UTF8')
    {
        ...
        if (XAK_CHARSET == 'utf-8')
        {
            $tcharset = 'UTF8';
        }
        elseif (XAK_CHARSET == 'gbk')
        {
            $tcharset = 'GB2312';
        }
    ...
    function process_cat()
    {
        ...
        while ($row = $this->sdb->fetchRow($res))
        {
            ...
            /* 插入分类 */
            if (!$db->autoExecute($xak->table('category'), $cat, 'INSERT', '', 'SILENT'))
            {
                //return $db->error();
            }
        }
        ...
        while ($row = $this->sdb->fetchRow($res))
        {
            ...
            $type['enabled']    = '1';
            if (!$db->autoExecute($xak->table('goods_type'), $type, 'INSERT', '', 'SILENT'))
            {
                //return $db->error();
            }
        }
        ...
        while ($row = $this->sdb->fetchRow($res))
        {
            ...
            if (!$db->autoExecute($xak->table('attribute'), $attr, 'INSERT', '', 'SILENT'))
            {
                 //return $db->error();
            }
        }
    ...
    function process_brand()
    {
        ...
        while ($row = $this->sdb->fetchRow($res))
        {
            $brand = array(
                ...
                'brand_desc' => '',
                'site_url' => xak_iconv($this->scharset, $this->tcharset, addslashes($row['brand_site_url'])),
                'brand_logo' => xak_iconv($this->scharset, $this->tcharset, addslashes($row['brand_logo']))
            );
            if (!$db->autoExecute($xak->table('brand'), $brand, 'INSERT', '', 'SILENT'))
            {
                //return $db->error();
            }
        }
    ...
    function process_goods()
    {
        ...
        /* 查询品牌列表 name => id */
        $brand_list = array();
        $sql = "SELECT brand_id, brand_name FROM " . $xak->table('brand');
        $res = $db->query($sql);
        ...
        while ($row = $this->sdb->fetchRow($res))
        {
            ...
            $goods['goods_sn']      = xak_iconv($this->scharset, $this->tcharset, addslashes($row['bn']));
            $goods['goods_name']    = $row['goods'];
            $goods['brand_id']      = trim($row['brand']) == '' ? '0' : $brand_list[xak_iconv($this->scharset, $this->tcharset, addslashes($row['brand']))];
            ...
            $goods['goods_brief']   = xak_iconv($this->scharset, $this->tcharset, addslashes($row['intro']));
            $goods['goods_desc']    = str_replace('pictures/newsimg/', $this->tdocroot . '/images/upload/Image/', xak_iconv($this->scharset, $this->tcharset, addslashes($row['memo'])));
            ...
            /* 插入 */
            if (!$db->autoExecute($xak->table('goods'), $goods, 'INSERT', '', 'SILENT'))
            {
                //return $db->error();
            }
            ...
            if ($row['linkclass'] != '')
            {
                ...
                foreach ($cat_id_list as $cat_id)
                {
                    ...
                    if (!$db->autoExecute($xak->table('goods_cat'), $goods_cat, 'INSERT', '', 'SILENT'))
                    {
                        //return $db->error();
                    }
                }
            ...
            while ($attr = $this->sdb->fetchRow($res1))
            {
                ...
                $goods_attr['attr_value']   = xak_iconv($this->scharset, $this->tcharset, addslashes($attr['prop_value']));
                $goods_attr['attr_price']   = '0';
                if (!$db->autoExecute($xak->table('goods_attr'), $goods_attr, 'INSERT', '', 'SILENT'))
                {
                    //return $db->error();
                }
            }
            ...
            if ($row['multi_image'])
            {
                ...
                foreach ($img_list as $img)
                {
                    ...
                    if (!$db->autoExecute($xak->table('goods_gallery'), $goods_gallery, 'INSERT', '', 'SILENT'))
                    {
                        //return $db->error();
                    }
                }
        ...
        while ($row = $this->sdb->fetchRow($res))
        {
            ...
            if (!$db->autoExecute($xak->table('link_goods'), $link_goods, 'INSERT', '', 'SILENT'))
            {
                //return $db->error();
            }
        ...
        while ($row = $this->sdb->fetchRow($res))
        {
            ...
            if (!$db->autoExecute($xak->table('group_goods'), $group_goods, 'INSERT', '', 'SILENT'))
            {
                //return $db->error();
            }
        }
    ...
    function process_users()
    {
        ...
        while ($row = $this->sdb->fetchRow($res))
        {
            ...
            $user_rank['rank_name']     = xak_iconv($this->scharset, $this->tcharset, addslashes($row['name']));
            ...
            if (!$db->autoExecute($xak->table('user_rank'), $user_rank, 'INSERT', '', 'SILENT'))
            {
                //return $db->error();
            }
        ...
        while ($row = $this->sdb->fetchRow($res))
        {
            ...
            $user['user_name']      = xak_iconv($this->scharset, $this->tcharset, addslashes($row['user']));
            $user['password']       = $row['password'];
            $user['question']       = xak_iconv($this->scharset, $this->tcharset, addslashes($row['pw_question']));
            $user['answer']         = xak_iconv($this->scharset, $this->tcharset, addslashes($row['pw_answer']));
            $user['sex']            = $row['sex'];
            ...
            if (!$db->autoExecute($xak->table('users'), $user, 'INSERT', '', 'SILENT'))
            {
                //return $db->error();
            }
        ...
        while ($row = $this->sdb->fetchRow($res))
        {
            ...
            $address['address_name']    = xak_iconv($this->scharset, $this->tcharset, addslashes($row['name']));
            $address['user_id']         = $row['memberid'];
            $address['consignee']       = xak_iconv($this->scharset, $this->tcharset, addslashes($row['name']));
            $address['email']           = $row['email'];
            $address['address']         = xak_iconv($this->scharset, $this->tcharset, addslashes($row['address']));
            $address['zipcode']         = $row['zipcode'];
            $address['tel']             = $row['telphone'];
            $address['mobile']          = $row['mobile'];

            if (!$db->autoExecute($xak->table('user_address'), $address, 'INSERT', '', 'SILENT'))
            {
                //return $db->error();
            }
        }
        ...
        while ($row = $this->sdb->fetchRow($res))
        {
            if ($row['gid'] > 0 && $row['levelid'] > 0 && !isset($temp_arr[$row['gid']][$row['levelid']]))
            {
                ...
                if (!$db->autoExecute($xak->table('member_price'), $member_price, 'INSERT', '', 'SILENT'))
                {
                    //return $db->error();
                }
            }
        ...
        while ($row = $this->sdb->fetchRow($res))
        {
            ...
            if (!$db->autoExecute($xak->table('user_account'), $user_account, 'INSERT', '', 'SILENT'))
            {
                //return $db->error();
            }
        }
    ...
    function process_article()
    {
        ...
        while ($row = $this->sdb->fetchRow($res))
        {
            $cat = array();
            $cat['cat_id']      = $row['catid'];
            $cat['cat_name']    = xak_iconv($this->scharset, $this->tcharset, addslashes($row['cat']));
            $cat['cat_type']    = '1';
            $cat['sort_order']  = $row['pid'];
            $cat['is_open']     = '1';

            if (!$db->autoExecute($xak->table('article_cat'), $cat, 'INSERT', '', 'SILENT'))
            {
                //return $db->error();
            }
        }
        ...
        while ($row = $this->sdb->fetchRow($res))
        {
            ...
            $article['title']       = xak_iconv($this->scharset, $this->tcharset, addslashes($row['title']));
            $article['content']     = xak_iconv($this->scharset, $this->tcharset, addslashes($row['con']));
            ...
            if (!$db->autoExecute($xak->table('article'), $article, 'INSERT', '', 'SILENT'))
            {
                //return $db->error();
            }
        }
        ...
        while ($row = $this->sdb->fetchRow($res))
        {
            ...
            $link['link_name']   = xak_iconv($this->scharset, $this->tcharset, addslashes($row['linktitle']));
            ...
            if (!$db->autoExecute($xak->table('friend_link'), $link, 'INSERT', '', 'SILENT'))
            {
                //return $db->error();
            }
        }
    ...
    function process_order()
    {
        ...
        while ($row = $this->sdb->fetchRow($res))
        {
            ...
            $order['consignee']         = xak_iconv($this->scharset, $this->tcharset, addslashes($row['name']));
            $order['address']           = xak_iconv($this->scharset, $this->tcharset, addslashes($row['addr']));
            $order['zipcode']           = $row['zip'];
            $order['tel']               = $row['tel'];
            $order['mobile']            = $row['mobile'];
            $order['email']             = $row['email'];
            $order['postscript']        = xak_iconv($this->scharset, $this->tcharset, addslashes($row['memo']));
            $order['shipping_name']     = is_null($row['tmethod']) ? ' ' : xak_iconv($this->scharset, $this->tcharset, addslashes($row['tmethod']));
            $order['pay_name']          = is_null($row['payment']) ? ' ' : xak_iconv($this->scharset, $this->tcharset, addslashes($row['payment']));
            $order['inv_payee']         = xak_iconv($this->scharset, $this->tcharset, addslashes($row['invoiceform']));
            ...
            while ($row = $this->sdb->fetchRow($res1))
            {
                ...
                $goods['goods_id']      = $row['gid'];
                $goods['goods_name']    = xak_iconv($this->scharset, $this->tcharset, addslashes($row['goods']));
                $goods['goods_sn']      = xak_iconv($this->scharset, $this->tcharset, addslashes($row['bn']));
                ...
                if (!$db->autoExecute($xak->table('order_goods'), $goods, 'INSERT', '', 'SILENT'))
                {
                    //return $db->error();
                }
            }
    ...
    function process_config()
    {
        ...
        $config = array();
        $config['shop_name']        = xak_iconv($this->scharset, $this->tcharset, addslashes($row['offer_name']));
        $config['shop_title']       = xak_iconv($this->scharset, $this->tcharset, addslashes($row['offer_shoptitle']));
        $config['shop_desc']        = xak_iconv($this->scharset, $this->tcharset, addslashes($row['offer_metadesc']));
        $config['shop_address']     = xak_iconv($this->scharset, $this->tcharset, addslashes($row['offer_addr']));
        $config['service_email']    = $row['offer_email'];
        $config['service_phone']    = $row['offer_tel'];
        $config['icp_number']       = xak_iconv($this->scharset, $this->tcharset, addslashes($row['offer_certtext']));
        ...
        {
            $sql = "UPDATE " . $xak->table('shop_config') . " SET " .
                    "value = '$value' " .
