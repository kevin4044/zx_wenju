...
class shopex46
{
    ...
    function process_cat()
    {
        ...
        while ($row = $this->sdb->fetchRow($res))
        {
            ...
            if ($has_attr)
            {
                if (!$db->autoExecute($xak->table('goods_type'), $cat, 'INSERT', '', 'SILENT'))
                {
                    //return $db->error();
                }
            }
            ...
            for ($i = 1; $i <= 40; $i++)
            {
                if (trim($row["attr".$i]) != '')
                {
                    $attr['attr_name']  = xak_iconv($this->scharset, $this->tcharset, $row["attr".$i]);
                    $attr['sort_order'] = $i;
    ...
    function process_brand()
    {
        ...
        {
            $brand = array(
                'brand_name' => xak_iconv($this->scharset, $this->tcharset, addslashes($row['brand'])),
                'brand_desc' => '',
            );
    ...
    function process_goods()
    {
        ...
        while ($row = $this->sdb->fetchRow($res))
        {
            ...
            $goods['goods_desc']    = str_replace('pictures/newsimg/', $this->tdocroot . '/images/upload/', xak_iconv($this->scharset, $this->tcharset, addslashes($row['memo'])));
            $goods['is_real']       = '1';
            ...
            /* 取得该分类的所有属性 */
            $attr_list = array();
            $sql = "SELECT * FROM " . $xak->table('attribute') . " WHERE cat_id = '$row[catid]'";
            $res1 = $db->query($sql);
            ...
            if ($attr_list)
            {
                ...
                for ($i = 1; $i <= 40; $i++)
                {
                    if (trim($row['attr' . $i]) != '')
                    {
                        ...
                        $goods_attr['attr_value'] = trim(xak_iconv($this->scharset, $this->tcharset, $row['attr' . $i]));
