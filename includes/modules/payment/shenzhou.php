...
class shenzhou
{
   ...
   /** 生成支付代码
    ...
    function get_code($order, $payment)
    {
        ...
        $bg_url             = '';
        $page_url           = $GLOBALS['xak']->url() . 'respond.php';
        $version            = 'v2.0';
        $language           = 1;
    ...
    function respond()
    {
        ...
        if (strtoupper($sign_msg) == strtoupper($merchant_signmsg))
        {
            ...
            {
                $sql = "SELECT order_amount FROM " . $GLOBALS['xak']->table('order_info') ."WHERE order_id = '$order_id'";
                ...
                elseif ($get_order_amount == $order_amount && $pay_amount > 0) //订单金额相等 实际支付金额 > 0的情况
                {
                    ...
                    $sql = 'UPDATE' . $GLOBALS['xak']->table('order_info') . "SET `money_paid` = (money_paid  + '$pay_amount')," .
                        " order_amount = (order_amount - '$pay_amount') WHERE order_id = '$order_id'";
                    $result = $GLOBALS['db']->query($sql);
                    $sql = 'UPDATE' . $GLOBALS['xak']->table('order_info') . "SET `order_status` ='" . OS_CONFIRMED . "' WHERE order_id = '$orderId'";
                    $result = $GLOBALS['db']->query($sql);
