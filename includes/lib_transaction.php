...
function edit_profile($profile)
{
    ...
    $cfg = array();
    $cfg['username'] = $GLOBALS['db']->getOne("SELECT user_name FROM " . $GLOBALS['xak']->table('users') . " WHERE user_id='" . $profile['user_id'] . "'");
    ...
    {
        $GLOBALS['db']->autoExecute($GLOBALS['xak']->table('users'), $profile['other'], 'UPDATE', "user_id = '$profile[user_id]'");
    }
...
function get_profile($user_id)
{
    ...
    $sql  = "SELECT user_name, birthday, sex, question, answer, rank_points, pay_points,user_money, user_rank,".
           ...
           "FROM " .$GLOBALS['xak']->table('users') . " WHERE user_id = '$user_id'";
    ...
    {
        $sql = "SELECT rank_id, rank_name, discount FROM ".$GLOBALS['xak']->table('user_rank') .
               " WHERE rank_id = '$infos[user_rank]'";
    }
    else
    {
        $sql = "SELECT rank_id, rank_name, discount, min_points".
               " FROM ".$GLOBALS['xak']->table('user_rank') .
    ...
    $sql = "SELECT type_name, type_money ".
           "FROM " .$GLOBALS['xak']->table('bonus_type') . " AS t1, " .$GLOBALS['xak']->table('user_bonus') . " AS t2 ".
...
function add_bonus($user_id, $bouns_sn)
{
    ...
    /* 查询红包序列号是否已经存在 */
    $sql = "SELECT bonus_id, bonus_sn, user_id, bonus_type_id FROM " .$GLOBALS['xak']->table('user_bonus') .
    ...
    if ($row)
    {
        if ($row['user_id'] == 0)
        {
            ...
            $sql = "UPDATE " .$GLOBALS['xak']->table('user_bonus') . " SET user_id = '$user_id' ".
...
{
    /* 查询订单信息，检查状态 */
    $sql = "SELECT user_id, order_id, order_sn , surplus , integral , bonus_id, order_status, shipping_status, pay_status FROM " .$GLOBALS['xak']->table('order_info') ." WHERE order_id = '$order_id'";
    ...
    //将用户订单设置为取消
    $sql = "UPDATE ".$GLOBALS['xak']->table('order_info') ." SET order_status = '".OS_CANCELED."' WHERE order_id = '$order_id'";
...
{
    /* 查询订单信息，检查状态 */
    $sql = "SELECT user_id, order_sn , order_status, shipping_status, pay_status FROM ".$GLOBALS['xak']->table('order_info') ." WHERE order_id = '$order_id'";
    ...
    {
        $sql = "UPDATE " . $GLOBALS['xak']->table('order_info') . " SET shipping_status = '" . SS_RECEIVED . "' WHERE order_id = '$order_id'";
        if ($GLOBALS['db']->query($sql))
        {
            /* 记录日志 */
...
function save_consignee($consignee, $default=false)
{
    ...
    {
        /* 修改地址 */
        $res = $GLOBALS['db']->autoExecute($GLOBALS['xak']->table('user_address'), $consignee, 'UPDATE', 'address_id = ' . $consignee['address_id']." AND `user_id`= '".$_SESSION['user_id']."'");
    }
    else
    {
        /* 添加地址 */
        $res = $GLOBALS['db']->autoExecute($GLOBALS['xak']->table('user_address'), $consignee, 'INSERT');
...
{
    $sql = "SELECT user_id FROM " .$GLOBALS['xak']->table('user_address') . " WHERE address_id = '$id'";
    ...
    {
        $sql = "DELETE FROM " .$GLOBALS['xak']->table('user_address') . " WHERE address_id = '$id'";
        $res = $GLOBALS['db']->query($sql);
...
function update_address($address)
{
    ...
    {
         /* 更新指定记录 */
        $GLOBALS['db']->autoExecute($GLOBALS['xak']->table('user_address'), $address, 'UPDATE', 'address_id = ' .$address_id . ' AND user_id = ' . $address['user_id']);
    }
    else
    {
        /* 插入一条新记录 */
        $GLOBALS['db']->autoExecute($GLOBALS['xak']->table('user_address'), $address, 'INSERT');
...
function get_order_detail($order_id, $user_id = 0)
{
    ...
    {
         $shipping_code = $GLOBALS['db']->GetOne("SELECT shipping_code FROM ".$GLOBALS['xak']->table('shipping') ." WHERE shipping_id = '$order[shipping_id]'");
    ...
    if ($order['pay_status'] != PS_UNPAYED)
    {
        ...
        foreach ($virtual_goods AS $code => $goods_list)
        {
            ...
            if ($code == 'package_buy')
            {
                foreach ($goods_list as $goods)
                {
                    $sql = 'SELECT g.goods_id FROM ' . $GLOBALS['xak']->table('package_goods') . ' AS pg, ' . $GLOBALS['xak']->table('goods') . ' AS g ' .
...
{
    include_once(ROOT_PATH . 'includes/lib_order.php');
    $sql  = "SELECT order_sn FROM ".$GLOBALS['xak']->table('order_info') .
...
function merge_user_order($from_order, $to_order, $user_id = 0)
{
    if ($user_id > 0)
    {
        ...
        {
            $sql = "SELECT user_id FROM " .$GLOBALS['xak']->table('order_info').
...
function return_to_cart($order_id)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        if($row['product_id'])
        {
            ...
            $sql="SELECT product_number from ".$GLOBALS['xak']->table('products')."where product_id='$order_goods_product_id'";
        ...
        if (empty($cart_goods))
        {
            ...
            $GLOBALS['db']->autoExecute($GLOBALS['xak']->table('cart'), $return_goods, 'INSERT');
        }
        else
        {
            // 有相同商品，修改数量
            $sql = "UPDATE " . $GLOBALS['xak']->table('cart') . " SET " .
...
function save_order_address($address, $user_id)
{
    ...
    /* 检查订单状态 */
    $sql = "SELECT user_id, order_status FROM " .$GLOBALS['xak']->table('order_info'). " WHERE order_id = '" .$address['order_id']. "'";
    $row = $GLOBALS['db']->getRow($sql);
    if ($row)
    {
        ...
        $GLOBALS['db']->autoExecute($GLOBALS['xak']->table('order_info'), $address, 'UPDATE', "order_id = '$address[order_id]'");
        return true;
    }
    else
    {
...
function get_user_bouns_list($user_id, $num = 10, $start = 0)
{
    $sql = "SELECT u.bonus_sn, u.order_id, b.type_name, b.type_money, b.min_goods_amount, b.use_start_date, b.use_end_date ".
           " FROM " .$GLOBALS['xak']->table('user_bonus'). " AS u ,".
