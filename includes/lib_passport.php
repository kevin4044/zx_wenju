...
function register($username, $password, $email, $other = array())
{
    ...
    else
    {
        ...
        if (isset($affiliate['on']) && $affiliate['on'] == 1)
        {
            ...
            if ($up_uid)
            {
                if (!empty($affiliate['config']['level_register_all']))
                {
                    ...
                    {
                        $rank_points = $GLOBALS['db']->getOne("SELECT rank_points FROM " . $GLOBALS['xak']->table('users') . " WHERE user_id = '$up_uid'");
                ...
                //设置推荐人
                $sql = 'UPDATE '. $GLOBALS['xak']->table('users') . ' SET parent_id = ' . $up_uid . ' WHERE user_id = ' . $_SESSION['user_id'];
        ...
        $GLOBALS['db']->autoExecute($GLOBALS['xak']->table('users'), $update_data, 'UPDATE', 'user_id = ' . $_SESSION['user_id']);
...
function check_userinfo($user_name, $email)
{
    ...
    {
        xak_header("Location: user.php?act=get_password\n");
...
function send_pwd_email($uid, $user_name, $email, $code)
{
    ...
    $reset_email = $GLOBALS['xak']->url() . 'user.php?act=get_password&uid=' . $uid . '&code=' . $code;
...
function send_regiter_hash ($user_id)
{
    ...
    $validate_email = $GLOBALS['xak']->url() . 'user.php?act=validate_email&hash=' . $hash;

    $sql = "SELECT user_name, email FROM " . $GLOBALS['xak']->table('users') . " WHERE user_id = '$user_id'";
...
{
    $res = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['xak']->table('admin_user') .
