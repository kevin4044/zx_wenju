...
{
    $sql = 'SELECT region_id, region_name FROM ' . $GLOBALS['xak']->table('region') .
...
{
    /* 获得配置信息 */
    $sql = 'SELECT configure FROM ' . $GLOBALS['xak']->table('shipping_area') . " WHERE shipping_area_id = '$area_id'";
    ...
    if ($res === NULL)
    {
        ...
        if ($data === false)
        {
            ...
            $sql = "SELECT gc.cat_id, COUNT(*) AS goods_num " .
                    " FROM " . $GLOBALS['xak']->table('goods_cat') . " AS gc , " . $GLOBALS['xak']->table('goods') . " AS g " .
...
function load_config()
{
    ...
    if ($data === false)
    {
        $sql = 'SELECT code, value FROM ' . $GLOBALS['xak']->table('shop_config') . ' WHERE parent_id > 0';
        $res = $GLOBALS['db']->getAll($sql);
        ...
        if (!isset($GLOBALS['_CFG']['xak_version']))
        {
            /* 如果没有版本号则默认为2.0.5 */
            $GLOBALS['_CFG']['xak_version'] = 'v2.0.5';
        }
        ...
        {
            $arr['integrate_code'] = 'xakshop'; // 默认的会员整合插件为 xakshop
        }
...
{
    $sql = 'SELECT brand_id, brand_name FROM ' . $GLOBALS['xak']->table('brand') . ' ORDER BY sort_order';
    $res = $GLOBALS['db']->getAll($sql);
...
function get_brands($cat = 0, $app = 'brand')
{
    ...
    $sql = "SELECT b.brand_id, b.brand_name, b.brand_logo, b.brand_desc, COUNT(*) AS goods_num, IF(b.brand_logo > '', '1', '0') AS tag ".
            "FROM " . $GLOBALS['xak']->table('brand') . "AS b, ".
...
function get_promotion_info($goods_id = '')
{
    ...
    $gmtime = gmtime();
    $sql = 'SELECT act_id, act_name, act_type, start_time, end_time FROM ' . $GLOBALS['xak']->table('goods_activity') . " WHERE is_finished=0 AND start_time <= '$gmtime' AND end_time >= '$gmtime'";
    if(!empty($goods_id))
    {
    ...
    $favourable = array();
    $sql = 'SELECT act_id, act_range, act_range_ext, act_name, start_time, end_time FROM ' . $GLOBALS['xak']->table('favourable_activity') . " WHERE start_time <= '$gmtime' AND end_time >= '$gmtime'";
    if(!empty($goods_id))
    {
    ...
    {
        $sql = "SELECT cat_id, brand_id FROM " . $GLOBALS['xak']->table('goods') .
...
{
    $sql = 'SELECT template_subject, is_html, template_content FROM ' . $GLOBALS['xak']->table('mail_templates') . " WHERE template_code = '$tpl_name'";
...
function order_action($order_sn, $order_status, $shipping_status, $pay_status, $note = '', $username = null, $place = 0)
{
    ...
    $sql = 'INSERT INTO ' . $GLOBALS['xak']->table('order_action') .
            ...
            'FROM ' . $GLOBALS['xak']->table('order_info') . " WHERE order_sn = '$order_sn'";
...
function price_format($price, $change_price = true)
{
    ...
    if ($change_price && defined('XAK_ADMIN') === false)
    {
...
function virtual_card_shipping ($goods, $order_sn, &$msg, $process = 'other')
{
    ...
    /* 检查有没有缺货 */
    $sql = "SELECT COUNT(*) FROM ".$GLOBALS['xak']->table('virtual_card')." WHERE goods_id = '$goods[goods_id]' AND is_saled = 0 ";
    ...
    if ($num < $goods['num'])
    {
     ...
     /* 取出卡片信息 */
     $sql = "SELECT card_id, card_sn, card_password, end_date, crc32 FROM ".$GLOBALS['xak']->table('virtual_card')." WHERE goods_id = '$goods[goods_id]' AND is_saled = 0  LIMIT " . $goods['num'];
    ...
    /* 更新库存 */
    $sql = "UPDATE ".$GLOBALS['xak']->table('goods'). " SET goods_number = goods_number - '$goods[num]' WHERE goods_id = '$goods[goods_id]'";
    $GLOBALS['db']->query($sql);
...
function virtual_card_result($order_sn, $goods)
{
    ...
    /* 获取已经发送的卡片数据 */
    $sql = "SELECT card_sn, card_password, end_date, crc32 FROM ".$GLOBALS['xak']->table('virtual_card')." WHERE goods_id= '$goods[goods_id]' AND order_sn = '$order_sn' ";
...
function get_snatch_result($id)
{
    $sql = 'SELECT u.user_id, u.user_name, u.email, lg.bid_price, lg.bid_time, count(*) as num' .
            ' FROM ' . $GLOBALS['xak']->table('snatch_log') . ' AS lg '.
            ' LEFT JOIN ' . $GLOBALS['xak']->table('users') . ' AS u ON lg.user_id = u.user_id'.
    ...
    if ($rec)
    {
        ...
        /* 活动信息 */
        $sql = 'SELECT ext_info " .
               " FROM ' . $GLOBALS['xak']->table('goods_activity') .
...
function build_uri($app, $params, $append = '', $page = 0, $keywords = '', $size = 0)
{
    ...
    if (($rewrite == 2) && (strpos(strtolower(XAK_CHARSET), 'utf') !== 0))
    {
        $uri = urlencode($uri);
    }
    return $uri;
...
function log_account_change($user_id, $user_money = 0, $frozen_money = 0, $rank_points = 0, $pay_points = 0, $change_desc = '', $change_type = ACT_OTHER)
{
    ...
    $GLOBALS['db']->autoExecute($GLOBALS['xak']->table('account_log'), $account_log, 'INSERT');
    ...
    if ($res === NULL)
    {
        ...
        if ($data === false)
        {
            $sql = "SELECT c.*, COUNT(s.cat_id) AS has_children, COUNT(a.article_id) AS aricle_num ".
               ' FROM ' . $GLOBALS['xak']->table('article_cat') . " AS c".
               " LEFT JOIN " . $GLOBALS['xak']->table('article_cat') . " AS s ON s.parent_id=c.cat_id".
               " LEFT JOIN " . $GLOBALS['xak']->table('article') . " AS a ON a.cat_id=c.cat_id".
               " GROUP BY c.cat_id ".
...
function get_volume_price_list($goods_id, $price_type = '1')
{
    ...
    $sql = "SELECT `volume_number` , `volume_price`".
           " FROM " .$GLOBALS['xak']->table('volume_price'). "".
...
function get_package_info($id)
{
    ...
    $sql = "SELECT pg.package_id, pg.goods_id, pg.goods_number, pg.admin_id, ".
           ...
           "   LEFT JOIN ". $GLOBALS['xak']->table('goods') . " AS g ".
...
function get_package_goods($package_id)
{
    ...
    if ($good_product_str != '')
    {
        $sql = "SELECT goods_attr_id, attr_value FROM " .$GLOBALS['xak']->table('goods_attr'). " WHERE goods_id IN ($good_product_str)";
...
function get_good_products($goods_id, $conditions = '')
{
    ...
    /* 取货品 */
    $sql = "SELECT * FROM " .$GLOBALS['xak']->table('products'). " WHERE $_goods_id $conditions";
    $result_products = $GLOBALS['db']->getAll($sql);

    /* 取商品属性 */
    $sql = "SELECT goods_attr_id, attr_value FROM " .$GLOBALS['xak']->table('goods_attr'). " WHERE $_goods_id";
    $result_goods_attr = $GLOBALS['db']->getAll($sql);
