...
function sub_str($str, $length = 0, $append = true)
{
    ...
    {
        $newstr = mb_substr($str, 0, $length, XAK_CHARSET);
    }
    elseif (function_exists('iconv_substr'))
    {
        $newstr = iconv_substr($str, 0, $length, XAK_CHARSET);
    }
    else
    {
...
{
    /* 如果邮件编码不是XAK_CHARSET，创建字符集转换对象，转换编码 */
    if ($GLOBALS['_CFG']['mail_charset'] != XAK_CHARSET)
    {
        $name      = xak_iconv(XAK_CHARSET, $GLOBALS['_CFG']['mail_charset'], $name);
        $subject   = xak_iconv(XAK_CHARSET, $GLOBALS['_CFG']['mail_charset'], $subject);
        $content   = xak_iconv(XAK_CHARSET, $GLOBALS['_CFG']['mail_charset'], $content);
        $shop_name = xak_iconv(XAK_CHARSET, $GLOBALS['_CFG']['mail_charset'], $GLOBALS['_CFG']['shop_name']);
    }
 ...
 * @return  void
 **/
function xak_header($string, $replace = true, $http_response_code = 0)
{
...
function xak_iconv($source_lang, $target_lang, $source_string = '')
{
    static $chs = NULL;
...
function json_str_iconv($str)
{
    if (XAK_CHARSET != 'utf-8')
    {
        if (is_string($str))
        {
            return addslashes(stripslashes(xak_iconv('utf-8', XAK_CHARSET, $str)));
        }
        elseif (is_array($str))
        {
...
function to_utf8_iconv($str)
{
    if (XAK_CHARSET != 'utf-8')
    {
        if (is_string($str))
        {
            return xak_iconv(XAK_CHARSET, 'utf-8', $str);
        }
        elseif (is_array($str))
        {
            foreach ($str as $key => $value)
