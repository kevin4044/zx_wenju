...
/** LICENSE 相关函数库
 * ----------------------------------------------------------------------------
 * $File: lib_article.php 16336 2009-06-24 07:09:13
*/
...
function license_login($certi_added = '')
{
    ...
    $certi['certi_url'] = sprintf($GLOBALS['xak']->url()); // 网店URL
    $certi['certi_session'] = $GLOBALS['sess']->get_session_id(); // 网店SESSION标识
    $certi['certi_validate_url'] = sprintf($GLOBALS['xak']->url() . 'certi.php'); // 网店提供于官方反查接口
...
function license_reg($certi_added = '')
{
    ...
    {
        // 注册信息入库
        $sql = "UPDATE " . $GLOBALS['xak']->table('shop_config') . "
