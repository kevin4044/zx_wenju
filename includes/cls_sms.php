...
class sms
{
    ...
    {
        $sql = 'SELECT `value`
                FROM '.$this->xak->table('shop_config')." WHERE `code` = 'ent_id'";
    ...
    {
        $sql = 'SELECT `email` FROM ' . $this->xak->table('admin_user') . " WHERE `user_id` = '" . $_SESSION['admin_id'] . "'";
