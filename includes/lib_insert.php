...
function insert_query_info()
{
    ...
    $online_count = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['xak']->table('sessions'));
...
function insert_history()
{
    ...
    {
        $where = db_create_in($_COOKIE['XAK']['history'], 'goods_id');
        $sql   = 'SELECT goods_id, goods_name, goods_thumb, shop_price FROM ' . $GLOBALS['xak']->table('goods') .
        ...
        while ($row = $GLOBALS['db']->fetch_array($query))
        {
            ...
            $str.='<ul class="clearfix"><li class="goodsimg"><a href="'.$goods['url'].'" target="_blank"><img src="'.$goods['goods_thumb'].'" alt="'.$goods['goods_name'].'" class="B_blue" /></a></li><li><a href="'.$goods['url'].'" target="_blank" title="'.$goods['goods_name'].'">'.$goods['short_name'].'</a> <font class="f1">'.$goods['shop_price'].'</font><br /></li></ul>';
        }
...
function insert_ads($arr)
{
    ...
    if (!empty($arr['num']) && $arr['num'] != 1)
    {
        $sql  = 'SELECT a.ad_id, a.position_id, a.media_type, a.ad_link, a.ad_code, a.ad_name, p.ad_width, ' .
                ...
                'FROM ' . $GLOBALS['xak']->table('ad') . ' AS a '.
                'LEFT JOIN ' . $GLOBALS['xak']->table('ad_position') . ' AS p ON a.position_id = p.position_id ' .
...
function insert_member_info()
{
    ...
    {
        if (!empty($_COOKIE['XAK']['username']))
        {
            $GLOBALS['smarty']->assign('xsp_username', stripslashes($_COOKIE['XAK']['username']));
        }
...
function insert_bought_notes($arr)
{
    ...
    $sql = 'SELECT u.user_name, og.goods_number, oi.add_time, IF(oi.order_status IN (2, 3, 4), 0, 1) AS order_status ' .
           'FROM ' . $GLOBALS['xak']->table('order_info') . ' AS oi LEFT JOIN ' . $GLOBALS['xak']->table('users') . ' AS u ON oi.user_id = u.user_id, ' . $GLOBALS['xak']->table('order_goods') . ' AS og ' .
