...
function add_feed($id, $feed_type)
{
    ...
    if ($feed_type == BUY_GOODS)
    {
        ...
        $id = intval($id);
        $order_res = $GLOBALS['db']->getAll("SELECT g.goods_id, g.goods_name, g.goods_sn, g.goods_desc, g.goods_thumb, o.goods_price FROM " . $GLOBALS['xak']->table('order_goods') . " AS o, " . $GLOBALS['xak']->table('goods') . " AS g WHERE o.order_id='{$id}' AND o.goods_id=g.goods_id");
        foreach($order_res as $goods_data)
        {
            if(!empty($goods_data['goods_thumb']))
            {
                $url = $GLOBALS['xak']->url() . $goods_data['goods_thumb'];
            }
            else
            {
                $url = $GLOBALS['xak']->url() . $GLOBALS['_CFG']['no_picture'];
            }
            $link = $GLOBALS['xak']->url() . "goods.php?id=" . $goods_data["goods_id"];
