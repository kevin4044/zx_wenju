...
function update_user_info()
{
    ...
    $sql = 'SELECT u.user_money,u.email, u.pay_points, u.user_rank, u.rank_points, '.
            ...
            ' FROM ' .$GLOBALS['xak']->table('users'). ' AS u ' .
            ' LEFT JOIN ' .$GLOBALS['xak']->table('user_bonus'). ' AS ub'.
            ' ON ub.user_id = u.user_id AND ub.used_time = 0 ' .
            ' LEFT JOIN ' .$GLOBALS['xak']->table('bonus_type'). ' AS b'.
    ...
    if ($row = $GLOBALS['db']->getRow($sql))
    {
        ...
        if($row['user_rank'] >0)
        {
            $sql="SELECT special_rank from ".$GLOBALS['xak']->table('user_rank')."where rank_id='$row[user_rank]'";
            if($GLOBALS['db']->getOne($sql)==='0' || $GLOBALS['db']->getOne($sql)===null)
            {   
                $sql="update ".$GLOBALS['xak']->table('users')."set user_rank='0' where user_id='$_SESSION[user_id]'";
        ...
        if ($row['user_rank'] == 0)
        {
            ...
            $sql = 'SELECT rank_id, discount FROM ' . $GLOBALS['xak']->table('user_rank') . " WHERE special_rank = '0' AND min_points <= " . intval($row['rank_points']) . ' AND max_points > ' . intval($row['rank_points']);
            if ($row = $GLOBALS['db']->getRow($sql))
            {
        ...
        {
            // 特殊等级
            $sql = 'SELECT rank_id, discount FROM ' . $GLOBALS['xak']->table('user_rank') . " WHERE rank_id = '$row[user_rank]'";
            if ($row = $GLOBALS['db']->getRow($sql))
            {
                $_SESSION['user_rank'] = $row['rank_id'];
                $_SESSION['discount']  = $row['discount'] / 100.00;
    ...
    /* 更新登录时间，登录次数及登录ip */
    $sql = "UPDATE " .$GLOBALS['xak']->table('users'). " SET".
...
function get_parent_cats($cat)
{
    ...
    $arr = $GLOBALS['db']->GetAll('SELECT cat_id, cat_name, parent_id FROM ' . $GLOBALS['xak']->table('category'));
...
{
    $sql = 'SELECT id, number, type FROM ' . $GLOBALS['xak']->table('template') .
...
{
    $sql = 'SELECT cat_name FROM ' . $GLOBALS['xak']->table('article_cat') . " WHERE cat_id = '" . $id ."'";
...
function get_shop_help()
{
    $sql = 'SELECT c.cat_id, c.cat_name, c.sort_order, a.article_id, a.title, a.file_url, a.open_type ' .
            ...
            'LEFT JOIN ' .$GLOBALS['xak']->table('article_cat'). ' AS c ' .
...
function get_vote($id = '')
{
    ...
    if ($vote_arr !== false && !empty($vote_arr))
    {
        ...
        $sql_option = 'SELECT v.*, o.option_id, o.vote_id, o.option_name, o.option_count ' .
                      'FROM ' . $GLOBALS['xak']->table('vote') . ' AS v, ' .
                            $GLOBALS['xak']->table('vote_option') . ' AS o ' .
        ...
        /* 总票数 */
        $sql = 'SELECT SUM(option_count) AS all_option FROM ' . $GLOBALS['xak']->table('vote_option') .
...
function is_spider($record = true)
{
    ...
    foreach ($searchengine_bot AS $key => $value)
    {
        if (strpos($spider, $value) !== false)
        {
            ...
            if ($record === true)
            {
                $GLOBALS['db']->autoReplace($GLOBALS['xak']->table('searchengine'), array('date' => local_date('Y-m-d'), 'searchengine' => $spider, 'count' => 1), array('count' => 1));
            }
...
function visit_stats()
{
    ...
    $visit_times = (!empty($_COOKIE['XAK']['visit_times'])) ? intval($_COOKIE['XAK']['visit_times']) + 1 : 1;
    setcookie('XAK[visit_times]', $visit_times, $time + 86400 * 365, '/');
    ...
    $sql = 'INSERT INTO ' . $GLOBALS['xak']->table('stats') . ' ( ' .
...
function save_searchengine_keyword($domain, $path)
{
    ...
    if (!empty($keywords))
    {
        ...
        if (XAK_CHARSET == 'utf-8' && in_array($searchengine, $gb_search))
        {
            $keywords = xak_iconv('GBK', 'UTF8', $keywords);
        }
        if (XAK_CHARSET == 'gbk' && !in_array($searchengine, $gb_search))
        {
            $keywords = xak_iconv('UTF8', 'GBK', $keywords);
        }

        $GLOBALS['db']->autoReplace($GLOBALS['xak']->table('keywords'), array('date' => local_date('Y-m-d'), 'searchengine' => $searchengine, 'keyword' => addslashes($keywords), 'count' => 1), array('count' => 1));
    }
}
...
function get_dyna_libs($theme, $tmp)
{
    ...
    $sql = 'SELECT region, library, sort_order, id, number, type' .
            ' FROM ' . $GLOBALS['xak']->table('template') .
...
function recalculate_price()
{
    ...
    foreach ($res AS $row)
    {
        ...
        $goods_sql = "UPDATE " .$GLOBALS['xak']->table('cart'). " SET goods_price = '$goods_price' ".
    ...
    /* 删除赠品，重新选择 */
    $GLOBALS['db']->query('DELETE FROM ' . $GLOBALS['xak']->table('cart') .
...
{
    /* 取得评论列表 */
    $count = $GLOBALS['db']->getOne('SELECT COUNT(*) FROM ' .$GLOBALS['xak']->table('comment').
    ...
    $sql = 'SELECT * FROM ' . $GLOBALS['xak']->table('comment') .
            " WHERE id_value = '$id' AND comment_type = '$type' AND status = 1 AND parent_id = 0".
...
function get_user_bonus($user_id = 0)
{
    ...
    $sql = "SELECT SUM(bt.type_money) AS bonus_value, COUNT(*) AS bonus_count ".
            "FROM " .$GLOBALS['xak']->table('user_bonus'). " AS ub, ".
                $GLOBALS['xak']->table('bonus_type') . " AS bt ".
...
function set_affiliate()
{
    ...
    if (!empty($_GET['u']) && $config['on'] == 1)
    {
        if(!empty($config['config']['expire']))
        {
            ...
            setcookie('xakhop_affiliate_uid', intval($_GET['u']), gmtime() + 3600 * $config['config']['expire'] * $c);
        }
        else
        {
            setcookie('xakhop_affiliate_uid', intval($_GET['u']), gmtime() + 3600 * 24); // 过期时间为 1 天
        }
    }
}
...
{
    if (!empty($_COOKIE['xakhop_affiliate_uid']))
    {
        $uid = intval($_COOKIE['xakhop_affiliate_uid']);
        if ($GLOBALS['db']->getOne('SELECT user_id FROM ' . $GLOBALS['xak']->table('users') . "WHERE user_id = '$uid'"))
        {
            return $uid;
        }
        else
        {
            setcookie('xakhop_affiliate_uid', '', 1);
        }
    }
...
function article_categories_tree($cat_id = 0)
{
    if ($cat_id > 0)
    {
        $sql = 'SELECT parent_id FROM ' . $GLOBALS['xak']->table('article_cat') . " WHERE cat_id = '$cat_id'";
    ...
    /*
     ...
     如果不是取当前分类及其下的子分类
    */
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['xak']->table('article_cat') . " WHERE parent_id = '$parent_id'";
    if ($GLOBALS['db']->getOne($sql))
    {
        /* 获取当前分类及其子分类 */
        $sql = 'SELECT a.cat_id, a.cat_name, a.sort_order AS parent_order, a.cat_id, ' .
                    'b.cat_id AS child_id, b.cat_name AS child_name, b.sort_order AS child_order ' .
                'FROM ' . $GLOBALS['xak']->table('article_cat') . ' AS a ' .
                'LEFT JOIN ' . $GLOBALS['xak']->table('article_cat') . ' AS b ON b.parent_id = a.cat_id ' .
...
function get_article_parent_cats($cat)
{
    ...
    $arr = $GLOBALS['db']->GetAll('SELECT cat_id, cat_name, parent_id FROM ' . $GLOBALS['xak']->table('article_cat'));
...
function get_library_number($library, $template = null)
{
    ...
    if (!isset($lib_list[$template]))
    {
        ...
        $sql = "SELECT library, number FROM " . $GLOBALS['xak']->table('template') .
...
{
    $sql = 'SELECT * FROM '. $GLOBALS['xak']->table('nav') . '
...
function license_info()
{
    if($GLOBALS['_CFG']['licensed'] > 0)
    {
        ...
        $license = '<a href="http://www.xak.cn/license.php?product=xakshop_b2c&url=' . urlencode($host) . '" target="_blank"
>&nbsp;&nbsp;Licensed</a>';
        return $license;
    }
    else
