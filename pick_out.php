...
if (empty($cat_id))
{
    ...
    $sql = "SELECT DISTINCT t.cat_id, t.cat_name " .
            "FROM " . $xak->table('goods_type') . " AS t, " . $xak->table('attribute') . " AS a, " . $xak->table('goods_attr') . " AS g " .
    ...
    $sql = "SELECT DISTINCT a.attr_id ".
            "FROM ".$xak->table('goods_attr')." AS g, ".$xak->table('attribute') ." AS a ".
    ...
    /* 获取商品总数 */
    $goods_count = $db->GetOne("SELECT COUNT(DISTINCT(goods_id)) FROM " . $xak->table('goods_attr'));
    /* 获取符合条件的商品id */
    //$sql = "SELECT DISTINCT goods_id FROM " .$xak->table('goods_attr'). " LIMIT 100";
    $sql = "SELECT DISTINCT goods_id FROM " .$xak->table('goods_attr');
...
{
    /* 取得商品类型名称 */
    $sql = "SELECT cat_name FROM ".$xak->table('goods_type')." WHERE cat_id = '$cat_id'";
    ...
    if (!empty($_GET['attr']))
    {
        ...
        foreach ($_GET['attr'] AS $key => $value)
        {
            ...
            if ($i > 0)
            {
                ...
                $goods_result = $db->getCol("SELECT goods_id FROM " . $xak->table("goods_attr") . " WHERE goods_id IN (" . implode(',' , $goods_result) . ") AND attr_id='$key' AND attr_value='$value'");
            }
            else
            {
                $goods_result = $db->getCol("SELECT goods_id FROM " . $xak->table("goods_attr") . " WHERE attr_id='$key' AND attr_value='$value'");
            }
            $i++;
        }

        /* 获取指定attr_id的名字 */
        $sql = "SELECT attr_id, attr_name FROM ".$xak->table('attribute')." WHERE attr_id ".db_create_in(implode(',',$attr_picks));
        $rs = $db->query($sql);
        while ($row = $db->fetchRow($rs))
        {
    ...
    else
    {
        ...
        /* 查出数量 */
        $goods_count = $db->GetOne("SELECT COUNT(distinct(g.goods_id)) FROM ".$xak->table('goods_attr')." AS g, ".$xak->table('attribute')." AS a WHERE g.attr_id = a.attr_id AND a.cat_id = '$cat_id' ");

        /* 防止结果过大，最多只查出前100个goods_id */

        $sql = "SELECT DISTINCT g.goods_id FROM ".$xak->table('goods_attr')." AS g, ".$xak->table('attribute')." AS a WHERE g.attr_id = a.attr_id AND a.cat_id = '$cat_id' LIMIT 100";
        $in_goods = $db->GetCol($sql);
        $in_goods = 'AND g.goods_id '.db_create_in(implode(',', $in_goods));
    ...
    /* 获取符合条件的属性 */
    $sql = "SELECT DISTINCT a.attr_id FROM ".$xak->table('goods_attr')." AS g, ".$xak->table('attribute') ." AS a ".
    ...
    /* 获取所有属性值 */
    $sql = "SELECT DISTINCT g.attr_id, a.attr_name, g.attr_value FROM ".$xak->table('goods_attr')." AS g, ".$xak->table('attribute') ." AS a WHERE a.attr_id = g.attr_id ".$in_attr.$in_goods;
    $rs = $db->query($sql);
...
$sql = "SELECT c.cat_id, c.cat_name, COUNT(g.goods_id) AS goods_num ".
       " FROM " . $xak->table('goods') . " AS g ".
       " LEFT JOIN " . $xak->table('category') . " AS c ON c.cat_id = g.cat_id ".
       " WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0  " . $in_goods .
       " GROUP BY g.cat_id ";
