...
if ($_REQUEST['act'] == 'list')
{
    ...
    /* 取得批发商品总数 */
    $sql = "SELECT COUNT(*) FROM " . $xak->table('wholesale') . " AS w, " . $xak->table('goods') . " AS g " . $where;
    $count = $db->getOne($sql);

    if ($count > 0)
    {
        $default_display_type = $_CFG['show_order_type'] == '0' ? 'list' : 'text';
        $display  = (isset($_REQUEST['display']) && in_array(trim(strtolower($_REQUEST['display'])), array('list', 'text'))) ? trim($_REQUEST['display']) : (isset($_COOKIE['XAK']['display']) ? $_COOKIE['XAK']['display'] : $default_display_type);
...
elseif ($_REQUEST['act'] == 'price_list')
{
    ...
    $sql = "SELECT * FROM " . $xak->table('wholesale') .
    ...
    while ($row = $db->fetchRow($res))
    {
        ...
        foreach ($price_list as $attr_price)
        {
            if ($attr_price['attr'])
            {
                $sql = "SELECT attr_value FROM " . $xak->table('goods_attr') .
    ...
    {
        echo xak_iconv('UTF8', 'GB2312', $data);
    }
    else
    {
        echo $data;
...
elseif ($_REQUEST['act'] == 'add_to_cart')
{
    ...
    /* 刷新页面 */
    xak_header("Location: ./wholesale.php\n");
    exit;
}
...
elseif ($_REQUEST['act'] == 'submit_order')
{
    ...
    foreach ($_SESSION['wholesale_goods'] as $goods)
    {
        ...
        if (!empty($goods['goods_attr_id']))
        {
            ...
            $sql = "SELECT product_id FROM " . $xak->table('products') . " WHERE goods_attr = '$goods_attr' AND goods_id = '" . $goods['goods_id'] . "'";
...
function wholesale_list($size, $page, $where)
{
    ...
    $sql = "SELECT w.*, g.goods_thumb, g.goods_name as goods_name " .
            "FROM " . $GLOBALS['xak']->table('wholesale') . " AS w, " .
                      $GLOBALS['xak']->table('goods') . " AS g " . $where .
...
function get_price_ladder($goods_id)
{
    ...
    $sql = "SELECT prices FROM " . $GLOBALS['xak']->table('wholesale') .
